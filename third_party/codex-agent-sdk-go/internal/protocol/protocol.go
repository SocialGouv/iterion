...
func (c *Controller) readLoop(
	...
	for {
		...
		case msg, ok := <-messages:
			if !ok {
				// Transports buffer a fatal read error on errs just before
				// closing both channels. When both are ready the select picks
				// randomly, so drain errs here or the error is lost about half
				// the time and the query ends silently with no result.
				c.drainTransportError(errs)
		...
		case err, ok := <-errs:
			if !ok {
				// errs closing is not a shutdown signal on its own: the
				// transports close errs BEFORE messages, which may still
				// hold buffered events (e.g. a late turn.completed). Stop
				// selecting on errs and keep draining messages.
				errs = nil
...
// drainTransportError performs a non-blocking read of a pending transport
// error so it survives the messages channel closing first.
func (c *Controller) drainTransportError(errs <-chan error) {
	select {
	case err, ok := <-errs:
		if ok && err != nil {
			c.log.Debug("transport error in protocol", slog.String("error", err.Error()))
