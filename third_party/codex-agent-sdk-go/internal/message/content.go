...
// MarshalJSON emits the localImage discriminator required by the live
// app-server turn/start schema. The snake_case local_image spelling is
// rejected by current codex versions ("unknown variant `local_image`,
// expected one of `text`, `image`, `localImage`, `skill`, `mention`");
// it remains accepted on input for backward compatibility.
