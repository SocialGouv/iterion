...
const (
	// initialScanBufferSize is the scanner's starting allocation; bufio
	// grows it on demand up to maxScanTokenSize.
	initialScanBufferSize = 1024 * 1024 // 1MB
	// maxScanTokenSize is the maximum buffer size for reading CLI output
	// lines. Image-generation events inline the full image as base64 in a
	// single line (several MB per image), so this must stay far above 1MB.
	maxScanTokenSize = 64 * 1024 * 1024 // 64MB
...
func (t *CLITransport) ReadMessages(
	...
	go func() {
		...
		// Set large buffer for big messages; start small and let bufio
		// grow on demand rather than pre-allocating the full cap.
		buf := make([]byte, 0, initialScanBufferSize)
