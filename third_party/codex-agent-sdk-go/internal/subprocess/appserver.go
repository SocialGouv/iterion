...
// appServerScannerInitialBuffer is the scanner's starting allocation;
// bufio grows it on demand up to appServerScannerBuffer.
const appServerScannerInitialBuffer = 1 << 20 // 1 MB

// appServerScannerBuffer caps a single JSON-RPC line read from the
// app-server. Image-generation events inline the full image as base64
// in one line (several MB for a single 1024px PNG), so this must be
// far above the historical 1 MB or the scanner aborts mid-session.
const appServerScannerBuffer = 64 << 20 // 64 MB
...
func (t *AppServerTransport) Start(ctx context.Context) error {
	...
	t.stdout.Buffer(
		make([]byte, 0, appServerScannerInitialBuffer),
...
// ReadErr reports a fatal stdout read failure (e.g. a JSON-RPC line
// exceeding the scanner buffer). It returns nil after a graceful Close,
// where the aborted read is expected.
func (t *AppServerTransport) ReadErr() error {
	t.mu.Lock()
	defer t.mu.Unlock()
...
// recordReadErr captures the scanner error that terminated readLoop.
// Without this, a failure such as bufio.ErrTooLong silently ends the
// session: the process looks like it exited cleanly and the query
// returns no result at all. Errors during an intentional Close are
// expected (the pipe is torn down) and are not recorded.
func (t *AppServerTransport) recordReadErr() {
	err := t.stdout.Err()
	if err == nil {
		return
	}
	...
	t.log.Error("app-server stdout read failed, terminating session",
		slog.String("error", err.Error()),
	)
}
