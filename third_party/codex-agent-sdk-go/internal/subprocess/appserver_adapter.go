...
// surfaceReadErr forwards a fatal transport read failure (e.g. a JSON-RPC
// line overflowing the stdout scanner) to the errs channel so the query
// loop reports it instead of ending silently without a result message.
func (a *AppServerAdapter) surfaceReadErr() {
	type readErrer interface{ ReadErr() error }

	re, ok := a.inner.(readErrer)
	if !ok {
		return
	}
	...
	select {
	case a.errs <- fmt.Errorf("app-server transport read failed: %w", err):
	default:
		a.log.Warn("errs channel full, dropping transport read error",
			slog.String("error", err.Error()),
		)
	}
}
