...
- `pkg/usagecap/` — Caps an LLM **subscription** below the provider's own wall, so bots stop before they eat the forfait an operator also works on. The cap is a property of the CREDENTIAL and of the deployment that owns it, never of a run: configured machine-wide, with deliberately no per-run flag and no DSL field so a bot cannot lift the guard. Two windows, each with its own posture — `ITERION_USAGE_CAP_5H_PCT` (`DefaultFiveHourMode` = `soft`: blocks work that has not started, work in flight runs on) and `ITERION_USAGE_CAP_WEEK_PCT` (`DefaultWeekMode` = `hard`: additionally stops work in flight at the next observation) — plus `ITERION_USAGE_CAP` (`off` kills both), `ITERION_USAGE_CAP_TRUST_WINDOW` (how long a stored reading is believed after it was OBSERVED — the same ledger backs the cap pre-flight and the credential-skip evidence, so both must forget a pre-reset reading at the same instant) and `ITERION_USAGE_CAP_REFUSAL_REST_MAX` (ceiling on the escalating rest an account-level refusal earns; `off`/`0` restores the flat one-hour re-probe). A malformed value is an ERROR, not a fallback — every wrong answer here fails open, and a guard that silently stopped guarding because of a typo is worse than one that refuses to start. `PolicySource` is the seam that lets an enforcement point read a value changing under it: `StaticPolicy` (env-only deployments and tests) and the DB-backed `Resolver` (ADR-090 — the percentages, and only the percentages, are runtime-mutable; the MODES and the kill switch stay env-only because a posture change is a decision a redeploy should witness, and `ITERION_USAGE_CAP=off` WINS over a DB percentage so an override can never re-arm a disarmed guard). Consumed by the runner's admission `Preflight` + live `Guard` and by the `claude_code`/`pi` delegates. See [docs/usage-caps.md](docs/usage-caps.md)
...
- `pkg/worktreepool/` — Classifies and reclaims the FULL checkouts `worktree: auto` parks under `<store>/worktrees/`. `Scan` orders a store's worktrees oldest-first (the head is what an operator is least likely to want back) and classifies each in full; `Level` (`conservative` → `moderate` → `aggressive`) then decides which verdicts are *reported* as candidates — classification never gates. Two callers ask different questions of the same facts, which is why the `admission` predicate travels with the sweep: `iterion clean` asks "is this inside the level the operator chose", the automatic **pool bound** asks "can this be taken with nothing lost at all". The bound runs on the launch path — ceiling `ITERION_WORKTREE_POOL_MAX` (`DefaultBudget` 8, or `off`), a 10s `DefaultEnforcementTimeout`, at most `maxClassificationsPerPass` (4) subprocess-heavy git inspections per pass with a persisted cursor so bounding one launch never starves later candidates. Live-run-held worktrees are never candidates and never count against the budget. A pass reports (never logs internally) a `BudgetReport` — reclaimed, spared-by-reason, `Incomplete`, `Limited` — so the caller decides how loud to be. See [docs/worktree-pool.md](docs/worktree-pool.md)
...
- `pkg/routing/` — The **launch-frozen outcome contract**'s reading: `Evaluate(run) Verdict` decides a terminal run's `RoutingPolicy` as `merge` | `relaunch` | `escalate`. A **pure decision function** — no I/O, no state, no action — so the launch surfaces that validate a policy (`pkg/server/runs_launch.go`, `pkg/runview/service_launch.go`) and the reactor that acts on one (`pkg/server/outcome_router.go`) share exactly one reading of a contract. `escalate` is the default: every uncertain path (an expression that cannot be read strictly, a failure the policy does not permit acting on, a policy allowing nothing) lands there, never on merge. `Validate`/`ValidateRefs` refuse an unparseable or dangling-ref policy at launch; `CurrentPolicyVersion` (1) is what gets frozen onto the run. `DecisionRelaunch` is declared, validated and persisted but **not applied** — no shipped consumer enforces `MaxRelaunches` yet, and a consumer wiring relaunch must ship the counter with it. See [docs/outcome-router.md](docs/outcome-router.md)
...
- `pkg/operatormcp/` — The **operator-facing** MCP tool surface served by `iterion mcp` — the seam that lets any MCP client (Claude Code, the desktop, an IDE) drive iterion end to end. Two families share one server: `local_*` (the local store and engine — validate/launch/follow runs, the native kanban board, bot discovery; reads go straight to the run store, launches spawn a **detached** `iterion run|resume --background` subprocess so the run outlives the client's session) and `remote_*` (a logged-in instance over its HTTP API — a typed core plus the `remote_api` escape hatch and route/OpenAPI discovery, mirroring the `iterion remote` CLI positioning). `Family` (`--only local|remote`) and `ReadOnly` (`--read-only`) shape the registry at build time; a tool's `ReadOnly` flag feeds the MCP `readOnlyHint` annotation so it must be TRUTHFUL, and `ListedInReadOnly` is what keeps `remote_api` exposed-but-GET-only instead of hidden. A `spawnGate` serializes the check-then-spawn section of `local_run`/`local_resume` so two concurrent calls cannot both pass the live-runner check and clobber each other's `.pid`. Following the `boardops` precedent the package owns the tool definitions and dispatch; the stdio JSON-RPC framing lives with the other MCP servers in [cmd/iterion/mcp.go](cmd/iterion/mcp.go). See [docs/mcp-server.md](docs/mcp-server.md)
...
iterion clean [--level conservative|moderate|aggressive] [--older-than 168h] [--keep-last N] [--apply] [--all-projects] [--include-resumable] [--with-runs] [--store-dir]  # Reclaim disk by deleting run worktrees whose work has landed. DRY RUN without --apply. Safety is what git can PROVE about the commits (merged / own-branch / unlanded / orphan / nested-repo), never age or run status alone; `runs prune` does not help, it only touches <store-dir>/runs/ (docs/worktree-pool.md)
...
iterion secret set|list|rm [--store-dir] [--project]  # Local (desktop / headless) secret store — `set` reads the value from a prompt, stdin or `--from-env`, with `--hosts` (egress allowlist) and `--kind token|json|pem|raw`; `list` prints names + last4, never values (see docs/secrets.md)
iterion plugin list|info|config|enable|disable|run|install|uninstall  # Plugin ecosystem (rewriters, MCP servers, skills/commands/agents, hooks, lifecycle). Builtins are embedded; installed plugins live under ~/.iterion/plugins/ with enable state in ~/.iterion/plugins.yaml (see docs/plugins.md)
iterion supervise --run-id <id> | --claude-session <cwd|id> [--node] [--system] [--monitor] [--model] [--cooldown 30s] [--max-evals 20] [--name] [--store-dir]  # Attach an LLM supervisor to a running run, or to a raw Claude Code session; `supervise install-hook|uninstall-hook [--cwd] [--project]` wires the raw-session path (see docs/supervisors.md)
