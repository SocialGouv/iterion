...
- `pkg/workspacetrack/` — **workspace versioning**: content-addressed capture/restore of the files a run produces, stored under `<store>/runs/<id>/workspace/` (objects + chained snapshot manifests + a stat cache). Backs `iterion rewind`'s file restore for runs with **no** isolated worktree — the default shape, where git cannot serve because the workspace is the operator's live checkout and `git add -A` would stage their own work. `worktree: auto` runs keep the per-node git snapshots. Ignore rules are iterion's own (`.iterionignore`, falling back to `.gitignore`). See [docs/workspace-versioning.md](docs/workspace-versioning.md)
