...
- `pkg/backend/` — Execution stack (LLM + tools)
  ...
  - `delegate/` — Backend interface and CLI delegates (`claude_code`, `pi`, `kimi`, `grok`, legacy `codex`); `claw` implements the same interface in-process under `model/`
...
| Type | Description |
|------|-------------|
| **Agent** | LLM node with tools, structured I/O, and any selected backend (`claw`, `claude_code`, `pi`, `kimi`, `grok`, or legacy `codex`) |
...
- `ClawExecutor` (`pkg/backend/model/executor.go`) — production `NodeExecutor` impl, dispatches through `delegate.Backend` to `claw`, `claude_code`, `pi`, `kimi`, `grok`, or legacy `codex`; direct generation used by human nodes calls `pkg/backend/model/generation.go` (`GenerateTextDirect` / `GenerateObjectDirect`).
