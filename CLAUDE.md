...
- `pkg/cli/` — CLI command implementations (validate, run, inspect, resume, diagram, studio, report, dispatch, issue, bench, bots, bundle, sandbox, schedule, marketplace, remote, secret, version)
...
- `pkg/runshell/` — Interactive post-mortem shells in preserved run worktrees (the studio's "Open shell" on a failed run): one PTY-backed `Session` per WebSocket, the worktree is the state so a reconnect is a fresh shell. Unix-only (`ErrUnsupported` → HTTP 501 on the Windows/desktop build). See [docs/post-mortem-shell.md](docs/post-mortem-shell.md)
- `pkg/artifactlabels/` — Shape-derived semantic labels for a published artifact (`plan`, `verdict`) so it groups under "Plans"/"Verdicts" in the studio with no authoring effort; `Classify` returns each matching label independently and must stay in sync with the studio's render-time detection (`ArtifactDiff.tsx` `isVerdictShaped`/`isPlanShaped`)
