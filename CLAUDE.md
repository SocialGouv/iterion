...
- `pkg/errtrack/` — Error tracking + tracing over one sentry-go client (Sentry or GlitchTip), enabled only when `SENTRY_DSN` is set and a no-op otherwise: panic capture (`TrackPanic`/`Go` guard a detached goroutine, whose panic no top-level recover can ever see), fatal exits, error logs, scrubbing, and — as a SECOND opt-in on `SENTRY_TRACES_SAMPLE_RATE` — one span per API request and per in-process LLM call. Extend this package rather than adding a second tracker. See [docs/observability.md](docs/observability.md)
...
- `pkg/usagecap/` — Stops a deployment from spending an LLM **subscription** down to the provider's own wall, by reading the utilization each call reports for the two rolling windows (5h / 7d) and enforcing a percentage the operator chose. Two postures, because the windows fail differently: **soft** (the 5h default) starts nothing new but never interrupts work in flight; **hard** (the weekly default) stops the run where it stands. A Stop becomes the same usage-window error the provider would have raised, so the existing park-and-retry path carries it. The cap is a property of the CREDENTIAL and the deployment, never of a run — machine-wide `ITERION_USAGE_CAP{,_5H_PCT,_5H_MODE,_WEEK_PCT,_WEEK_MODE}` with no per-run flag and no DSL field, so a bot cannot lift the guard protecting the operator's own quota. A malformed value is a startup ERROR, not a silent fall back to "no cap". Percentages are also runtime-mutable over a DB record (ADR-090, `iterion remote admin caps set`). See [docs/usage-caps.md](docs/usage-caps.md)
...
- `pkg/worktreepool/` — Bounds the disk a long-lived store's `worktree: auto` checkouts take. Classifies each parked worktree once (`inspectGit` → landing / dirty / durably-held) and lets two callers ask different questions of the same facts through one `admission` predicate: `iterion clean` asks "is this inside the level the operator chose", the automatic pool bound asks "can this be taken with NOTHING lost". It counts only worktrees no live run owns — a running checkout is never reclaimed — so `DefaultBudget` (8, `ITERION_WORKTREE_POOL_MAX`) caps the PRESERVED pool, not concurrency. See [docs/worktree-pool.md](docs/worktree-pool.md)
...
- `pkg/routing/` — The single reading of a run's **launch-frozen outcome contract** (`store.RoutingPolicy`: `success_when`, `block_when[]`, `merge_into`, `merge_strategy`, `allowed_actions`, `max_relaunches`, and a `hash` pinning the resolved contract at launch). `Evaluate` is a PURE decision function over the terminal run — no I/O, no state, no action — returning a `Verdict{Decision, Reason, PolicyHash}` so every consumer reads one contract identically and an audit can replay WHY. **Escalate is the default and strictness is the design**: no policy, a contract newer than `CurrentPolicyVersion`, no terminal outputs, any blocker true *or unreadable*, or a `success_when` whose refs are absent/non-bool all land on escalate rather than merge — each REF is resolved and type-checked individually, since `!`/`&&`/`||` would otherwise coerce an absent field into `true`. The consumer still applies its own preconditions (banked-branch integrity, continuation ownership, relaunch budget) before acting. See [docs/outcome-router.md](docs/outcome-router.md)
