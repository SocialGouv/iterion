...
| `secret_kv` | a secret word starting a name part — snake, kebab, dotted, quoted or bracketed — or a camelCase part (`dbPassword`, `DBPassword`, `dbPass`), with only the suffixes a secret name takes (`_KEY`, `_VALUE`, `PHRASE`, a number, an environment: `SECRET_KEY=…`, `PASSWORD_2_PROD=…`, `PASSPHRASE=…`, `TOKEN_PROD=…`), then `=`, `:` or `=>` (`"api_key": "…"`, `[password] => …`); a quoted value runs to its closing quote (an escaped `\"` stays inside; a quote never closed runs to the end of the line unless the tail reads as code or as a sentence quoting the key — `logger.debug("token=" + token);` and `stopped at token=" near offset 12` are prose); an unquoted value rides the plain tokens after it (`password: hunter2 sekrit7` redacts the whole tail; a token holding `=` — `user=42` — ends it); a flag `--password …`, `--db-pass …`, `--passphrase …`; SQL `IDENTIFIED BY '…'` / `WITH PASSWORD '…'`; a URL's credentials `scheme://user:…@` (the password from 4 characters — shorter DSN passwords are a documented bound). Not a secret: a logger's own mask or a placeholder (`[Redacted]`, `********`, `${DB_PASSWORD}`, `CHANGE_ME`), a word of 12 letters or fewer (a status or a usage text: `expired`, `requires`), a lowercase snake_case value (`closed_for_winter` under a `mountain_pass=` status key), a path **shape** (a relative prefix `./`, `../`, `~/`, or a system prefix `/run/secrets/db` — a base64 token may start with `/` and hold more slashes, and is redacted), a count under a `…_token` key (`"prompt_token": 1234`), an API page token (`NextToken=`, `page_token=`); `total_tokens=…`, `PASSWORD_FILE=…` are not secret keys | key kept, value replaced |
