...
```iter fragment
tool commit_changes:
  command: `git add -A -- . ':(exclude).claude' && git commit -m {{input.msg}}`   # one string, run through `bash -c`
...
```iter fragment
tool commit_changes:
  command: `git add -A -- . ':(exclude).claude' && git commit -F - <<< {{input.msg}}`
  goal: "Commit the upgrade; working tree clean except known caches."
  postcondition: `cd {{input.workspace_dir}} && ! git status --porcelain -- . ':(exclude).claude' | grep -q . && printf '{"sha":"%s"}' "$(git rev-parse HEAD)"`
