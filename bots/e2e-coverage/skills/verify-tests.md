---
name: verify-tests
description: Detect and run a repository's OWN test suite so a deterministic gate can confirm the new tests actually pass before the campaign commits. Stack-agnostic — read this when adding tests or asked to verify them.
---

<!-- TODO(skill-dedup): duplicated across bundles — peers:
     bots/test-coverage/skills/verify-tests.md. iterion has no
...
This is the floor of the whole bot. The doctrine judges whether your
tests are *meaningful*; this gate proves they at least *compile and
pass*. A coverage bot whose tests don't run is a façade by definition.
...
- **Task runner / Makefile** — `Taskfile.yml` → `task test`; `Makefile` →
  ...
  `<pkgmgr> test`. Almost always the right answer when present. List the
  targets that actually exist (`task --list`, `make -qp`) — a dedicated
  e2e target (`test:e2e`, `test:integration`) is exactly what an
  e2e-coverage run should include.
...
- **Language defaults (only when there is no wrapper)** — pick the runner
  the repo already uses (look at existing tests + CI). When in doubt,
  read `.github/workflows/*` (or other CI config): CI is the source of
  truth for how this repo tests itself.

**Scope the run to keep the loop fast — but never scope OUT the suite
your new tests live in.** Include the whole-module compile + the suite(s)
containing the tests this campaign added (for an e2e campaign that means
the e2e suite itself). Scope OUT the unrelated slow layers: opt-in
live/real-credential suites, benchmark jobs — unless those are exactly
what was requested.

## 2. Only add a harness when the repo has none

If the repo already has an e2e/integration harness, **plug into it** —
its fixtures, its stubs, its naming. If the repo has NO test system at
all, introduce the single most idiomatic runner for its stack and wire
the minimal config its ecosystem expects — and flag that bootstrap
...
**coverage** bot this is a near-certain sign you should bootstrap the
repo's first harness using its idiomatic runner (see §2).
