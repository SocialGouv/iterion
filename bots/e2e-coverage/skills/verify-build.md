---
name: verify-build
description: Detect and run a repository's OWN build + test so a deterministic gate can confirm the tree actually compiles and passes before the campaign's work counts. Stack-agnostic — read this when asked to verify a build.
---

<!-- TODO(skill-dedup): duplicated across bundles — peers:
     bots/test-coverage/skills/verify-build.md (and siblings in the loop
...
This is the backstop for the campaign's deterministic gate. A change that
breaks compilation in *another* file (a renamed or retyped symbol with a
caller the current pass never exercised) or a bug only a test catches is
invisible to a per-chunk read. The build + tests are what catch it — that is
...
- **Pinned toolchain — honour it or the build fails on a version mismatch.**
  ...
  - **devbox is first-class inside the iterion sandbox** — when the repo
    ...
    work normally. Last-resort only: if the wrapper still fails for a
...
common breakage) + run the relevant tests. For an e2e-coverage campaign the
suite the new e2e tests live in is NOT optional — include it; scope OUT the
unrelated slow layers (real-credential/live suites) instead.
...
The deterministic gate re-runs **this** script and gates the run on its real
...
- `rm`, move, truncate, or recreate `.git` (or `.hg`/`.svn`), and NEVER run
  ...
  **disconnects the operator's worktree and strands their commits**.
