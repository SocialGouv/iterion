...
// TestCampaignBotsPerseveranceCoach guards Persy — the perseverance-coach
// supervisor — on every campaign bot, non-vacuously. The supervisor
// cross-reference diagnostics (C190 unknown watched node, C191 malformed
// monitor, C193 unknown system prompt) are WARNINGS by design — a typo'd
// `watches:`, `monitors:` or `system:` still compiles clean and the coach
// silently never arms. This test pins what each bot promises: a supervisor
// exists, it watches the campaign agent node with pre-seeded monitors, its
// policy prompt is declared, and the campaign contract keeps the
...
//
// The list is planPhaseBots: the seven ADR-058 campaign bots share the one
// `campaign` agent Persy watches. Adding a campaign bot means adding it
// there so the coach is guarded for it too.
func TestCampaignBotsPerseveranceCoach(t *testing.T) {
	for _, bot := range planPhaseBots {
		t.Run(bot, func(t *testing.T) {
			...
			if len(wf.Supervisors) == 0 {
				t.Fatalf("%s declares no supervisor — the perseverance coach is missing", bot)
			}
			...
			// its lane silently. The fleet's set is pinned here instead.
			...
			if campaign == nil {
				t.Fatalf("%s has no campaign agent node", bot)
			}
			...
			if !strings.Contains(sys.Body, "PERSISTENCE") {
				t.Errorf("%s: campaign contract has no PERSISTENCE clause — the coach enforces a policy the agent was never given", bot)
			}
		})
	}
}
