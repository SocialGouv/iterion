...
<!-- TODO(skill-duplication): iterion has no skill-sharing primitive; this file is
shipped verbatim in two bundles — bots/modernize/skills/ (the bot that EXECUTES the
contract) and bots/assessment/skills/ (the bot that WRITES it). A Go test pins the two
copies byte for byte, so an edit to either reddens until both carry it. -->
...
[
  ...
  {"class":"template_attr_semantics","crossing":"front framework or template engine",
   "ships_green_because":"the DOM still contains the element; only an attribute's rendered FORM changed, and no functional test reads attribute spelling — and on client-rendered stacks the HTTP capture sees the template only (a server-rendered stack DOES expose it to HTTP capture; probe in a rendered DOM either way)",
