...
[
  ...
  {"class":"template_attr_semantics","crossing":"front framework or template engine",
   "ships_green_because":"the DOM still contains the element; only an attribute's rendered FORM changed, and no functional test reads attribute spelling",
   "blind_spot":"boolean and enumerated attributes rendered differently (present-with-false vs absent), styling hooks that key on attribute presence",
   "derive":"from the notes, list attribute-rendering changes; grep templates for bindings of the affected attributes on element kinds where the semantics differ; render and inspect the actual attribute"},
