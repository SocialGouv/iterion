...
mid-run peer-unavailability behaviour: `skip` (default — the reviewer's
`action: skip` route: continue unreviewed, loudly stamped; the peer is an
optional enrichment and must never block the campaign — the primary family
