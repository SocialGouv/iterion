...
Open one session per distinct authorisation level, and always include an anonymous one.
...
Ids are stable and never reused — a reference file is named after its id, and renumbering
silently re-points every mutant's `targets`. Keep a `note` wherever the entry's purpose is not
obvious from its path; the next reader is deciding whether a diff matters, and the note is what
lets them.
