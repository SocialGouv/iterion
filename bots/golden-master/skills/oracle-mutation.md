...
- that relocation **declines, directory-wide**, as soon as anything under `mutants/holdout/` is
  ...
  in place, unsealed and readable, and every held-out figure then reads `0/0`. So never commit
  your own set in stride, and commit a successor set for a later gate only after a run has sealed
  yours out of the tree;
