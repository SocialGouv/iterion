...
def extension_verdict(ws, gm_rel, base, acted_commits=None, acted_blobs=None,
    ...
    if corpus_changed:
        ...
        if head_c is not None and base_c is not None:
            # `duplicate_groups` is judged apart, not exempted. It is the one
            # key outside `entries` an extension can be FORCED to touch: a lot
            # whose new entry captures a reference byte-identical to an
            # existing one must declare the separating mutant or the gate
            # refuses the net it just extended — a gate refusing and naming a
            # remedy another judge refuses, which is the shape this obligation
            # exists to remove, relocated one layer up.
            judged_apart = ("entries", "duplicate_groups")
            if {k: v for k, v in head_c.items() if k not in judged_apart} != \
                    {k: v for k, v in base_c.items() if k not in judged_apart}:
                corpus_problems.append(
            ...
            # THE DECLARATION CHANNEL, constrained rather than frozen.
            #
            # A declaration is not inert prose: it DISCHARGES a refusal.
            # Executed — a declaration naming two PRE-EXISTING entries, with no
            # entry added at all, flips the gate from REFUSED to GREEN. So an
            # actor free to write here could make an existing net greener
            # without extending it, which is the additions-only authority this
            # whole function holds. Three rules keep the channel to the case
            # that forces it open, and every one of them guards an ADDED
            # declaration, because adding is the only direction that can green
            # anything:
            #
            #   1. an added declaration is well-formed, because the gate's
            #      reader drops a malformed one in SILENCE and it would read as
            #      no declaration at all — an unconstrained free write into the
            #      key just unfrozen;
            #   2. an added declaration names at least one entry THIS extension
            #      added — the case that forces the channel open is exactly "my
            #      new entry collides with an existing one", and a group of two
            #      pre-existing entries means the base gate was already red,
            #      which an extension does not get to settle;
            #   3. no two declarations name the same group once one of them is
            #      added — the reader keys a dict on the group, so the second
            #      silently wins or loses an ordering nobody audits.
            #
            # REMOVAL IS ALLOWED, deliberately, and an earlier rule forbidding
            # it was withdrawn as soon as it was executed. Dropping a
            # declaration can only do one of two things: leave an OBSERVED
            # group undeclared, which the gate refuses — redder, not greener —
            # or clear a record whose references have since diverged, which is
            # the remedy the gate itself prints ("Remove the declaration").
            # Forbidding it made those two sentences contradict each other, and
            # since `duplicate_groups_unproven` is now a term of convergence
            # the net could not converge through the extension lane at all
            # until someone ran a re-anchor. Neither direction is exploitable:
            # whether a group is still observed is decided by MEASUREMENT, not
            # by the actor writing the file.
            #
            # A MODIFICATION is still refused, without a rule of its own — a
            # rewritten declaration is not present verbatim in base, so it is
            # judged as ADDED and rule 2 stops it unless this extension really
            # added one of its members.
            #
            # The separator itself needs no rule: naming one that does not move
            # part of the group is refused BY MEASUREMENT at the next gate,
            # which is the point of declaring it as data.
            #
            # Compared RAW, never through duplicate_group_decls: that reader
            # drops a malformed declaration silently, so comparing normalised
            # forms would let a base declaration be rewritten into an
            # unreadable one and still pass as "unchanged".
            head_dg = head_c.get("duplicate_groups", [])
            base_dg = base_c.get("duplicate_groups", [])
            if not isinstance(head_dg, list):
                corpus_problems.append(
                    "corpus.json `duplicate_groups` is not a list — refused, "
                    "not crashed")
            else:
                def _dg_ser(d):
                    return json.dumps(d, sort_keys=True, ensure_ascii=False, default=str)

                head_ser = [_dg_ser(d) for d in head_dg]
                base_ser = [_dg_ser(d) for d in base_dg] \
                    if isinstance(base_dg, list) else []
                bearers = {}
                for decl, s in zip(head_dg, head_ser):
                    added = s not in base_ser
                    kv = duplicate_group_key(decl)
                    if kv is None:
                        if added:
                            corpus_problems.append(
                                "added `duplicate_groups` declaration %s is "
                                "malformed (it needs at least two `ids` and a "
                                "non-empty `separated_by`) — the gate's reader "
                                "drops it in silence, so it would read as no "
                                "declaration at all" % s)
                        continue
                    bearers.setdefault(kv[0], []).append(added)
                    if added and not (set(kv[0]) & added_ids):
                        corpus_problems.append(
                            "added `duplicate_groups` declaration for %s names "
                            "no entry this extension added — a declaration "
                            "DISCHARGES a gate refusal, so writing one about "
                            "entries that already existed makes an existing "
                            "net greener without extending it"
                            % ", ".join(map(repr, kv[0])))
                for key, adds in sorted(bearers.items()):
                    if len(adds) > 1 and any(adds):
                        corpus_problems.append(
                            "%d `duplicate_groups` declarations name the same "
                            "group %s — the reader keys a dict on the group, "
                            "so the second silently wins or loses an ordering "
                            "nobody audits" % (len(adds), ", ".join(map(repr, key))))
...
def duplicate_group_key(decl):
    """One `duplicate_groups` declaration, normalised — or None if malformed.

    Malformed is: not an object, fewer than two ids, or no separator named.

    Per declaration rather than per corpus, because two readers need it and
    they need different views: the gate collapses the declarations into a
    by-group map, while the extension guard must see each one SEPARATELY (a
    second declaration for one group disappears into that map). One shape rule
    read by both, so a declaration the guard admits can never be one the gate
    silently ignores.
    """
    if not isinstance(decl, dict):
        return None
    ids = decl.get("ids")
    sep = decl.get("separated_by")
    if not isinstance(ids, list) or len(ids) < 2 or not isinstance(sep, str) or not sep:
        return None
    return tuple(sorted(str(i) for i in ids)), sep
...
def duplicate_group_decls(corpus):
    ...
    Refused, not crashed — the standard the extension guard already holds. This
    reader runs once per mutant, from inside `score_mutant` with the mutant
    APPLIED and its marker armed, so a `TypeError` on a non-list value escapes
    before any report is printed: a traceback instead of a verdict, no JSON at
    all, and a workspace left mutated with the leftover on record — the state
    the stop-on-leftover machinery exists to prevent. A value that is not a
    list declares nothing, and is read as nothing.
    """
...
def unproven_duplicate_groups(duplicate_refs, corpus, verdicts, restricted=False,
    ...
    `withheld` names separators whose verdict this pass reserves on purpose;
    they land in the caller's `deferred` sink instead of the refusal.
    """
    ...
    # WITHHELD is neither ABSENT nor PROVED, and both readings are wrong in a
    # way the campaign pays for. Read as ABSENT, the refusal says "add the
    # mutant to the set" about a mutant that IS in the set — a work item
    # nobody can action. Read as PROVED, the group is discharged by a
    # measurement that never happened.
    #
    # So: DEFERRED, in a sink the caller owns, never in `unproven`, which is
    # the field the gate reads. At the final gate this path is unreachable —
    # nothing is withheld there — so the obligation is unchanged where it
    # decides.
    #
    # No sink, no deferral. Without somewhere to say it, the honest answer is
    # the strict one: a caller that forgets the sink gets the HARSHER verdict,
    # never the softer. A default that can only tighten is the only kind safe
    # to add to a term of convergence.
    withheld = set(withheld) if deferred is not None else set()
    unproven = []
    for g in sorted(observed):
        ...
        if v is None:
            if sep in withheld:
                deferred.append({"ids": list(g), "separated_by": sep, "why":
                                 "the declared separator belongs to the held-out set, whose "
                                 "verdicts this pass reserves — not scored here, and not "
                                 "scorable here. Neither proved nor refuted until the final "
                                 "gate scores it."})
                continue
        ...
        elif len(inside) > 1 or len(still) > 1:
            # A STRICT SUBSET IS NOT ENOUGH ONCE THE GROUP EXCEEDS TWO, and
            # reading it as enough was a false green of the exact family this
            # obligation exists to remove: three byte-identical references, a
            # separator moving one, and the remaining PAIR reported proved
            # while nothing had ever told the two apart.
            #
            # The measurement compares each entry to its OWN reference — never
            # two entries to each other — so two members on the same side of
            # the split carry no evidence about each other, whether they both
            # moved or both stayed. Only a split into two singletons is a
            # separation this net has actually observed.
            unproven.append({"ids": list(g), "separated_by": sep,
                             "moved": sorted(inside), "why":
                             "the declared separator splits the group into %s and %s, and a "
                             "side holding more than one entry is NOT separated: the capture "
                             "compares each entry to its own reference, never two entries to "
                             "each other, so those references remain interchangeable on the "
                             "evidence. One mutant can only tell two apart — drop the "
                             "redundant entries, or make them observe something that differs."
                             % (sorted(inside), sorted(still))})
...
# How many undischarged records the refusal SPELLS OUT. The rest are counted,
# never dropped: they stay in the report's `duplicate_groups_unproven`, which is
# the field the gate reads. This exists because `log_tail` is a 6000-character
# tail over ALL problems, so one unbounded list silently evicts every other
# refusal in the run.
_DUP_SHOWN = 6
...
def duplicate_groups_refusal(unproven, duplicate_refs, corpus_distinct, corpus_total):
    ...
    The embedded list is CAPPED. `log_tail` keeps the last 6000 characters of
    every problem joined, and it is the campaign's only diagnostic (the gate
    publishes it as `fail_log`). Measured: twelve undischarged three-member
    groups render one 6444-character problem — over the whole budget by itself,
    so it evicts the negative control, the blind lanes, the score, and even its
    own header, leaving the reader a headless fragment of JSON. Every record is
    published intact in the report's `duplicate_groups_unproven`, which is what
    the gate actually reads; the log only has to name the problem well enough to
    act on it.
    """
    shown, hidden = unproven[:_DUP_SHOWN], max(0, len(unproven) - _DUP_SHOWN)
    return ("%d identity claim(s) are UNPROVED. Each record below carries its own reason, "
            ...
            "observations wide and not %d. Unproved: %s%s"
            % (len(unproven), len(duplicate_refs), corpus_distinct, corpus_total,
               json.dumps(shown, ensure_ascii=False),
               "" if not hidden else
               (" … and %d more, withheld so this refusal cannot evict the rest of the log; "
                "every record is in the report's `duplicate_groups_unproven`, which is the "
                "field the gate reads." % hidden)))
...
def score_mutant(meta, config, corpus, canon, refs, ws, seed):
    ...
    #
    # Pinned only if the entry EXISTS. A stale declaration names ids the corpus
    # no longer has; `capture` walks the corpus and skips them, so a phantom
    # would sit in the sample never captured and never compared — while
    # `control_covered` counts the SAMPLE. A mutant declaring the whole corpus
    # as targets has an empty control sample and is refused as vacuously clean;
    # one phantom would hand it a non-zero coverage and walk it past the single
    # check that catches that mutant.
    sample = control_ids(corpus, set(targets), seed)
    known = {e["id"] for e in corpus["entries"]}
    pinned = (separator_group_ids(corpus, meta.get("id")) & known) - set(targets)
    if pinned:
...
def _selftest():
    ...
    class World:
        ...
        def capture(self, config, corpus, canon, ids=None):
            # Le vrai `capture` PARCOURT le corpus et saute les ids qu'il ne
            # connait pas ; il ne lit pas la liste demandee. Le double doit
            # tolerer la meme chose, sinon un id fantome fait ici une
            # KeyError la ou la production compte silencieusement un temoin
            # qu'elle n'a jamais capture — le double masquerait la classe de
            # defaut au lieu de la mesurer.
            ids = set(ids) if ids is not None else set(self.refs)
    ...
    try:
        ...
        # LE CANAL DE DECLARATION, dans les deux sens.
        #
        # `duplicate_groups` est la seule cle hors `entries` qu'une extension
        # peut etre CONTRAINTE de toucher : un lot dont l'entree neuve capture
        # une reference byte-identique a une existante doit declarer le mutant
        # separateur, sinon la porte refuse le filet qu'il vient d'etendre. Gele
        # en bloc, on retrouvait exactement la forme que cette obligation
        # supprime — une porte qui refuse en nommant un remede qu'un autre juge
        # refuse — remontee d'un cran.
        #
        # Mais une declaration n'est pas de la prose : elle ACQUITTE un refus.
        # Mesure : une declaration portant sur deux entrees PREEXISTANTES, sans
        # aucune entree ajoutee, fait passer la porte de REFUSE a VERT. D'ou
        # quatre regles, falsifiees ici dans les deux sens.
        dg_req = ('{"id": "E-2", "lot": "L", "type": "add-entry",'
                  ' "corpus_entries": [{"id": "2"}]}')
        dg_act = ('{"id": "E-2", "lot": "L",'
                  ' "recorded_paths": [".golden-master/corpus.json"]}')

        def xdg(corpus, req=dg_req, act=dg_act):
            """Un lot depose sa demande, puis l'acte avec CE corpus."""
            xreset()
            xrequest(req)
            xledger(("request", req), ("act", act))
            with open(os.path.join(xgm, "corpus.json"), "w", encoding="utf-8") as f:
                json.dump(corpus, f)
            xcommit()
            return xverdict(xbase)

        # 1. Le cas legitime : l'entree ajoutee entre en collision, le lot la
        #    declare. C'est la seule forme qui doit passer.
        check("entree ajoutee + sa declaration necessaire -> ok",
              xdg({"entries": [base_entry, new_entry],
                   "duplicate_groups": [{"ids": ["1", "2"],
                                         "separated_by": "M-07"}]})["acted"][0]["ok"],
              True)
        # 2. La declaration SEULE, sans aucune entree ajoutee : c'est le
        #    depassement d'autorite — rendre vert un filet existant sans
        #    l'etendre.
        check("declaration seule, aucune entree ajoutee -> refusee",
              xdg({"entries": [base_entry],
                   "duplicate_groups": [{"ids": ["1", "9"],
                                         "separated_by": "M-07"}]})["acted"][0]["ok"],
              False)
        # 3. Une declaration ajoutee qui ne nomme AUCUNE entree de cette
        #    extension : meme depassement, deguise a cote d'un ajout reel.
        check("declaration a cote d'un ajout, mais sur des entrees anciennes -> refusee",
              xdg({"entries": [base_entry, new_entry],
                   "duplicate_groups": [{"ids": ["1", "77"],
                                         "separated_by": "M-07"}]})["acted"][0]["ok"],
              False)
        # 4. Deux declarations pour le MEME groupe : le lecteur en garde une,
        #    au hasard d'un ordre que personne n'audite.
        check("deux declarations pour un meme groupe -> refusee",
              xdg({"entries": [base_entry, new_entry],
                   "duplicate_groups": [{"ids": ["1", "2"], "separated_by": "M-07"},
                                        {"ids": ["2", "1"],
                                         "separated_by": "M-99"}]})["acted"][0]["ok"],
              False)
        # 5. Une declaration ajoutee MALFORMEE : le lecteur de la porte la
        #    laisse tomber en silence, donc elle vaudrait « pas de declaration »
        #    tout en ecrivant librement dans la cle qu'on vient de degeler.
        check("declaration ajoutee malformee -> refusee",
              xdg({"entries": [base_entry, new_entry],
                   "duplicate_groups": [{"ids": ["2"],
                                         "separated_by": "M-07"}]})["acted"][0]["ok"],
              False)

        # 6. Le sens du RETRAIT, qui n'est PAS symetrique de l'ajout. Retirer
        #    une declaration ne peut faire que deux choses : laisser un groupe
        #    OBSERVE sans declaration — que la porte refuse, donc plus rouge —
        #    ou effacer un enregistrement dont les references ont diverge, ce
        #    qui est le remede que la porte imprime elle-meme (« Remove the
        #    declaration »). L'interdire faisait se contredire ces deux phrases
        #    et, `duplicate_groups_unproven` etant un terme de convergence,
        #    fermait la lane d'extension jusqu'a un re-ancrage. Ni l'un ni
        #    l'autre sens n'est exploitable : c'est la MESURE, pas l'acteur, qui
        #    decide si un groupe est encore observe.
        #
        #    Une RETOUCHE, elle, reste refusee — sans regle propre : la
        #    declaration reecrite n'est plus verbatim dans la base, donc elle est
        #    jugee comme AJOUTEE et doit nommer une entree de cette extension.
        #    La base porte ici la declaration.
        xreset()
        with open(os.path.join(xgm, "corpus.json"), "w", encoding="utf-8") as f:
            json.dump({"entries": [base_entry],
                       "duplicate_groups": [{"ids": ["1", "5"],
                                             "separated_by": "M-01"}]}, f)
        fixture_git(xroot, "add", "-A")
        fixture_git(xroot, "commit", "-qm", "base avec declaration")
        ...
        def xdg_from(corpus):
            ...
            fixture_git(xroot, "clean", "-qfd")
            xrequest(dg_req)
            xledger(("request", dg_req), ("act", dg_act))
            with open(os.path.join(xgm, "corpus.json"), "w", encoding="utf-8") as f:
                json.dump(corpus, f)
            xcommit()
            return xverdict(xbase_dg)

        check("une extension qui laisse la declaration de base intacte -> ok",
              xdg_from({"entries": [base_entry, new_entry],
                        "duplicate_groups": [{"ids": ["1", "5"],
                                              "separated_by": "M-01"}]})["acted"][0]["ok"],
              True)
        check("declaration de base retiree -> ok, c'est le remede que la porte imprime",
              xdg_from({"entries": [base_entry, new_entry],
                        "duplicate_groups": []})["acted"][0]["ok"], True)
        # Et la retouche reste refusee, sans regle dediee : la declaration
        # reecrite n'est plus verbatim dans la base, donc elle est jugee comme
        # ajoutee et ne nomme aucune entree de cette extension.
        check("separateur d'une declaration de base reecrit -> refuse",
              xdg_from({"entries": [base_entry, new_entry],
                        "duplicate_groups": [{"ids": ["1", "5"],
                                              "separated_by": "M-99"}]})["acted"][0]["ok"],
              False)
        # Retirer une declaration dont le groupe est ENCORE observe ne verdit
        # rien : la porte refuse alors « aucune entree ne declare ce groupe ».
        # C'est la mesure qui tranche, pas l'ecriture.
        check("un groupe encore observe et prive de sa declaration reste refuse par la porte",
              [x["ids"] for x in unproven_duplicate_groups(
                  [["1", "5"]], {"entries": [], "duplicate_groups": []}, [])],
              [["1", "5"]])
        # Et la cle hors `entries` qui reste GELEE : rien ne l'epinglait, donc
        # degeler `duplicate_groups` pouvait emporter le reste sans bruit.
        check("une AUTRE cle hors `entries` reste refusee",
              xdg({"entries": [base_entry, new_entry],
                   "seed": 7})["acted"][0]["ok"], False)
    ...
    # REFUSE, PAS PLANTE — le standard que la garde d'extension tient deja. Ce
    # lecteur tourne une fois par mutant, depuis score_mutant, mutant APPLIQUE
    # et marqueur arme : une TypeError s'en echappe avant tout rapport, laissant
    # une traceback au lieu d'un verdict et un arbre mute avec le reliquat au
    # dossier.
    for _bad in (5, True, 1.5, "pas-une-liste", {"ids": ["a", "b"]}, None):
        check("duplicate_groups=%r ne declare rien, et ne plante pas" % (_bad,),
              duplicate_group_decls({"duplicate_groups": _bad}), {})
    ...
    check("aucun epinglage pour un mutant qui ne separe rien",
          v_nopin["collateral"], [])
    # UN FANTOME NE CONTROLE RIEN. Une declaration caduque nomme des ids que le
    # corpus n'a plus : `capture` parcourt le corpus et les saute, donc ils ne
    # sont ni captures ni compares — alors que `control_covered` compte
    # l'ECHANTILLON. Un mutant qui declare tout le corpus en cibles n'a aucun
    # temoin et se fait refuser comme vacuement propre ; un fantome lui
    # donnerait une couverture non nulle et le ferait passer devant le seul
    # controle qui l'attrape.
    v_ghost = score(["%03d" % n for n in range(1, 13)], [], ["001"],
                    dup_groups=[{"ids": ["777", "888"], "separated_by": "t"}])
    check("une declaration caduque n'invente pas de couverture de temoin",
          v_ghost["control_covered"], 0)
    ...
    # AU-DELA DE DEUX, un sous-ensemble strict ne prouve plus rien. Trois
    # references identiques, un separateur qui en deplace UNE, et la PAIRE
    # restante passait pour prouvee alors que rien ne l'avait jamais separee —
    # le faux vert exact que cette obligation existe pour supprimer. La mesure
    # compare chaque entree a SA reference, jamais deux entrees entre elles.
    T = [{"ids": ["012", "013", "014"], "separated_by": "sep-01"}]
    check("groupe de trois, separateur qui en deplace une : refuse",
          whys(["012", "013", "014"], T, {"sep-01": {"012"}}),
          [["012", "013", "014"]])
    check("groupe de trois, separateur qui en deplace deux : refuse aussi",
          whys(["012", "013", "014"], T, {"sep-01": {"012", "013"}}),
          [["012", "013", "014"]])
    check("et le refus nomme les deux cotes de la coupe",
          unproven_duplicate_groups(
              [["012", "013", "014"]],
              {"entries": [], "duplicate_groups": T},
              [{"id": "sep-01", "valid": True, "targets_declared": ["012"],
                "undetected_targets": [], "collateral": []}])[0]["moved"],
          ["012"])
    ...
    # UN SEPARATEUR TENU A L'ECART N'EST NI ABSENT NI PROUVE. En selfcheck le
    # jeu held-out n'est pas score PAR CONSTRUCTION, donc un separateur qui en
    # vient se lirait « absent du jeu de mutants » : faux, et infixable par la
    # campagne — le mutant EST dans le jeu, son verdict est reserve. Le lire
    # comme PROUVE serait pire : decharger une classe que rien n'a mesuree.
    corpus_h = {"entries": [], "duplicate_groups": D}
    sink = []
    check("separateur tenu a l'ecart : aucun refus",
          [x["ids"] for x in unproven_duplicate_groups(
              [["012", "013"]], corpus_h, [], False, ["sep-01"], sink)],
          [])
    check("... et il est DIFFERE, pas efface",
          [x["ids"] for x in sink], [["012", "013"]])
    # SANS PUITS, PAS DE REPORT : un appelant qui oublie le sink obtient le
    # verdict le PLUS SEVERE, jamais le plus doux. Un defaut qui ne peut que
    # resserrer est le seul qu'on ajoute sans danger a un terme de convergence.
    check("sans puits, un separateur tenu a l'ecart redevient un refus",
          [x["ids"] for x in unproven_duplicate_groups(
              [["012", "013"]], corpus_h, [], False, ["sep-01"])],
          [["012", "013"]])
    # A LA PORTE FINALE RIEN N'EST TENU A L'ECART : la voie du report y est
    # inatteignable, donc elle ne peut pas adoucir le verdict qui decide.
    sink2 = []
    check("porte finale : aucun report possible, le refus tient",
          [[x["ids"] for x in unproven_duplicate_groups(
              [["012", "013"]], corpus_h, [], False, (), sink2)], sink2],
          [[["012", "013"]], []])
    # ET LE VERDICT D'UN HELD-OUT SCORE VAUT PREUVE : il n'a rien de special
    # une fois mesure.
    #
    # CE QUE CE BANC NE COUVRE PAS, ET IL FAUT LE DIRE : que l'APPELANT joigne
    # bien `held` aux verdicts visibles. C'est pourtant la moitie la plus grave
    # du correctif — sans elle un separateur scelle est INPROUVABLE A JAMAIS.
    # Le verifier demanderait une porte de bout en bout (application, captures,
    # references, mutants qui s'appliquent et se revertissent) ; la fixture du
    # selftest est unitaire. Un banc absent qu'on nomme vaut mieux qu'un banc
    # absent qu'un commentaire declare present.
    check("un separateur held-out SCORE decharge le groupe",
          [x["ids"] for x in unproven_duplicate_groups(
              [["012", "013"]], corpus_h,
              [{"id": "sep-01", "valid": True, "targets_declared": ["013"],
                "undetected_targets": [], "collateral": []}])],
    ...
    check("la liste montree est celle qui est comptee",
          msg.count('"ids"'), 1)
    # ET ELLE EST BORNEE. `log_tail` ne garde que les 6000 derniers caracteres
    # de TOUS les problemes reunis, et c'est le seul diagnostic que la campagne
    # recoit (`fail_log`). Mesure : douze groupes de trois non acquittes
    # rendaient un seul probleme de 6444 caracteres — au-dela du budget entier,
    # donc il chassait le controle negatif, les lanes aveugles, le score, et
    # jusqu'a son propre en-tete. Rien n'est perdu : tout est dans le rapport.
    many = [{"ids": ["e%03d" % i, "e%03d" % (i + 1)], "separated_by": "M-%02d" % i,
             "why": "le separateur ne deplace aucun membre du groupe"} for i in range(20)]
    long_msg = duplicate_groups_refusal(many, [], 11, 14)
    check("le refus ne detaille que les premiers enregistrements",
          long_msg.count('"ids"'), _DUP_SHOWN)
    check("et il compte ceux qu'il retient",
          "and %d more" % (20 - _DUP_SHOWN) in long_msg, True)
    check("l'en-tete survit a la troncature de log_tail",
          long_msg[-6000:].startswith("20 identity claim(s) are UNPROVED"), True)
...
def main():
    ...
    report = {"mode": mode, "config": cfg_name,
              ...
              # Reported, never gated: a deferral is the absence of a verdict,
              # not a verdict. It exists so a selfcheck cannot read as PROVED
              # what it merely could not measure.
    ...
    try:
        ...
        # `held` BELONGS IN THE EVIDENCE, and leaving it out made a declared
        # held-out separator permanently unprovable: the final gate scores it
        # into `held`, never into `verdicts`, so this predicate could not see
        # the very measurement it demands. With the obligation wired to
        # convergence, that is not a wrong message — it is a corpus the gate
        # can never accept, whose only escape is to declare a VISIBLE
        # separator, which is the opposite of what a sealed set is for.
        #
        # `withheld` covers the other half, and only in selfcheck, where `held`
        # is empty BY CONSTRUCTION (the seal moves the files out and the loop
        # above does not run). Nothing is withheld at the final gate, so the
        # deferral path cannot soften the verdict that decides.
        withheld_ids = ([m.get("id") for m in held_meta]
                        if mode == "selfcheck" else [])
        deferred = []
        unproven = unproven_duplicate_groups(
            report["duplicate_refs"], corpus, list(verdicts) + list(held),
            bool(only), withheld_ids, deferred)
