...
def probe_mutation(meta, ws):
    ...
    if code != 0:
        ...
        if rcode != 0:
            ...
            # Same term score_mutant sets when a revert fails under it (and for
            # the same reason): an invalid mutant is dropped from the score, so
            # without this the run would go on scoring the REST of the lot
            # against a tree this one left half-mutated, and could still come
            # out green. revert_clean is the gate's own conjunction term.
