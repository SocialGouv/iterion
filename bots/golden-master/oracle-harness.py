...
def marker_dir_is_ours(marker):
    """True when the marker's directory is one only this user could have written.

    A marker names a script the next gate will run, and its path is
    PREDICTABLE: with GM_SCRATCH unset — the shipped default, the bot sets
    nothing — it is <system temp dir>/gm-applied, on a root shared with every
    other user of the machine. Whoever pre-creates that directory owns it: our
    makedirs(exist_ok=True) succeeds on it, our chmod raises EPERM, and we
    would go on to read THEIR marker and revert whatever it names. Containment
    bounds WHICH directories a marker may name; this bounds who may name one.

    lstat, not stat: a symlink in that position is never one we made, and its
    own 0o777 mode fails the privacy test here without a separate branch.
    """
    getuid = getattr(os, "getuid", None)
    if getuid is None:      # no POSIX ownership to read (Windows)
        return True
    try:
        st = os.lstat(os.path.dirname(marker))
    except OSError:
        return False
    return st.st_uid == getuid() and not st.st_mode & 0o077
...
def write_applied_marker(ws, meta):
    ...
    try:
        ...
        os.chmod(d, 0o700)      # the repair; the check below is the verdict
        if not marker_dir_is_ours(marker):
...
def revert_leftover_mutant(ws):
    ...
    A marker is an instruction to run a script, so it is REFUSED — reported,
    nothing executed — on either of two grounds: it names a directory that is
    not one of this workspace's `mutant_roots`, or it sits in a directory this
    user does not privately own.

    A refusal drops NOTHING. Reading is not deciding: whether the record may
    be forgotten depends on the tree, which this function does not look at —
    `settle_leftover` is where that call is made, once. Dropping it here made
    the refusal one-shot; the gate below would refuse a dirty tree, but the
    record was already gone, so the NEXT gate found nothing, said nothing, and
    judged a tree that may still carry the mutant.
    """
    marker = applied_marker_for(ws)
    if not os.path.isfile(marker):
        return None
    if not marker_dir_is_ours(marker):
        # Not ours to read — and not ours to delete either. Nothing is executed
        # and nothing is dropped. `write_applied_marker` refuses the same
        # directory, so no mutant can be applied while it stands; and for the
        # narrow case of a directory that WAS ours when the marker went down
        # and has changed hands since, a dirty tree turns this refusal into a
        # refusal to gate — see leftover_disposition.
        return {"id": None, "dir": "", "code": None, "out": "", "marker": marker,
                "refused": True, "untrusted_dir": True}
...
def dirty_paths(ws):
    ...
    "Does not answer" covers all three ways it can happen, not just a non-zero
    exit: git absent from PATH raises, and a cold index on a slow filesystem
    can sit there. Both were a traceback and a hang on the path that runs
    BEFORE the report is printed — and this report is the product, so dying
    there leaves the campaign parsing a stack trace instead of reading a
    verdict. Every other subprocess in this harness is already bounded.
    """
    try:
...
def leftover_disposition(left, dirty=()):
    ...
    if left.get("untrusted_dir"):
        ...
        kind, text = "refused", (
            ...
            "workspace's mutant set nor its sealed held-out pile (%s): nothing "
            "was executed." % (left.get("dir") or "?"))
    ...
    if dirty:
        return "still_mutated", (
            ...
            "hand (git checkout -- <the mutant's paths>), then delete the marker %s and "
            "gate — it is KEPT until you do, and every gate until then refuses the same "
            "way."
            % (text, len(dirty), ", ".join(sorted(dirty)[:12]), left.get("marker")))
    return kind, text


def settle_leftover(ws, left, dirty=()):
    """`leftover_disposition`, plus the one side effect its verdict implies.

    A refused marker is forgotten only once the gate has decided to PROCEED on
    it — i.e. only on a tree git reports clean, which cannot be carrying the
    mutant. When the verdict escalates to "still_mutated" the record is KEPT,
    so the refusal is sticky: the next gate reads the same marker and refuses
    the same way, until the operator reverts and deletes it. That also arms
    `write_applied_marker`'s O_EXCL refusal for free — no new mutant is
    applied on top of one that may still be in the tree.

    A marker in a directory that is not this user's own is never dropped,
    whatever the verdict: it is not ours to touch.
    """
    kind, text = leftover_disposition(left, dirty)
    if kind != "still_mutated" and left.get("refused") and not left.get("untrusted_dir"):
...
def _selftest():
    ...
    try:
        ...
        try:
            ...
            # `dirty_paths` promet une liste vide quand git NE REPOND PAS, et git
            # absent du PATH est la facon la plus totale de ne pas repondre. Sans
            # le garde c'est une trace de pile, sur le chemin qui s'execute AVANT
            # l'impression du rapport — la campagne lit alors un backtrace la ou
            # elle attend un verdict JSON.
            saved_path = os.environ.get("PATH", "")
            os.environ["PATH"] = os.path.join(tmp, "aucun-binaire-ici")
            try:
                check("git absent du PATH rend une liste vide, pas une trace",
                      dirty_paths(tmp), [])
            finally:
            ...
            check("le refus nomme le mutant encore applique", "leftover-1" in (why2 or ""), True)
            ...
            check("lire n'est pas decider : le marqueur survit a la lecture",
            ...
            # personne n'a ecrit.
            kind_d, text_d = settle_leftover(tmp, left, ["src/Api.java", "b.txt"])
            ...
            check("et nomme les chemins sales", ["src/Api.java" in text_d, "b.txt" in text_d],
                  [True, True])
            # ── Et ce refus TIENT. Efface des la lecture, il ne valait qu'une
            # passe : la porte refusait bien, mais le marqueur avait deja disparu,
            # donc la SUIVANTE ne trouvait plus rien a signaler et jugeait un arbre
            # portant peut-etre encore le mutant. Or le declencheur mesure de toute
            # cette branche est un REESSAI automatique sur le meme arbre : re-gater
            # sans revert humain est la sequence attendue, pas le cas limite.
            check("le marqueur survit au refus, et le texte dit de l'effacer",
                  [os.path.isfile(applied_marker_for(tmp)), (left or {}).get("marker") in text_d],
                  [True, True])
            # `again or {}` : sous la regression que ces trois checks attrapent, la
            # seconde lecture ne trouve plus de marqueur et rend None. Un `.get`
            # direct la ferait CRASHER — un echec de check dit ce qui a lache, une
            # trace d'AttributeError ne dit que par ou.
            again = revert_leftover_mutant(tmp)
            check("la porte suivante refuse encore, sur le meme marqueur",
                  [(again or {}).get("refused"),
                   settle_leftover(tmp, again or {}, ["src/Api.java"])[0],
                   os.path.exists(canary)],
                  [True, "still_mutated", False])
            # Sur un arbre que git dit PROPRE, rien ne peut plus porter le mutant :
            # le refus redevient une note, et c'est la — une fois — que le marqueur
            # perime est oublie.
            check("sur un arbre propre le refus est une note, et le marqueur est jete",
                  [settle_leftover(tmp, again or {}, [])[0], os.path.isfile(applied_marker_for(tmp))],
                  ["refused", False])
            ...
            settle_leftover(tmp, left, [])      # arbre propre : le marqueur perime est oublie
            ...
            try:
                ...
                check("et le dit", "marker could not be written" in (why3 or ""), True)
            ...
            # dessus, notre chmod echoue, et on lirait SON marqueur.
            scripts("printf 'mutant\\n' >> f.txt", "touch %s" % shlex.quote(canary))
            apply_mutant(lmeta, tmp)
            mdir_of_marker = os.path.dirname(applied_marker_for(tmp))
            os.chmod(mdir_of_marker, 0o755)
            try:
                left = revert_leftover_mutant(tmp)
                check("un repertoire de marqueurs ouvert aux autres n'est pas cru",
                      [(left or {}).get("refused"), (left or {}).get("untrusted_dir")],
                      [True, True])
                check("il n'est ni execute ni efface",
                      [os.path.exists(canary), os.path.isfile(applied_marker_for(tmp))],
                      [False, True])
                # Meme quand la porte passe (arbre propre), un marqueur qui n'est
                # pas dans NOTRE repertoire n'est jamais efface : il n'est pas a
                # nous. Le refus y est donc sticky sans condition d'arbre.
                check("disposition d'un repertoire de marqueurs non prive, marqueur intact",
                      [settle_leftover(tmp, left, [])[0], os.path.isfile(applied_marker_for(tmp))],
                      ["refused", True])
                # Et rien n'est applique tant qu'il est la. ATTENTION a ce que ce
                # check etablit vraiment : c'est l'O_EXCL sur le marqueur encore
                # present qui refuse ici, PAS la privacite du repertoire — le
                # chmod de `write_applied_marker` REPARE un repertoire qui est
                # deja le notre (mesure : 0o755 -> 0o700, ecriture acceptee).
                # Cote ecriture, seul un repertoire qui ne peut pas DEVENIR le
                # notre est refuse (autre uid, ou lien symbolique), et c'est le
                # check du lien, plus bas, qui garde ce controle-la.
                check("et rien n'est applique tant que ce marqueur est la",
                      apply_mutant(lmeta, tmp)[0], None)
            finally:
                os.chmod(mdir_of_marker, 0o700)
            drop_applied_marker(tmp)
            sub("git", "checkout", "--", "f.txt")
            # Un lien symbolique a la place du repertoire n'est jamais l'un des
            # notres : refuse a l'ecriture, donc rien n'est applique.
            elsewhere = os.path.join(outside, "redirected")
            os.makedirs(elsewhere)
            os.environ["GM_SCRATCH"] = linked = tempfile.mkdtemp(prefix="gm-linked-")
            try:
                os.symlink(elsewhere, os.path.join(linked, "gm-applied"))
                check("un repertoire de marqueurs qui est un lien est refuse",
                      [apply_mutant(lmeta, tmp)[0], tree()], [None, "original\n"])
            finally:
                os.environ["GM_SCRATCH"] = scratch_ok
...
def main():
    ...
    #
    # Read in EVERY mode, including record, even though the "not committed"
    # note below is a gate-only concern: it is what turns a refused marker into
    # a refusal, and recording is the one mode where a mutated tree does not
    # just produce a wrong verdict — it becomes the reference every later
    # verdict is measured against.
    paths = dirty_paths(ws)
    if left:
        kind, text = settle_leftover(ws, left, paths)
        if kind == "still_mutated":
            bail(text)
        note(report, text)

    if mode != "record" and paths:
        note(report, (
