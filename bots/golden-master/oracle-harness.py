...
def gm_dir_for(ws):
    """THE net's directory inside a workspace. Derived once, like the pile."""
    return os.path.join(ws, os.environ.get("GM_DIR", ".golden-master"))


def _within(path, root):
    """True when `path` is `root` or lives under it, symlinks resolved.

    realpath on BOTH sides: comparing the literal strings would take
    `<root>/../elsewhere` for contained, and a symlink under the root for
    contained whatever it points at.
    """
...
def scratch_root():
    """THE temp root the harness puts its OUT-OF-TREE state under.

    One rule, one place. The sealed held-out pile and the applied-mutant marker
    must land under the SAME root: an operator who sets GM_SCRATCH so the pile
    survives a recreated container got a marker that did not — it stayed in
    tempfile.gettempdir() — so the leftover guard silently never fired in
    exactly the setup chosen to make state survive.

    GM_SEALED_DIR is deliberately NOT folded in: it names the sealed directory
    itself, not a root, and hanging other state off it would put the marker
    inside the pile the seal moves around.
    """
    return os.environ.get("GM_SCRATCH", tempfile.gettempdir())
...
def dirty_paths(ws):
    """The paths git reports as not committed. None when git does not answer.

    THE one parse of `git status --porcelain`. It writes `XY <path>`, and X is
    ' ' for an unstaged modification, so a strip() on the WHOLE output eats the
    leading space of the first line only: that line shifts by one and loses its
    first character (`build.gradle` -> `uild.gradle`) while the rest stay
    intact — a list in which exactly one entry is wrong, and when a single file
    is dirty it is the only one named. Hence the slice per line, before any
    global strip.

    None (git did not answer) is NOT [] (git answered "clean"): a caller says
    different things about a workspace it cannot read and one it read as clean.
    """
    p = subprocess.run(["git", "-C", ws, "status", "--porcelain"],
...
def applied_marker_for(ws):
    ...
    absolute path (sibling worktrees share a basename), under the same root as
    the sealed held-out set — through `scratch_root`, not a second copy of its
    rule, which is the duplication `sealed_dir_for`'s own docstring warns about.
    """
    key = hashlib.sha256(os.path.abspath(ws).encode("utf-8")).hexdigest()[:12]
    return os.path.join(scratch_root(), "gm-applied-%s.json" % key)
...
def apply_mutant(meta, ws):
    ...
    #
    # Written through os.open, not open(): the name is fully derivable from the
    # workspace path, and on a shared host the temp root is world-writable, so
    # a plain open() would follow a symlink someone else planted there (writing
    # wherever it points) and would leave the marker 0644 for anyone to rewrite.
    # O_NOFOLLOW refuses the symlink; 0600 keeps the file ours. O_TRUNC rather
    # than O_EXCL on purpose: a marker legitimately survives a revert that
    ...
    try:
        ...
        # held-out set — never reaches before the first apply.
        ...
        # os.open may well have SUCCEEDED with the write failing after it (a
        # full tmpfs /tmp is a documented incident on this repo), leaving a
        # 0-byte file behind. An empty marker names no mutant, so no later gate
        # can resolve it — and none of them removes a marker whose revert did
        # not exit 0, so every gate from here on refuses, permanently, over a
        # record of nothing. A marker we failed to write is not a record of an
        # applied mutant: it goes, and the warning below is the whole truth.
        forget_applied_marker(ws)
        ...
        # one who set GM_SCRATCH precisely so the state would survive. A guard
        # that is absent must SAY it is absent; the apply still proceeds,
        # because refusing here would break gates on hosts whose temp root has
        # always been unwritable and where the harness worked fine.
        log("WARNING: could not write the applied-mutant marker %s (%s). If this run "
            "is interrupted between apply.sh and revert.sh, the next gate will NOT "
            "know a mutant is applied and will judge a mutated tree." % (marker, e))
    return run_script(os.path.join(meta["dir"], "apply.sh"), ws)


def forget_applied_marker(ws):
    """Drop the applied-mutant marker. THE one place that disarms the guard.

    Three callers need it and each had its own copy of the try/except; the rule
    for WHEN it is legitimate (the mutant is demonstrably no longer applied) is
    subtle enough that three copies would eventually disagree — the duplication
    `sealed_dir_for`'s docstring is about.
    """
    try:
...
def revert_leftover_mutant(ws):
    ...
    file as "not committed". Returns None ONLY when there is no marker at all;
    otherwise a dict carrying what was found and `code` — 0 when the tree is
    demonstrably back at HEAD, and anything else (a non-zero exit, a vanished
    revert.sh, a marker refused as untrustworthy) meaning it is NOT. Callers
    decide through `leftover_verdict`; branching on truthiness alone reads all
    four as success.
    """
    marker = applied_marker_for(ws)
    # lstat, not isfile: isfile FOLLOWS a symlink, so a link planted at the
    # derivable name would have this read someone else's file as the record of
    # what to run. And the marker must be OURS — see the ownership check below.
    try:
        st = os.lstat(marker)
    except OSError:
        return None
    ...
    if not stat.S_ISREG(st.st_mode):
        refused = "the marker is not a regular file (a symlink or worse was planted at its name)"
    elif hasattr(os, "geteuid") and st.st_uid != os.geteuid():
        refused = ("the marker is owned by uid %s, not by the user running the gate (%s)"
    ...
    if refused is None:
        try:
            ...
            meta = {}
        # WHERE the script may live. This runs `<dir>/revert.sh` through sh, as
        # the gate's user, before any other harness logic — and `dir` comes from
        # a file whose name is fully derivable from the workspace path, in a
        # temp root that is world-writable on a shared host. Unchecked, planting
        # a marker naming a directory you control is arbitrary execution as the
        # gate. A real mutant only ever lives in the net or in the sealed pile
        # (see load_mutants), so anything else is refused rather than run.
        d = meta.get("dir")
        if d and not any(_within(d, root) for root in (gm_dir_for(ws), sealed_dir_for(ws))):
            refused = ("its revert.sh lives outside the net and the sealed pile (%s) — "
                       "refused rather than executed" % d)
    if refused is not None:
        # Deliberately NOT removed: the tree may well be mutated, and a marker
        # this process would not obey is exactly the one a human must look at.
        return {"id": meta.get("id"), "dir": meta.get("dir"), "code": None,
                "out": refused, "marker": marker}
    script = os.path.join(meta.get("dir") or "", "revert.sh")
    restored = []
    if not meta.get("dir"):
        # Empty or corrupt: it names no mutant, so this cannot revert one — and
        # cannot claim a mutant is applied either. Kept distinct from the
        # vanished-script case below, which DOES know what was applied; folding
        # the two made the report assert a mutated tree it had never looked at.
        code, out = None, ("the marker is empty or corrupt — it names no mutant "
                           "directory, so nothing here can say what was applied")
    elif os.path.isfile(script):
        # WHAT this revert is about to take back to HEAD. revert.sh is
        # canonically `git checkout -- <path>`, so anything the operator had
        # uncommitted in those paths dies with the mutant — and the sibling
        # WORKSPACE NOT COMMITTED notice cannot name them, because by the time
        # it looks they are clean again and gone from `git status`. Measured:
        # ' M f.txt' before the sweep, '' after, and a report naming only the
        # mutant id. That is verbatim the incident this guard exists for, whose
        # own account ends "naming only the mutant id and never the path".
        # Reverting is still right; being silent about what it cost is not.
        was = dirty_paths(ws)
        code, out = run_script(script, ws)
        now = dirty_paths(ws)
        if was is not None and now is not None:
            restored = sorted(set(was) - set(now))
    else:
    ...
    return {"id": meta.get("id"), "dir": meta.get("dir"), "code": code,
            "out": (out or "")[-300:], "marker": marker, "restored": restored}
...
def leftover_verdict(left):
    ...
    if not left:
        return False, None
    if left.get("code") == 0:
        # NAMING what the revert took back. Those paths were not committed a
        # moment ago and are at HEAD now; the mutant's own file is among them,
        # but so is anything the operator had in flight there — and nothing
        # downstream can still see it, since `git status` is clean on them by
        # the time the WORKSPACE NOT COMMITTED notice looks. A bare count, or
        # the mutant id alone, is what made the measured incident silent.
        gone = left.get("restored") or []
        return False, (
            ...
            "attempt's verdict, if any, judged a mutated program.%s"
            % (left.get("id"),
               "" if not gone else
               " That revert took %d path(s) back to HEAD: %s — they were NOT "
               "committed, so any work you had in flight there is gone."
               % (len(gone), ", ".join(gone[:12]))))
    # A marker that names no mutant is a THIRD outcome, and it must not borrow
    # the sentence below. That one asserts "THE TREE IS STILL MUTATED" — true
    # when a mutant is known to have been applied and not put back, but here
    # nothing was ever established: no id, no directory, no revert attempted.
    # Stated anyway it accused a possibly pristine tree, named no mutant, and
    # sent the operator after "that mutant's revert.sh" it could not identify.
    # Still a refusal — an unreadable marker is exactly when the harness does
    # not know — but the refusal now says what is actually the case.
    if not left.get("dir"):
        return True, (
            "the applied-mutant marker %s cannot be read: %s. So this run cannot "
            "tell whether a mutant is applied, nor which one, nor put it back — "
            "and a gate that cannot rule that out would be judging a program "
            "nobody can identify. Check the workspace against HEAD, then delete "
            "that marker and re-run." % (left.get("marker"), left.get("out")))
...
def probe_mutation(meta, ws):
    ...
    if code != 0:
        ...
        #
        # And that same rule binds HERE, which is why the revert is CONDITIONAL.
        # The canonical invalid mutant is one whose anchor a modernisation lot
        # removed: its apply.sh refuses and exits non-zero WITHOUT touching
        # anything. Reverting then runs `git checkout -- <path>` over a tree
        # this mutant never touched — and MODE=validate exists to be re-run
        # while re-anchoring, i.e. precisely while the repair is uncommitted.
        # Measured against the merge-base: half a day of uncommitted
        # re-anchoring, destroyed, under a reason naming only "apply.sh exited
        # 1". The predicate is the one the success path uses three lines down,
        # not a second opinion about what counts as having moved.
        touched = (tree_fingerprint(ws) != before_tree) or (
            before_data is not None and data_fingerprint(meta, ws) != before_data)
        ...
        if touched:
            ...
            if rcode != 0:
                ...
                # Same term score_mutant sets when a revert fails under it (and
                # for the same reason): an invalid mutant is dropped from the
                # score, so without this the run would go on scoring the REST of
                # the lot against a tree this one left half-mutated, and could
                # still come out green. revert_clean is the gate's own
                # conjunction term.
                extra["revert_clean"] = False
        else:
            # Nothing to put back — but the marker went down BEFORE apply.sh, so
            # it is armed and must still be disarmed here. Left standing, the
            # next gate would run this mutant's revert.sh over whatever is on
            # disk then: the very clobber this branch exists to stop.
            forget_applied_marker(ws)
        return dict(base, valid=False, detected=False, reason=reason, **extra), "failed"
...
def _selftest():
    ...
    try:
        ...
        # Le marqueur de mutant applique suit la MEME racine que la pile
        # scellee. Un operateur qui pose GM_SCRATCH pour que la pile survive a
        # un conteneur recree avait un marqueur qui, lui, ne survivait pas : la
        # garde du mutant laisse applique ne se declenchait jamais, precisement
        # dans le montage choisi pour faire durer l'etat.
        os.environ.pop("GM_SEALED_DIR", None)
        os.environ["GM_SCRATCH"] = "/persistent-root"
        check("GM_SCRATCH deplace la pile ET le marqueur, sous la meme racine",
              [sealed_dir_for("/w/app").startswith("/persistent-root" + os.sep),
               applied_marker_for("/w/app").startswith("/persistent-root" + os.sep)],
              [True, True])
        os.environ.pop("GM_SCRATCH", None)
        check("sans GM_SCRATCH, le marqueur retombe sur le temp du systeme",
              applied_marker_for("/w/app").startswith(tempfile.gettempdir() + os.sep), True)
        ...
        try:
            ...
            # Sous <ws>/.golden-master/mutants/ : la seule place ou vit un vrai
            # mutant (cf. load_mutants), et c'est ce confinement que la garde
            # verifie avant d'executer le moindre revert.sh.
            mroot = os.path.join(gm_dir_for(tmp), "mutants")
            mdir = os.path.join(mroot, "m1")
            os.makedirs(mdir)
            ...
            # Interruption ici : pas de revert. La porte suivante nettoie — et
            # l'operateur, entre-temps, a ecrit SON travail dans ce fichier.
            # Le revert le reprend a HEAD ; il faut donc que le chemin soit
            # NOMME. Sinon la perte est muette : `git status` est propre sur ce
            # chemin quand la notice « WORKSPACE NOT COMMITTED » regarde, donc
            # plus rien en aval ne peut le citer. Mesure : ' M f.txt' avant la
            # passe, '' apres, et un rapport ne nommant que l'id du mutant.
            with open(os.path.join(tmp, "f.txt"), "w", encoding="utf-8") as f:
                f.write("original\nMON TRAVAIL NON COMMITTE\n")
            left = revert_leftover_mutant(tmp)
            check("le mutant laisse applique est identifie", (left or {}).get("id"), "leftover-1")
            sweep_refuse, sweep_msg = leftover_verdict(left)
            check("ce que le revert a repris a HEAD est NOMME, pas seulement compte",
                  [sweep_refuse, "f.txt" in sweep_msg, (left or {}).get("restored")],
                  [False, True, ["f.txt"]])
            ...
            bdir = os.path.join(mroot, "m2")
            os.makedirs(bdir)
            with open(os.path.join(bdir, "apply.sh"), "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\nprintf 'mutant\\n' >> f.txt\nexit 1\n")
            with open(os.path.join(bdir, "revert.sh"), "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\ngit checkout -- f.txt\n")
            # Les VRAIS apply/revert ET les VRAIES empreintes (seules les
            # doublures du bloc 6 restent posees). Les empreintes ont TOUT a
            # dire sur un apply qui a echoue : ce sont elles qui etablissent
            # s'il a touche quelque chose, donc si un revert est legitime.
            ...
            g.update(apply_mutant=saved["apply_mutant"], revert_mutant=saved["revert_mutant"],
                     tree_fingerprint=saved["tree_fingerprint"],
                     data_fingerprint=saved["data_fingerprint"])
            try:
            ...
            check("un apply en echec revertible ne salit pas revert_clean",
                  v.get("revert_clean", True), True)

            # Un apply.sh qui echoue SANS RIEN TOUCHER ne doit rien revertir.
            # C'est le mutant invalide canonique : un lot de modernisation a
            # supprime son point d'appui, apply.sh refuse et sort non-zero sans
            # ecrire une ligne. Revertir la, c'est passer `git checkout --` sur
            # un arbre que ce mutant n'a pas touche — et MODE=validate existe
            # pour etre relance PENDANT le re-ancrage, donc sur un arbre sale.
            # Mesure contre la merge-base : une demi-journee de re-ancrage non
            # committe, detruite, sous une raison qui ne disait que « apply.sh
            # exited 1 ». Le marqueur, lui, est pose AVANT apply.sh : il doit
            # partir quand meme, sans quoi la porte suivante lancerait ce
            # revert.sh sur ce qui traine alors sur le disque.
            ndir = os.path.join(mroot, "m3")
            os.makedirs(ndir)
            with open(os.path.join(ndir, "apply.sh"), "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\nexit 1\n")
            with open(os.path.join(ndir, "revert.sh"), "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\ngit checkout -- f.txt\n")
            with open(os.path.join(tmp, "f.txt"), "w", encoding="utf-8") as f:
                f.write("original\nune demi-journee de travail non committe\n")
            stubbed = {k: g[k] for k in ("apply_mutant", "revert_mutant",
                                         "tree_fingerprint", "data_fingerprint")}
            g.update(apply_mutant=saved["apply_mutant"], revert_mutant=saved["revert_mutant"],
                     tree_fingerprint=saved["tree_fingerprint"],
                     data_fingerprint=saved["data_fingerprint"])
            try:
                v3, state3 = probe_mutation({"id": "m3", "dir": ndir, "targets": ["001"]}, tmp)
            finally:
                g.update(stubbed)
            with open(os.path.join(tmp, "f.txt"), encoding="utf-8") as f:
                garde = f.read()
            check("un apply en echec qui n'a RIEN touche ne revertit pas le travail en cours",
                  [state3, garde, os.path.isfile(applied_marker_for(tmp)),
                   "apply.sh exited 1" in (v3.get("reason") or "")],
                  ["failed", "original\nune demi-journee de travail non committe\n",
                   False, True])
            subprocess.run(["git", "checkout", "--", "f.txt"], cwd=tmp,
                           capture_output=True, text=True)

            # apply.sh ECHOUE et revert.sh AUSSI : l'arbre reste peut-etre
            # a moitie mute. Un mutant invalide sort du score, donc sans
            # revert_clean la porte noterait TOUT LE RESTE du lot sur cet
            # arbre-la et pourrait finir verte.
            with open(os.path.join(bdir, "revert.sh"), "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\nexit 7\n")
            stubbed = {k: g[k] for k in ("apply_mutant", "revert_mutant",
                                         "tree_fingerprint", "data_fingerprint")}
            g.update(apply_mutant=saved["apply_mutant"], revert_mutant=saved["revert_mutant"],
                     ...
                     data_fingerprint=saved["data_fingerprint"])
            try:
                v2, state2 = probe_mutation({"id": "m2", "dir": bdir, "targets": ["001"]}, tmp)
            finally:
                g.update(stubbed)
            check("apply ET revert en echec -> revert_clean faux, les deux causes dites",
                  [state2, v2.get("revert_clean"), "apply.sh exited 1" in (v2.get("reason") or ""),
                   "revert.sh then exited 7" in (v2.get("reason") or "")],
            ...
            shutil.rmtree(mdir)
            left = revert_leftover_mutant(tmp)
            # Porte sur l'IDENTITE rapportee et sur le message, jamais sur le
            # seul `code`. `(left or {}).get("code")` vaut None dans les DEUX
            # cas : le leftover correctement signale (dict, code None) ET la
            # fonction qui le tait en renvoyant None — l'oubli meme que ce test
            # existe pour interdire. Mesure : un `return None` a la place de ce
            # bras laissait passer les 126 verifications.
            ...
            check("et la porte REFUSE de juger un arbre qu'elle ne peut pas rendre a HEAD",
                  leftover_verdict(left)[0], True)

            # ── Le marqueur DESIGNE un script que la porte lance en sh, sous son
            # propre compte, avant toute autre logique — et son nom se derive
            # entierement du chemin du workspace, dans un temp root partage. Un
            # marqueur qui pointe hors du filet est REFUSE, pas execute.
            outside = os.path.join(tmp, "planted")
            os.makedirs(outside)
            witness = os.path.join(tmp, "PWNED")
            with open(os.path.join(outside, "revert.sh"), "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\ntouch %s\n" % shlex.quote(witness))
            with open(applied_marker_for(tmp), "w", encoding="utf-8") as f:
                json.dump({"id": "planted", "dir": outside}, f)
            left = revert_leftover_mutant(tmp)
            check("un marqueur pointant hors du filet : refuse, JAMAIS execute",
                  [os.path.exists(witness), (left or {}).get("code"),
                   "outside the net" in ((left or {}).get("out") or ""),
                   os.path.isfile(applied_marker_for(tmp))],
                  [False, None, True, True])
            check("et la porte refuse de juger sur un marqueur qu'elle n'obeit pas",
                  leftover_verdict(left)[0], True)
            # Tolerant : si la garde tombe, le marqueur a ete consomme et le
            # nettoyage ne doit pas masquer l'echec par une exception.
            try:
                os.remove(applied_marker_for(tmp))
            except OSError:
                pass

            # Un mutant du filet, lui, passe : la garde confine, elle ne casse
            # pas le cas nominal.
            ...
            saved["apply_mutant"](lmeta, tmp)
            check("le marqueur est ecrit en 0600",
                  stat.S_IMODE(os.lstat(applied_marker_for(tmp)).st_mode), 0o600)
            saved["revert_mutant"](lmeta, tmp)

            # Une racine GM_SCRATCH pas encore creee ne doit pas desarmer la
            # garde en silence : seul le SCELLEMENT cree ce repertoire, et un
            # run en record — ou sans jeu held-out — applique avant d'y passer.
            # Mesure : le marqueur n'etait pas ecrit, apply.sh tournait quand
            # meme, et la garde etait absente sans que personne le sache.
            os.environ["GM_SCRATCH"] = os.path.join(tmp, "pas-encore", "cree")
            try:
                saved["apply_mutant"](lmeta, tmp)
                check("racine GM_SCRATCH absente : elle est creee, la garde reste armee",
                      [os.path.isfile(applied_marker_for(tmp)),
                       (revert_leftover_mutant(tmp) or {}).get("id")],
                      [True, "leftover-1"])
            finally:
                os.environ.pop("GM_SCRATCH", None)

            # Une ECRITURE de marqueur qui echoue APRES l'os.open laisse un
            # fichier de 0 octet (un /tmp tmpfs plein, incident documente ici).
            # Ce marqueur-la ne nomme aucun mutant : aucune porte ne peut le
            # resoudre, et aucune n'efface un marqueur dont le revert n'est pas
            # sorti 0 — donc toutes refusent, pour toujours, sur un registre de
            # rien. Un marqueur qu'on n'a pas su ecrire n'est pas un registre.
            def dump_qui_echoue(*_a, **_k):
                raise OSError(28, "No space left on device")

            # `log` est capture, pas etouffe : l'avertissement fait partie du
            # contrat (une garde absente doit le DIRE), donc il est verifie ici
            # plutot que deverse sur stderr a chaque passage du harnais.
            vrai_dump, vrai_log, dits = json.dump, g["log"], []
            json.dump, g["log"] = dump_qui_echoue, dits.append
            try:
                saved["apply_mutant"](lmeta, tmp)
            finally:
                json.dump, g["log"] = vrai_dump, vrai_log
            check("marqueur non ecrit : rien de pose, et l'absence de garde est DITE",
                  [os.path.isfile(applied_marker_for(tmp)),
                   any("could not write the applied-mutant marker" in d for d in dits)],
                  [False, True])
            subprocess.run(["git", "checkout", "--", "f.txt"], cwd=tmp,
                           capture_output=True, text=True)

            # Et s'il en traine un quand meme (ecrit par une version d'avant),
            # la porte refuse SANS affirmer un arbre mute qu'elle n'a jamais
            # regarde : elle ne connait ni mutant, ni repertoire, et n'a tente
            # aucun revert. La phrase « THE TREE IS STILL MUTATED » accusait un
            # arbre peut-etre intact et renvoyait l'operateur vers « that
            # mutant's revert.sh » qu'elle etait incapable de nommer.
            with open(applied_marker_for(tmp), "w", encoding="utf-8") as f:
                f.write("")
            vide = revert_leftover_mutant(tmp)
            vide_refuse, vide_msg = leftover_verdict(vide)
            check("marqueur illisible : refus, mais sans affirmer l'arbre mute",
                  [vide_refuse, "STILL MUTATED" in vide_msg,
                   "cannot be read" in vide_msg,
                   applied_marker_for(tmp) in vide_msg],
...
def main():
    ...
    if mode != "record":
        # Le decoupage per-ligne (et pourquoi il ne peut pas etre un strip
        # global) vit dans `dirty_paths` — la garde du mutant laisse applique
        # lit la MEME sortie pour nommer ce que son revert a repris, et deux
        # analyseurs de `git status --porcelain` finiraient par diverger.
        paths = dirty_paths(ws)
        if paths:
            note(report, (
