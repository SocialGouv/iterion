...
def holdout_drawn_in_tree(gm_dir):
    """True when mutants/holdout/ carries entries this run WROTE, not inherited.

    The committed check above is directory-wide, so ONE tracked entry — a
    successor an earlier cycle left for a gate that never opted in — makes the
    seal decline for everything beside it. A set drawn in that state never
    leaves the tree: it stays readable by the hardening loop it is held out
    from, the sealed pile stays empty, and `holdout_detected == holdout_total`
    passes 0 == 0.

    Untracked-ness is what separates that from the two states which look
    identical in the report and are NOT defects: a lot's CI on a clean
    checkout (everything under mutants/holdout/ tracked, nothing drawn here)
    and a rite that sealed its own set before committing its successor (same).
    Both stay silent; only a run that DREW a set it cannot seal is refused.
    """
    code, out = run("git ls-files --others --exclude-standard -- mutants/holdout",
                    gm_dir, timeout=60)
    return code == 0 and bool(out.strip())
...
def _selftest():
    ...
    try:
        ...
        check("jeu committe -> laisse en place, rien de scelle",
              ...
              [False, True, False])
        # Le predicat qui separe l'HERITAGE du DESSIN. Le declin ci-dessus est
        # repertoire-large : une seule entree committee le ferme pour tout ce
        # qui est a cote, donc un jeu dessine la reste dans l'arbre, lisible
        # par la boucle de durcissement dont il est cense etre soustrait, et
        # le 0/0 qui en sort passe la conjonction du gate. Non-suivi est ce
        # qui distingue ce cas des deux qui lui ressemblent dans le rapport
        # sans etre des defauts : la CI d'un lot sur un checkout propre, et un
        # rite qui a scelle son jeu AVANT de committer son successeur.
        check("jeu committe seul -> rien n'a ete dessine ici",
              holdout_drawn_in_tree(gmd2), False)
        drawn = os.path.join(gmd2, "mutants", "holdout", "t02")
        os.makedirs(drawn)
        with open(os.path.join(drawn, "apply.sh"), "w", encoding="utf-8") as f:
            f.write("true\n")
        check("jeu dessine a cote d'un jeu committe -> vu, et le sceau le laisse en place",
              [holdout_drawn_in_tree(gmd2), seal_holdout(gmd2, sealed2),
               os.path.isdir(drawn)],
              [True, False, True])
        shutil.rmtree(drawn)
...
def main():
    ...
    if mode != "record":
        try:
            ...
            # Measured BEFORE the seal, which is the only moment it is still
            # answerable: a seal that proceeds moves those very entries out.
            drawn_before = holdout_drawn_in_tree(gm_dir)
        ...
        if awaiting:
            ...
            note(report, "a fresh held-out set is COMMITTED under mutants/holdout/ "
                         ...
                         "prepared.")
        # And the case where that debt stops being someone else's problem: a
        # run that DREW a set into a directory the decline holds shut. The
        # inherited entry is not the defect — leaving it in place is the whole
        # point — but the set drawn beside it never leaves the tree, so it is
        # readable by the very hardening loop it is held out from, and the
        # pile it never reached is what `holdout_detected == holdout_total`
        # then compares: 0 == 0, green, on a term nothing measured. A proof
        # obligation nothing refuses is a waiver with extra steps, so this one
        # is a FIELD the two gates read, not a notice. Only the net's owner can
        # clear it, by opting the gate that owns the orphaned set in.
        report["holdout_seal_blocked"] = bool(awaiting and drawn_before)
    ...
    try:
        ...
        if report["holdout_seal_blocked"]:
            problems.append("the held-out set drawn by this run CANNOT BE SEALED: "
                            "mutants/holdout/ already carries a committed set — a "
                            "successor an earlier cycle left for a gate that has not "
                            "opted in — and the seal declines DIRECTORY-WIDE, so what "
                            "was drawn here stays in the tree, readable by the "
                            "hardening loop it is held out from. The held-out figure "
                            "below is 0/0 and measures nothing. Nothing this run may "
                            "do clears it: writing `\"seal_committed\": true` would "
                            "spend the inherited set at the wrong gate, and deleting "
                            "it destroys an artefact this run did not draw. The net's "
                            "owner arms that opt-in on the gate that OWNS the "
                            "inherited set, and this run reports the debt.")
