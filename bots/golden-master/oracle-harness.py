...
def revert_leftover_mutant(ws):
    ...
    not one of this workspace's `mutant_roots` (dropped, it sits in our own
    private directory), or it sits in a directory this user does not privately
    own (left alone, it is not ours to touch).
    """
    marker = applied_marker_for(ws)
        ...
        try:
            ...
            try:
                ...
                check("il n'est ni execute ni efface",
                      ...
                      [False, True])
                check("disposition d'un repertoire de marqueurs non prive",
                      leftover_disposition(left)[0], "refused")
