...
class Session:
    ...
    def fetch(self, method, path, fields=None, timeout=60, follow=True):
        ...
        if fields:
            # An object value is how a corpus declares a FILE, and the only
            # meaning it has here. The route is chosen on the OBJECT, not on a
            # well-formed one: a near-miss declaration — `filename` mistyped,
            # or not a string — must reach `encode_multipart`'s named refusal
            # rather than `urlencode`, which flattens it to the repr of a
            # Python object. That flattening is the exact defect the multipart
            # path exists to close, and routing on `is_file_part` would leave
            # it open for every declaration an author got slightly wrong.
...
def is_file_part(value):
    ...
    an optional `content_type`. A STRING is an ordinary form field, so a corpus
    that declares no upload is encoded exactly as before; an object that fails
    this predicate is a file declaration its author got wrong, and
    `encode_multipart` refuses it by name rather than flatten it.
    """
    return isinstance(value, dict) and isinstance(value.get("filename"), str)


def file_part_bytes(value):
    """The part's payload, from EXACTLY ONE of `text` or `b64`. `text` is
    UTF-8 (a corpus stays readable and replayable by hand); `b64` carries what
    text cannot. Every way of getting the pair wrong is refused BY NAME —
    a corpus value that reaches the wire unexamined is a reference describing
    a request its author did not write."""
    declared = [k for k in ("text", "b64") if k in value]
    if not declared:
        raise SystemExit("file field %r declares neither `text` nor `b64` — "
                         ...
                         % (value.get("filename"),))
    if len(declared) == 2:
        raise SystemExit("file field %r declares BOTH `text` and `b64` — they "
                         "are alternatives, and silently preferring one puts "
                         "on the wire bytes the corpus author did not believe "
                         "they had declared"
                         % (value.get("filename"),))
    if "b64" in value:
        b64 = value["b64"]
        # A number, a list or an object here reached `b64decode` and left as a
        # bare TypeError: the judge died on a traceback instead of answering
        # with its verdict. Same refusal as a non-str `text`, one line up.
...
# What a part HEADER may carry. The three corpus-supplied values that reach
# one are emitted in DIFFERENT syntactic contexts, so they do not share a
# rule: `name` and `filename` sit inside a quoted-string, where a `"` ends the
# parameter early and the rest of the value becomes syntax; `content_type` is
# the WHOLE header value, where a quote is legitimate media-type syntax
# (`charset="UTF-8"`) — refusing it there would reject a valid declaration and
# prevent nothing. CR and LF end a header in all three, and a `--<boundary>`
# line behind them ends the PART: a filename carrying one made the wire read
# `doc=PWNED` and a fabricated `role=admin` while the corpus declared
# `doc=X, role=user` (measured). The reference would then describe a request
# nobody wrote — which is the one thing an observation point may never do.
#
# That refusal is also what makes `multipart_boundary`'s payload-only scan
# sound: a delimiter only acts at the start of a line, and no header value can
# open one.
QUOTED_PARAM = ('"', "\r", "\n")
HEADER_VALUE = ("\r", "\n")
_HEADER_POISON = {'"': "a quote", "\r": "a carriage return", "\n": "a newline"}


def checked_part_header(label, text, forbidden):
    """The header value the corpus declared, or a named refusal — never a
    request other than the one the corpus wrote."""
    # The last corpus value that reached a header unexamined: `content_type`
    # was coerced with `str`, so a list or an object arrived as its repr and
    # the wire read `Content-Type: ['a']`. The reference would then record the
    # application's reaction to the text of a Python object as its behaviour —
    # the same defect as a payload sent as its own error text, one header up.
    # (`name` and `filename` are strings by construction: JSON keys are, and
    # `is_file_part` requires one. The rule is stated once for all three.)
    if not isinstance(text, str):
        raise SystemExit("%s must be a string, got %s (%r) — coerced it would "
                         "put the text of a Python object into a part header"
                         % (label, type(text).__name__, text))
    for c in forbidden:
        if c in text:
            raise SystemExit("%s %r carries %s — emitted raw into a part "
                             "header it would send a request the corpus never "
                             "declared (an injected header, or a forged part). "
                             "Refused by name, like an undecodable payload."
                             % (label, text, _HEADER_POISON[c]))
    return text
...
def multipart_boundary(payloads):
    ...
    Payloads ONLY, and that is sound rather than partial: a delimiter is
    recognised at the start of a line, and `checked_part_header` refuses CR
    and LF in every value that reaches a header, so no header can open the
    line a `--<boundary>` would need. Drop that refusal and this scan becomes
    a hole.
    """
...
def encode_multipart(fields):
    ...
    for name, value in fields.items():
        if is_file_part(value):
            ...
            # The near miss, and the last way the original defect survives:
            # an object whose `filename` is mistyped or is not a string is
            # still a file the author meant to declare, but no longer one
            # `is_file_part` recognises. Flattened by `str`, it reaches the
            # wire as `{'filenam': 'a.pdf', ...}` — the application refuses
            # the TEXT of a Python object, and the reference records that
            # refusal as the behaviour. Refused by name, like a payload that
            # does not decode.
            raise SystemExit(
                "field %r is an object but not a file declaration: a file "
                "names itself — {\"filename\": <string>, \"text\"|\"b64\": "
                "...}. Emitted as an ordinary field it would put the text of "
                "a Python object on the wire, and the reference would record "
                "the refusal of that text as the application's behaviour."
                % (name,))
        else:
    ...
    for (name, value), payload in zip(fields.items(), payloads):
        head = '--%s\r\nContent-Disposition: form-data; name="%s"' % (
            boundary, checked_part_header("field name", name, QUOTED_PARAM))
        if is_file_part(value):
            head += '; filename="%s"' % checked_part_header(
                "filename", value["filename"], QUOTED_PARAM)
            head += "\r\nContent-Type: %s" % checked_part_header(
                "content_type", value.get("content_type")
                or "application/octet-stream", HEADER_VALUE)
...
def _selftest():
    ...
    try:
        ...
        try:
            ...
            # Et la declaration MANQUEE, seule dans son entree : routee sur
            # `is_file_part`, elle repartait par urlencode et posait le repr
            # de l'objet sur le fil, en silence. C'est le defaut d'origine,
            # survivant a la faute de frappe la plus probable.
            received.clear()
            try:
                sess.fetch("POST", "/upload",
                           fields={"doc": {"filenam": "a.pdf", "text": "X"}})
                check("declaration de fichier manquee et SEULE -> refus nomme",
                      ["aucun-refus", received.get("body")], ["SystemExit", None])
            except SystemExit:
                check("declaration de fichier manquee et SEULE -> refus nomme",
                      ["SystemExit", received.get("body")], ["SystemExit", None])
        finally:
            srv.shutdown()
            srv.server_close()
        ...
        # `b64` non-textuel : partait en TypeError nu depuis base64, donc en
        # trace d'appel au lieu du verdict. Le pendant non-textuel de `text`
        # etait deja refuse par son nom.
        for bad in (7, [1, 2], {"k": "v"}, True):
            refuses("b64 non textuel (%s) -> refus nomme" % type(bad).__name__,
                    lambda bad=bad: encode_multipart({"d": {"filename": "a",
                                                            "b64": bad}}))
        # Declarer les DEUX : `b64` gagnait en silence, et le corpus decrivait
        # alors des octets que son auteur n'avait pas declares.
        refuses("text ET b64 declares -> refus nomme, aucun gagnant silencieux",
                lambda: encode_multipart({"d": {"filename": "a", "text": "FROM-TEXT",
                                                "b64": "RlJPTS1CNjQ="}}))
        # La DECLARATION manquee, a cote d'un fichier valide : `filename`
        # mal orthographie ou non textuel. `is_file_part` ne la reconnait
        # plus, et `str` la posait sur le fil telle quelle.
        for near in ({"filename": 42, "text": "x"},
                     {"filenam": "a.pdf", "text": "x"},
                     {"filename": None, "b64": "aGk="},
                     {"text": "x"}):
            refuses("objet qui n'est pas une declaration de fichier (%s) -> refus nomme"
                    % sorted(near),
                    lambda near=near: encode_multipart(
                        {"ok": {"filename": "v.txt", "text": "K"}, "doc": near}))

        # Les EN-TETES de partie : le second moyen d'envoyer une requete que
        # le corpus n'a pas declaree. Le vecteur decisif d'abord — un nom de
        # fichier portant un CRLF et une ligne de frontiere fabriquait une
        # partie entiere, invisible pour `multipart_boundary` qui ne lit que
        # les charges utiles.
        forge = ('a.pdf"\r\n\r\nPWNED\r\n--%s\r\nContent-Disposition: '
                 'form-data; name="role"\r\n\r\nadmin\r\n--%s--\r\n'
                 % (FILE_PART_BOUNDARY, FILE_PART_BOUNDARY))
        refuses("nom de fichier fabriquant une partie entiere -> refus nomme",
                lambda: encode_multipart({"doc": {"filename": forge, "text": "X"},
                                          "role": "user"}))
        refuses("guillemet dans le nom de fichier -> refus nomme",
                lambda: encode_multipart({"d": {"filename": 'a"b.pdf', "text": "X"}}))
        refuses("CRLF dans le nom de fichier -> refus nomme",
                lambda: encode_multipart({"d": {"filename": "a\r\nX-Evil: 1",
                                                "text": "X"}}))
        refuses("guillemet dans le nom de champ -> refus nomme",
                lambda: encode_multipart({'n"x': "v",
                                          "d": {"filename": "a", "text": "X"}}))
        refuses("CRLF dans le nom de champ -> refus nomme",
                lambda: encode_multipart({"n\r\nX-Evil: 1": "v",
                                          "d": {"filename": "a", "text": "X"}}))
        refuses("CRLF dans content_type -> refus nomme",
                lambda: encode_multipart({"d": {"filename": "a", "text": "X",
                                                "content_type": "text/plain\r\nX-Evil: 1"}}))
        # `content_type` non textuel : partait en repr dans l'en-tete
        # (`Content-Type: ['a']`), derniere valeur du corpus atteignant un
        # en-tete sans etre examinee.
        for bad_ct in (42, ["a"], {"a": "b"}):
            refuses("content_type non textuel (%s) -> refus nomme" % type(bad_ct).__name__,
                    lambda bad_ct=bad_ct: encode_multipart(
                        {"d": {"filename": "a", "text": "X", "content_type": bad_ct}}))
        # Et le defaut, qui ne doit pas tomber sous ce refus : absent -> le
        # type par defaut, pas une erreur.
        check("content_type absent -> application/octet-stream",
              parse_multipart(*encode_multipart(
                  {"d": {"filename": "a.bin", "text": "X"}}))["d"][1],
              "application/octet-stream")
        # Et la borne de l'autre cote : un guillemet est de la SYNTAXE valide
        # dans un type de media, la ou il est une injection dans un
        # quoted-string. Le refuser rejetterait une declaration juste.
        qt, qct = encode_multipart({"d": {"filename": "a.txt", "text": "X",
                                          "content_type": 'text/plain; charset="UTF-8"'}})
        qmsg = email.parser.BytesParser().parsebytes(
            b"Content-Type: " + qct.encode() + b"\r\nMIME-Version: 1.0\r\n\r\n" + qt)
        check("parametre de media entre guillemets : accepte et relu",
              [[p.get_content_type() for p in qmsg.get_payload()],
               [p.get_param("charset") for p in qmsg.get_payload()]],
              [["text/plain"], ["UTF-8"]])
