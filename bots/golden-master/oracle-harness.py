...
def header_value(s):
    """A name, filename or content type as it may appear in a part header.
...
def _selftest():
    ...
    try:
        ...
        hostile = {"doc": {"filename": 'a";name="evil.pdf',
                           "content_type": "text/plain\r\nX-Injecte: 1",
                           "text": "X"}}
        hbody, hctype = encode_multipart(hostile)
        hparsed = parse_multipart(hbody, hctype)
        check("guillemet dans le nom de fichier : le parametre entier, non tronque",
              [list(hparsed), hparsed["doc"][0], hparsed["doc"][2]],
              [["doc"], 'a%22;name=%22evil.pdf', b"X"])
