...
def is_scaffold(line):
    ...
    # finalize applies to the wip bank (runOutputPaths), narrowed here to
    # what is UNTRACKED: a tracked file under .claude/ that was modified is
    # somebody's uncommitted work, which this reader exists to protect. A
    # rename is judged on its destination; a quoted path is unquoted.
    path = line[3:]
    if ' -> ' in path:
        path = path.split(' -> ', 1)[1]
    path = path.strip()
    if len(path) >= 2 and path[0] == chr(34) and path[-1] == chr(34):
        path = path[1:-1]
    return line[:2] == '??' and path.startswith('.claude/')
...
def uncommitted_holdout_entries(gm_dir):
    """The names under mutants/holdout/ that git does NOT track.

    The counterpart of `holdout_committed_entries`, and the one the ephemeral
    notice needs: the seal declines PER SET, so "is anything here committed?"
    stopped being the same question as "is MY set committed?" the day a run
    could stand beside a successor an earlier cycle left.
    """
    src = os.path.join(gm_dir, "mutants", "holdout")
    if not os.path.isdir(src):
        return set()
    return set(os.listdir(src)) - holdout_committed_entries(gm_dir)


def sealed_but_ephemeral(had_uncommitted, sealed_now):
    ...
    The first argument asks whether an UNCOMMITTED set was there to be sealed,
    not whether the directory held anything committed. Those were the same
    question while the seal declined directory-wide; once it declines per set,
    a run can hold both at once — a successor an earlier cycle committed, and
    its own fresh draw — and the directory-wide reading answers for the wrong
    one. It reported "a committed copy exists" on the strength of the
    SUCCESSOR, and this notice, the only warning a run gets that its own set
    will die with its workspace, went silent for every net carrying one.
    """
    return bool(sealed_now) and bool(had_uncommitted)


# Directories that live under mutants/ and are NOT mutants. They are named here
# because the loader used to exclude them by ACCIDENT: they carry no meta.json,
# and "no meta.json" was a silent skip. The moment that silence becomes a
# refusal, the exclusion has to be deliberate — otherwise the gate breaks on the
# evidence pile the net itself publishes under mutants/audit/.
...
def mutant_meta_at(root, name, kind, excluded=()):
    ...
    A FILE at the root (ADVERSARIAL-NOTES.md, X-LENS.md …) is not a mutant and
    is skipped, as is any name in `excluded`.

    A DIRECTORY without meta.json is a MALFORMED mutant, and that is the case
    this exists for. Dropping it in silence shrinks the set while the gate still
    compares `detected == total`, so a set of 7 that loses 3 meta.json reports a
    green 4/4 — the vacuity trap of `holdout 0/0` with a different number. It
    stayed invisible only because git does not lose a tracked file; a set
    carried on anything less durable makes it reachable.
    """
    if name in excluded:
        return None
...
def load_mutants(gm_dir, holdout, sealed_dir=None):
    if holdout:
        ...
        for name in sorted(os.listdir(root)):
            meta = mutant_meta_at(root, name, "held-out")
            if meta is not None:
                out.append(meta)
        return out
    ...
    out = []
    for name in sorted(os.listdir(root)):
        meta = mutant_meta_at(root, name, "visible",
                             excluded=MUTANT_ROOT_NON_MUTANTS)
        if meta is not None:
            out.append(meta)
    return out
...
def lane_exclusion_decls(corpus):
    """The `lane_exclusions` block, validated as data. Returns (decls, problems).

    corpus.json may carry `lane_exclusions` — a list of DECLARED, arbitrated
    renunciations of ONE observation on ONE lane of a multi-environment net:

        [{"entry": "042", "config": "config-b.json", "arbitration": "ARB-7",
          "decided_by": "...", "ts": "...", "cause": "..."}]

    What such a declaration is NOT: an entry deletion (the corpus keeps the
    entry, every lane keeps capturing it) and a canonicalisation rule (that
    would blind the renounced aspect on BOTH lanes, wider than the decision).
    What it IS: on the lane named in `config`, the entry stops CONFRONTING its
    reference — the negative control does not count it, a mutant is not judged
    undetected for leaving it still, and it is never a member of a control
    sample. On every other lane, and in record mode, the observation stands
    exactly as recorded, witness reference included.

    The shape is checked wherever the corpus is read for a verdict, because a
    declared exclusion that no code honours is a waiver with extra steps — and
    a MALFORMED one is a named refusal, never a silent drop: dropped, it would
    read as "this lane never renounced anything" and send the reader to write
    a declaration that already exists.
    """
    raw = corpus.get("lane_exclusions")
    if raw is None:
        return [], []
    if not isinstance(raw, list):
        return [], ["`lane_exclusions` must be a list of declarations, got %s"
                    % type(raw).__name__]
    problems, seen = [], set()
    known = {e["id"] for e in corpus["entries"]}
    for i, x in enumerate(raw):
        where = "lane_exclusions[%d]" % i
        if not isinstance(x, dict):
            problems.append("%s is not an object" % where)
            continue
        entry, cfg = x.get("entry"), x.get("config")
        if not isinstance(entry, str) or not entry.strip():
            problems.append("%s carries no `entry` id" % where)
        elif entry not in known:
            problems.append("%s names entry %r, which the corpus does not "
                            "declare — an exclusion of nothing proves nothing"
                            % (where, entry))
        if not isinstance(cfg, str) or not cfg.strip():
            problems.append("%s carries no `config` — an exclusion is scoped "
                            "to ONE lane, named as the config file the judge "
                            "reads" % where)
...
def lane_exclusion_ids(corpus, cfg_name):
    """Entry ids renounced on THE lane being judged; every other lane is untouched.

    Keyed on the config FILE the verdict carries (`config` in the report), so
    a set committed under one lane can never leak into the other's verdict —
    the same rule that keeps `seal_committed` per environment.
    """
    decls, problems = lane_exclusion_decls(corpus)
    if problems:
        # Unreadable declarations renounce nothing: strictest, and never lying.
        return set()
    return {d["entry"] for d in decls
            if isinstance(d, dict) and d.get("config") == cfg_name
            and isinstance(d.get("entry"), str) and d["entry"]}
...
def score_mutant(meta, config, corpus, canon, refs, ws, seed, excluded=()):
    """Apply → capture → compare → revert. Returns a per-mutant verdict dict.

    `excluded` carries the entries a `lane_exclusions` declaration renounces
    on THE lane being judged. There the mutant's contract shrinks BY
    DECLARATION: a renounced entry cannot be required to move (its reference
    describes another engine's rendering of the junction-table order), cannot
    serve as a control (it never reproduces its reference on this lane), and
    is not captured for comparison at all. When EVERY declared target is
    renounced the mutant measures nothing here — it is skipped, flagged
    `lane_excluded`, and counted neither in the score nor among the invalid:
    a verdict it cannot earn on either side. On the witness lane the set is
    empty and this is the code path it always was.
    """
    ...
    renounced_here = [t for t in targets if t in set(excluded)]
    targets = [t for t in targets if t not in set(excluded)]
    if not targets:
        code, out = revert_mutant(meta, ws)
        if meta.get("needs_restart", True):
            app_restart(config, ws)
        # A failed revert still leaves the tree not-known-to-be-at-baseline —
        # `leftover_on_record` will hold the scoring, exactly as for any other
        # mutant — but the verdict itself is a deferral, not an INVALID. And it
        # is NOT valid: the blind-lane pass would otherwise read its silence as
        # a target that did not move.
        return dict(verdict, valid=False, lane_excluded=True,
                    revert_clean=code == 0,
                    reason="every declared target (%s) is renounced on this "
                           "lane by `lane_exclusions` — not scored here"
                           % ", ".join(renounced_here))
    ...
    sample = control_ids(corpus, set(targets) | set(excluded), seed)
    corpus_ids = {e["id"] for e in corpus["entries"]}
    pinned = ((separator_group_ids(corpus, meta.get("id")) & corpus_ids)
              - set(targets) - set(excluded))
    if pinned:
...
def score_noop(config, corpus, canon, refs, ws, seed, excluded=()):
    ...
    `excluded` — the entries a `lane_exclusions` declaration renounces on THE
    lane being judged — is still captured (the corpus does not shrink), but it
    is not CONFRONTED with its reference here: its reference describes another
    engine's rendering, and counting it would make the negative control fail
    BY CONSTRUCTION on this lane, red for a reason no product work could ever
    lift. The declaration lives in corpus.json, arbitrated; the entry's
    reference stays enforced on every lane the declaration does not name.
    """
    ids = [e["id"] for e in corpus["entries"] if e["id"] not in set(excluded)]
    app_restart(config, ws)
...
def _selftest():
    ...
    def score(targets, sample, moved, unstable=(), revert_code=0, dup_groups=None,
        ...
        def sample_(_c, targets_, _s):
            pooled.append(set(targets_ or ()))
            return list(sample)
        ...
        g.update(apply_mutant=apply_, revert_mutant=revert_, tree_fingerprint=tree,
                 ...
                 capture=w.capture, control_ids=sample_)
        return score_mutant(meta, {}, corpus, None, refs, "/dev/null", 0, excluded), pooled

    try:
        # 1. Un mutant qui ne bouge QUE ses cibles.
        v, pooled = score(["001"], ["005", "006"], ["001"])
        ...
        v, pooled = score(["001"], ["005", "006"], ["001", "005"])
        ...
        v, pooled = score(["001"], ["005", "006"], ["001", "005"], unstable=["005"])
        ...
        v, pooled = score(["001"], ["005", "006"], ["001", "005", "006"], unstable=["006"])
        ...
        v, pooled = score(["001"], ["005"], ["001", "005"], revert_code=3)
        ...
        # 5b. LES EXCLUSIONS DE LANE — une renonciation DECLAREE dans le
        #     corpus, CONSUMEE par le code. Sur la lane qui renonce, l'entree
        #     ne confronte plus sa reference : le mutant n'y est pas juge
        #     aveugle pour l'avoir laissee immobile, l'entree n'entre plus dans
        #     l'echantillon de controle, et un mutant SANS cible restante y est
        #     DIFFERE — ni valide ni invalide, il n'y a pas ete juge. La lane
        #     temoin (excluded vide) suit le chemin d'avant, mot pour mot.
        v, pooled = score(["001", "052"], ["005", "006"], ["001"], excluded={"052"})
        check("renonciation : la cible restante suffit a detecter", v["detected"], True)
        check("renonciation : la cible renoncee n'est pas une lane aveugle",
              v.get("undetected_targets"), [])
        check("renonciation : l'entree quitte le puits par le MEME argument que "
              "les cibles (le harnais la retire de la pool, pas du monde)",
              pooled[0] if pooled else None, {"001", "052"})
        v, pooled = score(["052"], ["005"], [], excluded={"052"})
        check("toutes les cibles renoncees -> differe",
              (v.get("lane_excluded"), v.get("valid")), (True, False))
        check("le differe est dit, avec ses cibles",
              "renounced on this lane" in (v.get("reason") or ""), True)
        check("le puits de controle n'echantillonne jamais une entree renoncee",
              "052" in control_ids({"entries": [{"id": "%03d" % n} for n in range(1, 13)]},
                                   {"001", "052"}, 0), False)
        # Un revert qui echoue reste un revert qui echoue, differe ou non :
        # l'arbre n'est pas connu pour etre revenu a la baseline.
        v, pooled = score(["052"], ["005"], [], revert_code=3, excluded={"052"})
        check("toutes cibles renoncees + revert casse -> revert_clean faux",
              v.get("revert_clean"), False)

        # 5c. Le controle nul : la derive PERMANENTE d'une entree renoncee ne
        #     rend plus le controle bavard SUR LA LANE QUI RENONCE — capturée
        #     encore, confrontee plus. Sans l'exclusion, elle bavarderait, et
        #     ce rouge-la serait par construction : aucun travail produit ne
        #     peut le lever.
        ids_n = ["%03d" % n for n in range(1, 12)] + ["052"]
        refs_n = {i: "ref-" + i for i in ids_n}
        corpus_n = {"entries": [{"id": i, "surface": "http"} for i in ids_n]}
        w_n = World(refs_n, (), unstable=("052",))
        g.update(capture=w_n.capture, app_restart=lambda _c, _w: None)
        n_ = score_noop({}, corpus_n, None, refs_n, "/dev/null", 0)
        check("sans exclusion : la derive permanente rend le controle bavard",
              (n_["silent"], n_["noisy_entries"]), (False, ["052"]))
        n_ = score_noop({}, corpus_n, None, refs_n, "/dev/null", 0, excluded={"052"})
        check("renonciation declaree : le controle reste silencieux",
              (n_["silent"], n_["noisy_entries"]), (True, []))
        g.update(capture=World(refs_n, ()).capture)

        # 5d. La forme de la declaration : un refus NOMME, jamais une ligne
        #     morte. Une declaration que personne ne peut lire ne renonce rien.
        def decls_of(lane_exclusions, entries=("052",)):
            corpus_d = {"entries": [{"id": i} for i in entries],
                        "lane_exclusions": lane_exclusions}
            return lane_exclusion_decls(corpus_d)

        check("absente -> aucune declaration, aucun probleme",
              lane_exclusion_decls({"entries": [{"id": "052"}]}), ([], []))
        check("non-liste -> probleme nomme", len(decls_of("052")[1]), 1)
        check("sans entree -> probleme nomme", len(decls_of([{"config": "c.json", "cause": "x"}])[1]), 1)
        check("entree inconnue -> probleme nomme",
              len(decls_of([{"entry": "099", "config": "c.json", "cause": "x"}], entries=("052",))[1]), 1)
        ...
        check("declaration complete -> aucun probleme",
              decls_of([{"entry": "052", "config": "config-pg.json",
                         "arbitration": "A-X", "cause": "l'ordre des lignes"}])[1], [])
        corpus_l = {"entries": [{"id": "052"}],
                    "lane_exclusions": [{"entry": "052", "config": "config-pg.json",
                                         "cause": "l'ordre des lignes"}]}
        check("l'exclusion vaut sur LA lane nommee",
              lane_exclusion_ids(corpus_l, "config-pg.json"), {"052"})
        check("et ne vaut PAS sur les autres, temoin compris",
              lane_exclusion_ids(corpus_l, "config.json"), set())
        ...
        check("jeu frais scelle hors de l'arbre -> ephemere ; jeu committe -> non",
              [sealed_but_ephemeral(True, True), sealed_but_ephemeral(False, True),
               sealed_but_ephemeral(True, False)],
              [True, False, False])
        # LE MELANGE, la regression que ce predicat portait depuis que le
        # sceau renonce PAR JEU : un successeur COMMITE a cote du jeu FRAIS de
        # ce run. La lecture par repertoire repondait « une copie commitee
        # existe » sur la foi du SUCCESSEUR, et l'avertissement -- le seul que
        # le run recoit pour dire que son propre jeu mourra avec son espace de
        # travail -- se taisait pour tout net portant un successeur.
        wm = os.path.join(sroot, "ephemere-melange")
        gmd_wm = mk_holdout(wm)                   # t01, qui sera COMMITE
        for args in (("init", "-q"), ("add", "-A"), ("commit", "-qm", "successor")):
            fixture_git(wm, *args)
        frais = os.path.join(gmd_wm, "mutants", "holdout", "t02")
        os.makedirs(frais)                        # t02, FRAIS et non suivi
        with open(os.path.join(frais, "apply.sh"), "w", encoding="utf-8") as f:
            f.write("true\n")
        check("melange : le successeur commite ne masque plus le jeu frais ephemere",
              [sorted(holdout_committed_entries(gmd_wm)),
               sorted(uncommitted_holdout_entries(gmd_wm)),
               sealed_but_ephemeral(uncommitted_holdout_entries(gmd_wm), True)],
              [["t01"], ["t02"], True])
        # Et l'inverse, qui doit rester muet : SEUL un successeur commite est
        # la, ce run n'a rien tire, donc rien n'est devenu ephemere.
        ws = os.path.join(sroot, "ephemere-successeur-seul")
        gmd_ws = mk_holdout(ws)
        for args in (("init", "-q"), ("add", "-A"), ("commit", "-qm", "successor")):
            fixture_git(ws, *args)
        check("successeur seul : aucun jeu frais, donc aucun avertissement",
              [sorted(uncommitted_holdout_entries(gmd_ws)),
               sealed_but_ephemeral(uncommitted_holdout_entries(gmd_ws), True)],
              [[], False])
    ...
    v_pin, _pooled = score(["001"], ["005"], ["001", "009"],
    ...
    v_nopin, _pooled = score(["001"], ["005"], ["001", "009"],
...
def main():
    ...
    report = {"mode": mode, "config": cfg_name,
              ...
              # Les renonciations de lane, CONSUMEES : les observations que
              # `lane_exclusions` retire du verdict de CETTE lane (l'entree y
              # reste capturee et sa reference du temoin reste enforcee
              # ailleurs), et les mutants differes faute de cible restante.
              "lane_exclusions_applied": [],
    ...
    # A DECLARED lane renunciation is CONSUMED here, as data, before a boot
    # pays for it. `lane_exclusions` in corpus.json names, per declaration,
    # the entry, the lane (the config file being judged) and the written
    # cause — an arbitrated decision the corpus carries so that every reader
    # of the corpus reads the decision. From here it flows into the negative
    # control and into every mutant verdict on THIS lane; on any other lane,
    # and in record mode, the set is empty and the observation stands as
    # recorded. A malformed declaration is refused before anything runs: a
    # renunciation nobody can parse must not decay into a renunciation
    # nobody honours.
    lane_decls, lane_problems = lane_exclusion_decls(corpus)
    if lane_problems:
        bail("corpus.json carries %d malformed `lane_exclusions` declaration(s): %s. "
    ...
    if excluded_ids:
        reasons = "; ".join(
            "%s (%s)" % (d["entry"], (d.get("arbitration") or "sans reference d'arbitrage"))
            for d in lane_decls
            if isinstance(d, dict) and d.get("config") == cfg_name
            and d.get("entry") in excluded_ids)
        note(report, "lane_exclusions: %d observation(s) renounced on THIS lane (%s) — "
             "still captured, still recorded, still enforced on every other lane, but "
             "not confronted with their reference here [%s]. The corpus does not shrink: "
             "this is a declared renunciation, not a deletion."
             % (len(excluded_ids), ", ".join(sorted(excluded_ids)), reasons))
    ...
    # A malformed mutant refuses from inside the loader. Caught here so the
    # diagnosis lands in the REPORT — the gate's consumer reads that JSON, and a
    # bare traceback would leave it with an exit code and no term to point at.
    #
    # `holdout_detected = -1` for the same reason the reuse arm below does it:
    # bailing leaves the held-out figures at their 0/0 defaults and the gate
    # converges in part on `holdout_detected == holdout_total`, which 0 == 0
    ...
    if mode != "record":
        try:
            # Read BEFORE the seal, and per SET: what matters is whether an
            # UNCOMMITTED set was there to be relocated, not whether the
            # directory happened to hold something committed as well.
            uncommitted_before = uncommitted_holdout_entries(gm_dir)
        ...
        if sealed_but_ephemeral(uncommitted_before, sealed_now):
        ...
        if awaiting:
            ...
            note(report, "a fresh held-out set is COMMITTED under mutants/holdout/ "
                         ...
                         "prepared.")
        # Same refusal as the visible pile, and it matters MORE here: the
        # held-out set is scored once, so a silently shrunk one cannot be caught
        # by a later pass. Same deliberate inequality, so the refusal cannot
    ...
    try:
        ...
        # LES EXCLUSIONS DE LANE, EN CLAIR ET STRUCTURES. Un mutant dont la
        # declARATION entiere est renoncee sur LA lane jugee n'y mesure rien :
        # il n'y est ni valide ni invalide — il n'y a pas ete juge — et le
        # compter d'un cote ou de l'autre ferait du chiffre un terme qu'il
        # n'est pas. Rapporte, jamais gate : la lane temoin garde sa traverse.
        report["lane_exclusions_deferred"] = sorted(
            v["id"] for v in verdicts if v.get("lane_excluded"))
        scored = [v for v in verdicts if not v.get("lane_excluded")]
        valid = [v for v in scored if v.get("valid")]
        ...
        report["invalid"] = [{"id": v["id"], "reason": v.get("reason", "")}
                             for v in verdicts
                             if not v.get("valid") and not v.get("lane_excluded")]
        report.update(
            # Le total est le nombre de mutants JUGES sur cette lane : un
            # mutant differe (`lane_exclusions_deferred`) n'y est ni valide ni
            # invalide, et gonfler le total d'un non-jugement lirait comme une
            # validite manquante.
            total=len(scored),
            valid=len(valid),
        ...
        if not report["noop_silent"]:
            ...
            if noop.get("state_probe"):
                problems.append(
                    ...
                    "qui a derive :\n%s" % noop["state_probe"])
        for v in verdicts:
            # Un mutant DIFFERE par `lane_exclusions` n'est pas invalide : il
            # n'a pas ete juge sur cette lane, et le dire invalide rouvrirait
            # le rouge que la declaration arbitree vient de fermer.
            if not v.get("valid") and not v.get("lane_excluded"):
