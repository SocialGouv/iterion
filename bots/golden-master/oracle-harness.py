...
def extension_verdict(ws, gm_rel, base, acted_commits=None, acted_blobs=None,
    ...
    if corpus_changed:
        ...
        if head_c is not None and base_c is not None:
            ...
            # whole function holds. Four rules keep the channel to the case
            # that forces it open:
            #
            #   1. every base declaration survives VERBATIM — re-opening a
            #      settled identity claim is a re-anchor's job, not an
            #      extension's;
            #   2. an added declaration is well-formed, because the gate's
            #      reader drops a malformed one in SILENCE and it would read as
            #      no declaration at all — an unconstrained free write into the
            #      key just unfrozen;
            #   3. an added declaration names at least one entry THIS extension
            ...
            #   4. no two declarations name the same group once one of them is
            ...
            if not isinstance(head_dg, list):
                ...
                gone = [s for s in base_ser if s not in head_ser]
                if gone:
                    corpus_problems.append(
                        "%d `duplicate_groups` declaration(s) REMOVED or "
                        "MODIFIED: %s — a settled identity claim is re-opened "
                        "by a re-anchor, not by an extension"
                        % (len(gone), "; ".join(gone)))
                bearers = {}
...
def _selftest():
    ...
    try:
        ...
        # 6. Une declaration de BASE retiree ou retouchee : reouvrir une
        #    identite tranchee est le travail d'un re-ancrage, pas d'une
        #    extension. La base porte ici la declaration.
        xreset()
        with open(os.path.join(xgm, "corpus.json"), "w", encoding="utf-8") as f:
        ...
        check("une extension qui laisse la declaration de base intacte -> ok",
              ...
              True)
        check("declaration de base retiree -> refusee",
              xdg_from({"entries": [base_entry, new_entry],
                        "duplicate_groups": []})["acted"][0]["ok"], False)
