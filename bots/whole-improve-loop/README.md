...
- **`campaign`** (adaptive, claude_code, whole-repo, full tools) is the whole
  ...
  change → build → test → **commit** (`git add -A -- . ':(exclude).claude'` incl. untracked, semantic
...
passes, staging untracked files (`git add -A -- . ':(exclude).claude'`) so a change that **adds** files
