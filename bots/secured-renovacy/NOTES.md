...
- **Reproducibility pin (codex #14):** sandbox image is the rolling `:edge`; claude-code is installed `@latest` in post_create. The recipe has comments explaining how to pin, but doesn't force a specific digest because operator environments vary. When this recipe ships to a production pipeline, pin both deliberately.
