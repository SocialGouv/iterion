...
//
// whole-improve-loop, branch-improve-loop, feature-dev and docs-refresh are
// intentionally NOT in this set: their v2 shape (ADR-058 — one adaptive
// `campaign` agent that self-reviews via a fresh re-review of the work + the
// deterministic gates + its own judgment, the operator's proven manual
// pattern) has no cross-family reviewer nodes and no `condition` review
// router, so the ADR-052 mono/dual topology invariant does not apply to
// them. See bots/whole-improve-loop/main.bot for the reference shape.
