...
// carrying EVERY key the real producer emits, `generated` included. A stub
// that drops a term the reader depends on proves the reader against a document
// no run can hand it, and the reader's refusal for that missing term would
// never be seen.
func writeFloor(t *testing.T, scratch string, lines map[string]int) string {
	t.Helper()
	return writeFloorWithGenerated(t, scratch, lines, nil)
}

func writeFloorWithGenerated(t *testing.T, scratch string, lines map[string]int, generated []string) string {
	t.Helper()
	...
	document := map[string]any{
		...
		"generated": append([]string{}, generated...),
	}
...
// THE PROFILE DECLARES A `generated` CANONICAL EXCLUSION, so something has to
// apply it. It was compiled and never used: a file carrying a
// `Code generated … DO NOT EDIT` header inside a declared first-party subtree
// counted as source written here, inflating `first_party_lines` — one of the
// four index metrics, and the `min_first_party_lines` gate that decides
// whether a letter is published at all.
//
// Both directions on one bench: the exclusion moves the number when the tree
// carries generated code, and leaves it alone when it does not.
func TestAssessmentGeneratedCodeIsNotFirstPartySource(t *testing.T) {
	requireAssessmentTools(t)
	ws := measureWorkspace(t)
	stacks := []map[string]any{{"id": "synth", "evidence": "a", "supported": true}}
	survey := writeSurvey(t, t.TempDir(), "deadbeefdeadbeef", stacks, inDomainSurvey(2))
	...
	lines := floorLines(20000)
	lines["src/api.pb.txt"] = 40000

	plainScratch := t.TempDir()
	plain := measure(t, ws, plainScratch,
		survey, writeFloorWithGenerated(t, plainScratch, lines, nil))
	generatedScratch := t.TempDir()
	excluded := measure(t, ws, generatedScratch,
		survey, writeFloorWithGenerated(t, generatedScratch, lines, []string{"src/api.pb.txt"}))

	before, _ := plain["index"].(float64)
	after, _ := excluded["index"].(float64)
	if after >= before {
		t.Fatalf("declaring 40 000 lines self-generated did not lower the published index "+
			"(%v -> %v) — the profile advertises the exclusion and nothing applies it", before, after)
	}
	facts := assessmentString(t, excluded, "facts")
	if !strings.Contains(facts, "of self-declared generated code") {
		t.Errorf("the document does not say what was subtracted:\n%s", facts)
	}
	// And the legitimate case stays untouched: a tree with no generated file
	// publishes the same number it always did, and says so.
	if !strings.Contains(assessmentString(t, plain, "facts"), "no file inside the declared perimeter declares itself generated") {
		t.Errorf("a tree with no generated file does not say so:\n%s", assessmentString(t, plain, "facts"))
	}
	t.Logf("40 000 generated lines inside the perimeter: index %.3f -> %.3f, size %q -> %q",
		before, after, assessmentString(t, plain, "size"), assessmentString(t, excluded, "size"))
}

// The floor RECOGNISES what declares itself generated, in the one pass that
// already holds every body. Proven against the real node, not a stub.
func TestAssessmentFloorRecordsSelfDeclaredGeneratedFiles(t *testing.T) {
	requireAssessmentTools(t)
	ws := t.TempDir()
	...
	files := map[string]string{
		"src/api.pb.go":  "// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage api\n",
		"src/marked.go":  "/* @generated */\npackage api\n",
		"src/written.go": "package api\n\nfunc Real() {}\n",
		"go.mod":         "module x\n",
	}
	...
	got, _ := floor["generated"].([]any)
	found := map[string]bool{}
	for _, entry := range got {
		found[fmt.Sprint(entry)] = true
	}
	for _, want := range []string{"src/api.pb.go", "src/marked.go"} {
		if !found[want] {
			t.Errorf("%s declares itself generated and the floor did not record it: %v", want, got)
		}
	}
	if found["src/written.go"] {
		t.Error("a hand-written file was recorded as generated — the exclusion would eat real source")
	}
}
...
// commitAll turns a directory into a git repository carrying everything in it
