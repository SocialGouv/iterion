...
`chore(...)` config/deps. Stage with `git add -A -- ':/' $ITERION_TREE_NOISE` before EVERY commit — a file under `.claude/` that IS your deliverable is staged by name (`git add -- .claude/<path>`): the exclusion spares the mirror, not your work
