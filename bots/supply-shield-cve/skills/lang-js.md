...
## npm CVEs in supply-shield-cve

For the CVE bot the npm signal of record is `vuln-db-known` — from the
universal floor (trivy + osv-scanner over `package-lock.json` /
`yarn.lock` / `pnpm-lock.yaml`, no install) plus the `npm audit` heuristic
below when an installed tree is present. See `[[cve-scanning]]`. (The
js-x-ray AST *malware* analysis is the companion bot supply-shield's job,
not this one.)

## See also

- `[[cve-scanning]]` — the trivy + osv CVE floor and advisory handling.
- `[[malware-signals]]` — canonical signal catalogue (`vuln-db-known`).
