...
func init() {
	...
	f.StringVar(&resumeOpts.supervisors, "supervisors", "", "spawn the workflow's DSL-declared supervisor: watchers on resume: on|off. Empty inherits ITERION_SUPERVISORS — NOT the original launch, which is not persisted, so re-state a launch-time off here. See docs/supervisors.md.")
