	...
	for {
		...
		if time.Now().After(deadline) {
			...
			for key := range firstSeen {
				...
				// Same PID-reuse standard as procKey: only signal the process
				// that actually earned the entry, never whoever holds the
				// number now.
				if stat, err := processState(key.pid); err == nil && stat.start == key.start && stat.parent == os.Getpid() {
					reclaim(p)
				}
				_ = p.Release()
			}
