...
func TestOnlyAPublishedNodeLeavesAnArtifact(t *testing.T) {
	...
	exec.on("produces_artifact", func(map[string]any) (map[string]any, error) { return out, nil })
	exec.on("produces_nothing", func(map[string]any) (map[string]any, error) { return out, nil })
	...
	// The published node is readable by exactly the call the hand-off makes.
	art, err := s.LoadLatestArtifact(ctx, runID, "produces_artifact")
	if err != nil || art == nil {
		t.Fatalf("a node declaring `publish:` left no artifact (%v) — every hand-off from it resolves to nothing", err)
	}
	if n, _ := art.Data["total_findings"].(float64); n != 1 {
		t.Errorf("artifact does not carry the node's output: %v", art.Data)
	}

	// And the node that ran identically but declared no `publish:` left none —
	// the silent miss this test exists to make loud.
	if got, err := s.LoadLatestArtifact(ctx, runID, "produces_nothing"); err == nil && got != nil {
