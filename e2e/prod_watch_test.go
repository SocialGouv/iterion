...
func TestProdWatch_LokiWindowPagingAndTruncation(t *testing.T) {
	...
	stateJSON := fmt.Sprintf(`{"version":1,"generation":1,"cursors":{"loki":{"errors":%s}},"incidents":{},"health":{}}`,
		cursorOf(t, pq))
...
// cursorOf renders the cursor decide persists from a per-query result.
func cursorOf(t *testing.T, pq map[string]any) string {
	t.Helper()
	return mustJSON(t, map[string]any{"covered_to_ns": pq["covered_to_ns"], "frontier_ns": pq["frontier_ns"],
		"band": pq["band"], "band_base_ns": pq["band_base_ns"], "overlap_from_ns": pq["overlap_from_ns"]})
}
...
func TestProdWatch_LeakScanValidators(t *testing.T) {
	...
	lines := []string{
		...
		`card 4539/1488/0343/6467 declined`,                                   // slashes — R4
		`card 4539_1488_0343_6467 refused`,                                    // underscores — R4
		`card 4539,1488,0343,6467 rejected`,                                   // commas — R4
		`ref 4539|1488|0343|6467 kept`,                                        // pipes, no card word: a card's grouping and issuer — R4
		`cache keys 8858.1929.9353.5054 evicted`,                              // Luhn-valid, card-shaped, issuer 88: not a card — R4
		`build 8654-5926-2401-8899 deployed by ci`,                            // idem, issuer 86 — R4
		`counters 8858 1929 9353 5054`,                                        // idem — R4
		`shard offsets 9998 0031 7855 5636 1389`,                              // a row of five: the run continues past 19 digits — R4
		`quota window 3265-7832-2924-7109-808 used`,                           // 4-4-4-4-3, issuer 32: not a card — R4
		`ref 1234 5678 9012 3452 seen`,                                        // Luhn-valid 4-4-4-4, issuer 12: not a card — R4
		`ERROR transaction rejected 45 39 14 88 03 43 64 67 amount 5`,         // a PAN in pairs: not a phone number, masked whole — R4
	...
	want := map[string]float64{"nir": 2, "iban": 3, "card": 15, "jwt": 1, "bearer": 1, "secret_kv": 2, "email": 2, "phone_fr": 1}
	...
	for _, raw := range []string{"1 85 03 75 123 456 41", "3456 7890 189", "4539 1488 0343 6467", "SuperSecret123", "marie.curie", "pierre@", "eyJhbGciOiJIUzI1NiJ9",
		...
		"4539/1488", "4539_1488", "4539,1488", "4539|1488", "8858.1929", "8654-5926", "8858 1929", "9998 0031", "3265-7832", "1234 5678 9012 3452", "45 39 14 88"} {
	...
	if !strings.Contains(string(sig), "order <num> shipped") || !strings.Contains(string(sig), "counters <num>") || !strings.Contains(string(sig), "shard offsets <num>") || !strings.Contains(string(sig), "rejected <num> amount") {
		t.Fatal("a digit run under one repeated separator that is not a card must still be masked in the sample")
	}
...
func TestProdWatch_PlanGuards(t *testing.T) {
	...
	// The plan carries the band as it is — the cut is the producer's, and a
	// reader that cut it at the wrong end would forget the newest line.
	h.writeConfig(t, nil)
	line := pwLine{TS: nsAgo(30 * time.Second), Line: "ERROR known", Container: "api", Q: "errors-q"}
	h.lines.Store([]pwLine{line})
	sum := sha1.Sum([]byte(strconv.FormatInt(line.TS, 10) + "\x00" + line.Line))
	base := nsAgo(25 * time.Second)
	band := make([]string, 0, 4001)
	for i := 0; i < 4000; i++ {
		band = append(band, fmt.Sprintf("0:%016x", i))
	}
	band = append(band, strconv.FormatInt(line.TS-base, 10)+":"+hex.EncodeToString(sum[:])[:16])
	...
	_ = os.WriteFile(filepath.Join(h.ws, ".prod-watch", "state.json"), []byte(fmt.Sprintf(
		`{"version":1,"generation":1,"cursors":{"loki":{"errors":{"covered_to_ns":"%d","frontier_ns":"%d","band":%s,"band_base_ns":"%d","overlap_from_ns":"0"}}},"incidents":{},"health":{}}`,
		nsAgo(20*time.Second), nsAgo(20*time.Second), mustJSON(t, band), base)), 0o644)
	out, _, err = plan(h.tokenFile)
	if err != nil || len(out["loki"].(map[string]any)["windows"].(map[string]any)["errors"].(map[string]any)["band"].([]any)) != 4001 {
		t.Fatalf("the plan must pass the band through untouched: %v", err)
	}
	loki, stderr, err := runPyWhole(t, h.ws, pwSub(t, pwTool(t, wf, "poll_loki").Script, map[string]any{
		"grafana": out["grafana"], "loki": out["loki"], "timeout_secs": 5, "scratch_dir": h.scratch, "allow_private": true}, nil, map[string]string{"grafana_token": h.tokenFile}))
	if err != nil || loki["lines"].(float64) != 0 {
		t.Fatalf("the newest pair of a long band is still known: %v %s %v", loki, stderr, err)
	}
...
func TestProdWatch_LokiOverlapAfterTruncation(t *testing.T) {
	...
	pq := loki["per_query"].(map[string]any)["errors"].(map[string]any)
	if loki["truncated"] != true || len(pq["band"].([]any)) != 2 {
		t.Fatalf("a truncated tick must still hand over the band of the lines it counted: %v", pq)
	}
	if err := os.MkdirAll(filepath.Join(h.ws, ".prod-watch"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	stateJSON = fmt.Sprintf(`{"version":1,"generation":2,"cursors":{"loki":{"errors":%s}},"incidents":{},"health":{}}`,
		cursorOf(t, pq2))
	if err := os.WriteFile(filepath.Join(h.ws, ".prod-watch", "state.json"), []byte(stateJSON), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if loki3["lines"].(float64) != 0 || pq3["skipped_overlap"].(float64) < 1 || len(pq3["band"].([]any)) == 0 || loki3["truncated"] == true {
...
// — the cursor stays, time heals it — never a covered window, and never
// the death of the run: with a single query the node still answers (the
// lane is degraded; only decide may refuse a tick, when every lane failed).
func TestProdWatch_LokiInvertedWindowIsAnError(t *testing.T) {
	...
	if pq["covered_to_ns"] != w["prev_covered_to_ns"] || len(pq["band"].([]any)) != len(w["band"].([]any)) {
		t.Fatalf("the cursor and its band are handed back untouched: %v vs %v", pq, w)
	}
}
...
func TestProdWatch_LokiOverlapCapCarriesItsBound(t *testing.T) {
	...
	if outs["poll_loki"]["lines"].(float64) != 4100 || len(pq["band"].([]any)) != 4000 {
		t.Fatalf("tick 1 reads the burst and hands over the last 4000 pairs: lines=%v band=%d", outs["poll_loki"]["lines"], len(pq["band"].([]any)))
	}
	if from, _ := strconv.ParseInt(fmt.Sprint(pq["overlap_from_ns"]), 10, 64); from != lines[99].TS+1 {
		t.Fatalf("the cut band must carry the bound it still covers (just past line 99 at %d): %v", lines[99].TS, pq["overlap_from_ns"])
	}
	outs2 := h.tick(t, wf, false)
...
// TestProdWatch_LokiGroupWiderThanCapDrainsAcrossTicks: lines sharing one
// nanosecond, more of them than max_lines allows. The walk cannot get past
// the group in one tick; the page grows by the lines already seen at the
// resume point (re-reads are not charged), so the group drains max_lines
// new lines a tick and the line after it follows — every line written
// exactly once, the lane healthy, the mark never backwards. Neither a
// cursor parked for good nor an error that kills the run.
func TestProdWatch_LokiGroupWiderThanCapDrainsAcrossTicks(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, func(cfg map[string]any) {
	...
	written := countRawLines(t, nil, loki["raw_file"].(string))
	hwm, _ := strconv.ParseInt(pq["covered_to_ns"].(string), 10, 64)
	for tick := 2; tick <= 7; tick++ {
		stateJSON := fmt.Sprintf(`{"version":1,"generation":%d,"cursors":{"loki":{"errors":%s}},"incidents":{},"health":{}}`, tick, cursorOf(t, pq))
		if err := os.WriteFile(filepath.Join(h.ws, ".prod-watch", "state.json"), []byte(stateJSON), 0o644); err != nil {
			t.Fatal(err)
		}
		loki = poll()
		pq = loki["per_query"].(map[string]any)["errors"].(map[string]any)
		if pq["error"] != "" || loki["ok"] != true {
			t.Fatalf("tick %d: the group drains, it is not an error: %v", tick, pq)
		}
		if c, _ := strconv.ParseInt(pq["covered_to_ns"].(string), 10, 64); c < hwm {
			t.Fatalf("tick %d: the high-water mark moved backwards: %d < %d", tick, c, hwm)
		} else {
			hwm = c
		}
		countRawLines(t, written, loki["raw_file"].(string))
	}
	if len(written) != 11 {
		t.Fatalf("every line of the group and the line after it must be written: %d of 11 (%v)", len(written), written)
	}
...
// countRawLines adds the lines of a poll_loki raw file to the tally.
func countRawLines(t *testing.T, tally map[string]int, rawFile string) map[string]int {
	t.Helper()
	if tally == nil {
...
func lokiOnly(page, overlap int) func(cfg map[string]any) {
	return func(cfg map[string]any) {
		cfg["loki"].(map[string]any)["page_size"] = page
		cfg["loki"].(map[string]any)["overlap_seconds"] = overlap
		cfg["loki"].(map[string]any)["bootstrap_window_minutes"] = 10
		cfg["loki"].(map[string]any)["queries"] = map[string]any{"errors": "errors-q"}
		cfg["prometheus"] = map[string]any{"probes": []map[string]any{}}
		cfg["probes"] = []map[string]any{}
	}
...
// lokiTwoQueries: a second, always-empty query keeps the lane up when the
// first one fails — the failure fixtures need the tick to go on.
func lokiTwoQueries(page, overlap int) func(cfg map[string]any) {
	return func(cfg map[string]any) {
		lokiOnly(page, overlap)(cfg)
		cfg["loki"].(map[string]any)["queries"] = map[string]any{"errors": "errors-q", "other": "other-q"}
	}
}

// TestProdWatch_LokiFailureMidWalkKeepsItsLinesOnce: a query that fails
// after writing part of its window keeps those lines (they are scanned
// this tick) and does not write them again when the window is retried:
// the cursor stays, the band knows what was written.
func TestProdWatch_LokiFailureMidWalkKeepsItsLinesOnce(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	...
	var lines []pwLine
	for i := 0; i < 6; i++ {
		lines = append(lines, pwLine{TS: nsAgo(30*time.Second) + int64(i)*int64(time.Second), Line: fmt.Sprintf("ERROR line %d", i), Container: "api", Q: "errors-q"})
	}
	h.lines.Store(lines)
	h.failLokiFrom.Store(int64(len(h.calls()) + 2)) // the second page of errors-q…
	h.failLokiCount.Store(2)                        // …and its retry
	outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
	if !strings.Contains(fmt.Sprint(pq["error"]), "500") || pq["lines"].(float64) != 2 || outs["poll_loki"]["ok"] != true {
		t.Fatalf("tick 1: the second page fails after two lines were written; the lane stays up on its other query: %v ok=%v", pq, outs["poll_loki"]["ok"])
	}
	written := countRawLines(t, nil, outs["poll_loki"]["raw_file"].(string))
	h.failLokiFrom.Store(0)
	outs2 := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	pq2 := outs2["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
	countRawLines(t, written, outs2["poll_loki"]["raw_file"].(string))
	if pq2["error"] != "" || pq2["lines"].(float64) != 4 || len(written) != 6 {
		t.Fatalf("tick 2 writes the four remaining lines, not the two again: %v %v", pq2, written)
	}
	for l, n := range written {
		if n != 1 {
			t.Fatalf("%q written %d times", l, n)
...
// bound that falls back to covered − overlap reopens the window over the
// cut lines and rewrites them every other tick, for ever.
func TestProdWatch_LokiBurstPastTheBandCapIsCountedOnce(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	...
	for tick := 1; tick <= 4; tick++ {
		...
		if n := outs["poll_loki"]["lines"].(float64); (tick == 1 && n != 5200) || (tick > 1 && n != 0) {
			t.Fatalf("tick %d wrote %v lines (5200 once, then none): %v", tick, n, outs["poll_loki"]["per_query"])
		}
	}
	if len(written) != 5200 {
		t.Fatalf("every line of the burst is written: %d", len(written))
	}
}

// TestProdWatch_LokiOverlapRaisedNeverReopensBelowTheBand: raising
// overlap_seconds (the knob for late ingestion) must not open the next
// window below what the band knows — the lines there would be written a
// second time.
func TestProdWatch_LokiOverlapRaisedNeverReopensBelowTheBand(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.lines.Store([]pwLine{
		{TS: nsAgo(200 * time.Second), Line: "ERROR old", Container: "api", Q: "errors-q"},
		{TS: nsAgo(20 * time.Second), Line: "ERROR recent", Container: "api", Q: "errors-q"},
	})
	written := map[string]int{}
	for tick, overlap := range []int{1, 300, 300} {
		h.writeConfig(t, lokiOnly(1000, overlap))
		outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
		countRawLines(t, written, outs["poll_loki"]["raw_file"].(string))
...
func TestProdWatch_LokiForeignBandFormatIsRefusedByName(t *testing.T) {
	...
	secrets := map[string]string{"grafana_token": h.tokenFile}
	_, stderr, err := runPyWhole(t, h.ws, pwSub(t, pwTool(t, wf, "plan").Script, nil, vars, secrets))
	if err == nil || !strings.Contains(stderr, "offset:hash") || !strings.Contains(stderr, "errors") || strings.Contains(stderr, "Traceback") {
		t.Fatalf("a foreign band format must refuse by name, naming the query, before any lane runs: %v %s", err, stderr)
	}
}
...
// TestProdWatch_BootstrapGateFollowsTheLinesNotTheQueryOrder: a template
// fed by an established query AND by a query still reading its first
// window is news for the live lines only — whatever the order of the
// queries in the config. Attributed to the first query that emitted it,
// the gate either swallowed a live alert or replayed sixty lines of history
// as an escalation.
func TestProdWatch_BootstrapGateFollowsTheLinesNotTheQueryOrder(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	for _, newq := range []string{"aaa_new", "zzz_new"} {
		...
		t.Run(newq, func(t *testing.T) {
			...
			h.lines.Store(lines)
			h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000)) // the established query has its cursor
			h.writeConfig(t, func(cfg map[string]any) {
				lokiTwoQueries(1000, 60)(cfg)
				cfg["loki"].(map[string]any)["queries"].(map[string]any)[newq] = "new-q"
			})
			for i := 0; i < 60; i++ { // the new query's first window: history
				lines = append(lines, pwLine{TS: nsAgo(3*time.Minute) + int64(i)*int64(time.Second), Line: fmt.Sprintf("ERROR db timeout %d ms", i), Container: "api", Q: "new-q"})
			}
			lines = append(lines, pwLine{TS: nsAgo(5 * time.Second), Line: "ERROR db timeout 7 ms", Container: "api", Q: "errors-q"}) // live, on the established query
			h.lines.Store(lines)
			outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
...
// TestProdWatch_LokiFailedBootstrapWindowIsRetriedWhereItOpened: a query
// whose first walk failed keeps no mark, and its bootstrap window must not
// slide with the clock — the retry reopens where the failed attempt
// opened, or the bottom of the window is lost silently.
func TestProdWatch_LokiFailedBootstrapWindowIsRetriedWhereItOpened(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, func(cfg map[string]any) {
		lokiTwoQueries(1000, 60)(cfg)
		cfg["loki"].(map[string]any)["bootstrap_window_minutes"] = 1
	})
	h.lines.Store([]pwLine{{TS: nsAgo(58 * time.Second), Line: "ERROR near the bottom", Container: "api", Q: "errors-q"}})
	h.failLokiFrom.Store(int64(len(h.calls()) + 1))
	h.failLokiCount.Store(2)
	outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	if pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any); pq["error"] == "" || pq["lines"].(float64) != 0 {
		t.Fatalf("tick 1: the first walk fails before reading: %v", pq)
	}
	h.failLokiFrom.Store(0)
...
// TestProdWatch_LokiZeroWidthAfterTruncationAtTheBoundKeepsTheBand: with
// no overlap, the band's bound lands exactly on the frontier line; a
// zero-width tick right after must keep that line (nothing re-adds it), or
// the next full window, opening at the bound inclusive, writes it again.
func TestProdWatch_LokiZeroWidthAfterTruncationAtTheBoundKeepsTheBand(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	...
	countRawLines(t, written, outs["poll_loki"]["raw_file"].(string))
	if pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any); pq["truncated"] != true {
		t.Fatalf("tick 1 is truncated at L1: %v", pq)
	}
	outs = h.cursorTick(t, wf, cursorVars(h, 60, 400, 5000))
	if pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any); pq["from_ns"] != pq["to_ns"] || len(pq["band"].([]any)) == 0 {
		t.Fatalf("tick 2 is zero-width and keeps the band: %v", pq)
	}
	outs = h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	countRawLines(t, written, outs["poll_loki"]["raw_file"].(string))
	for _, l := range []string{"ERROR L0", "ERROR L1", "ERROR L2"} {
		if written[l] != 1 {
			t.Fatalf("%q written %d times: %v", l, written[l], written)
		}
	}
}

// TestProdWatch_LokiCursorOutOfTheWindowIsAGap: a cursor older than the max
// window declares a gap — the lines between are lost for good — and the
// tick's coverage is partial.
func TestProdWatch_LokiCursorOutOfTheWindowIsAGap(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, lokiOnly(1000, 60))
	old := nsAgo(2 * time.Hour)
	_ = os.MkdirAll(filepath.Join(h.ws, ".prod-watch"), 0o755)
	_ = os.WriteFile(filepath.Join(h.ws, ".prod-watch", "state.json"), []byte(fmt.Sprintf(
		`{"version":1,"generation":1,"cursors":{"loki":{"errors":{"covered_to_ns":"%d","frontier_ns":"%d","band":[],"band_base_ns":"0","overlap_from_ns":"%d"}}},"incidents":{},"health":{}}`,
		old, old, old)), 0o644)
	outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
...
// TestProdWatch_LokiBandCutNeverPassesTheFrontier: a truncated walk that
// stops inside a same-nanosecond group low in a full band: the cut must
// not drop that group, or the next window reopens past it and the unread
// rest of the group is lost for good.
func TestProdWatch_LokiBandCutNeverPassesTheFrontier(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, lokiOnly(1000, 300))
	base := nsAgo(200 * time.Second)
	var lines []pwLine
	for i := 0; i < 4000; i++ {
		lines = append(lines, pwLine{TS: base + int64(i)*int64(40*time.Millisecond), Line: fmt.Sprintf("ERROR steady %d", i), Container: "api", Q: "errors-q"})
	}
	h.lines.Store(lines)
	written := map[string]int{}
	outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	countRawLines(t, written, outs["poll_loki"]["raw_file"].(string))
	if len(written) != 4000 {
		t.Fatalf("tick 1 writes the steady lines: %d", len(written))
	}
	late := nsAgo(280 * time.Second) // older than the steady lines, inside the overlap
	for i := 0; i < 200; i++ {
		lines = append(lines, pwLine{TS: late, Line: fmt.Sprintf("ERROR late-group %d", i), Container: "api", Q: "errors-q"})
	}
	h.lines.Store(lines)
	outs = h.cursorTick(t, wf, cursorVars(h, 60, 0, 50))
	countRawLines(t, written, outs["poll_loki"]["raw_file"].(string))
	pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
	if pq["truncated"] != true || outs["poll_loki"]["lines"].(float64) != 50 {
		t.Fatalf("tick 2 reads 50 lines of the late group and is truncated inside it: %v", pq)
	}
	...
	if len(written) != 4200 {
		t.Fatalf("the rest of the late group must not be lost past the cut: %d of 4200 written", len(written))
	}
	for l, n := range written {
		if n != 1 {
			t.Fatalf("%q written %d times", l, n)
...
// TestProdWatch_DecideDarkQueryDoesNotHealTheLane: one query failing while
// another is fine keeps the run alive, but must not record the lane as
// healthy — the staleness note is how a query dark for good stays visible,
// and it carries the error.
func TestProdWatch_DecideDarkQueryDoesNotHealTheLane(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	...
	health := pwStateNext(t, out)["health"].(map[string]any)["loki"].(map[string]any)
	if health["last_ok"] != lastOK || health["last_error"] == nil {
		t.Fatalf("a tick with a failing query must not record the lane as healthy: %v", health)
	}
	var staleLoki map[string]any
	for _, s := range out["stale_sources"].([]any) {
		if m := s.(map[string]any); m["source"] == "loki" {
			staleLoki = m
		}
	}
...
// TestProdWatch_DecidePartialObservationConcludesNothing: a Loki tick that
// did not observe everything — a truncated walk, a zero-width window, a
// gap, or a lane not configured — concludes nothing about the incidents it
// did not see: no "not observed any more", no forgetting, for log and leak
// incidents alike.
func TestProdWatch_DecidePartialObservationConcludesNothing(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	incidents := func() map[string]any {
	...
	cases := map[string]map[string]any{
		...
		"gap":        {"lines": 3, "error": "", "truncated": false, "gap": true, "covered_to_ns": "900", "from_ns": "100", "to_ns": "900"},
	}
	for name, pq := range cases {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			h := newPWHarness(t)
			state := map[string]any{"version": 1, "generation": 3, "cursors": map[string]any{"loki": map[string]any{}}, "health": map[string]any{}, "incidents": incidents()}
			out, stderr, err := pwDecide(t, wf, h, signals, state, map[string]any{
				"loki_ok": true, "loki_truncated": pq["truncated"], "loki_errors": []any{}, "loki_per_query": map[string]any{"errors": pq},
				"lanes": map[string]any{"loki": true, "prometheus": false, "probes": false}})
			if err != nil {
				t.Fatalf("decide: %v %s", err, stderr)
			}
	...
	t.Run("lane-not-configured", func(t *testing.T) {
		t.Parallel()
		h := newPWHarness(t)
		state := map[string]any{"version": 1, "generation": 3, "cursors": map[string]any{"loki": map[string]any{}}, "health": map[string]any{}, "incidents": incidents()}
		out, stderr, err := pwDecide(t, wf, h, signals, state, map[string]any{
			"loki_ok": true, "loki_errors": []any{}, "loki_per_query": map[string]any{},
			"lanes": map[string]any{"loki": false, "prometheus": false, "probes": true}})
		if err != nil {
			t.Fatalf("decide: %v %s", err, stderr)
		}
		if got := alertsOf(t, map[string]map[string]any{"decide": out}); len(got) != 0 {
...
// TestProdWatch_DecideDroppedQueryKeepsItsMark: a query absent from this
// tick's config keeps its high-water mark (its band is dropped) so that a
// re-added query reopens at the mark instead of replaying a bootstrap
// window; a cursor untouched for longer than forget_after_days goes.
func TestProdWatch_DecideDroppedQueryKeepsItsMark(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	recent := time.Now().Add(-1 * time.Hour).UTC().Format("2006-01-02T15:04:05+00:00")
	ancient := time.Now().Add(-15 * 24 * time.Hour).UTC().Format("2006-01-02T15:04:05+00:00")
	state := map[string]any{"version": 1, "generation": 3, "incidents": map[string]any{}, "health": map[string]any{},
		"cursors": map[string]any{"loki": map[string]any{
	...
	if _, present := cursors["other"]; !present {
		t.Fatalf("the live query's cursor is persisted: %v", cursors)
	}
}

// TestProdWatch_LokiLateLinesUnderTruncationNeverRewrite: a truncated walk
// that stops among late-arriving lines (older than the previous cursor)
// neither moves the high-water mark backwards nor forgets the lines the
// previous band knew: nothing is written twice, and the line beyond the
// cap is written once the walk gets there.
func TestProdWatch_LokiLateLinesUnderTruncationNeverRewrite(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, lokiOnly(7, 60))
	l := func(agoSec int, name string) pwLine {
		return pwLine{TS: nsAgo(time.Duration(agoSec) * time.Second), Line: name, Container: "api", Q: "errors-q"}
	}
	lines := []pwLine{l(20, "A"), l(19, "B"), l(18, "C"), l(17, "D")}
	h.lines.Store(lines)
	written := map[string]int{}
	var hwm int64
	tick := func(label string) map[string]any {
		t.Helper()
		outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 3))
		pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
		if pq["error"] != "" {
			t.Fatalf("%s: %v", label, pq)
		}
		countRawLines(t, written, outs["poll_loki"]["raw_file"].(string))
		c, _ := strconv.ParseInt(pq["covered_to_ns"].(string), 10, 64)
		if c < hwm {
			t.Fatalf("%s: the high-water mark moved backwards: %d < %d", label, c, hwm)
		}
		hwm = c
		return pq
	}
	if pq := tick("tick 1"); pq["truncated"] != true || len(written) != 3 {
		t.Fatalf("tick 1 writes A, B, C and is truncated: %v %v", pq, written)
	}
	lines = append(lines, l(25, "late1"), l(24, "late2"), l(23, "late3"))
	h.lines.Store(lines)
	if pq := tick("tick 2"); pq["truncated"] != true || written["late3"] != 1 {
		t.Fatalf("tick 2 writes the late lines and is truncated: %v %v", pq, written)
	}
	tick("tick 3")
	tick("tick 4")
	for _, name := range []string{"A", "B", "C", "D", "late1", "late2", "late3"} {
		if written[name] != 1 {
			t.Fatalf("%s written %d times (want exactly once): %v", name, written[name], written)
		}
	}
}

// TestProdWatch_LokiSameNanosecondGroupAtTheBandCut: the band is bounded,
// but the cut never lands inside a group sharing one nanosecond — the
// whole group stays, and the next window does not count it again.
func TestProdWatch_LokiSameNanosecondGroupAtTheBandCut(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, lokiOnly(1000, 120))
	t0 := nsAgo(100 * time.Second)
	var lines []pwLine
	for i := 0; i < 200; i++ {
		lines = append(lines, pwLine{TS: t0, Line: fmt.Sprintf("ERROR same-ns %d", i), Container: "api", Q: "errors-q"})
	}
	for i := 0; i < 3900; i++ {
		lines = append(lines, pwLine{TS: t0 + int64(i+1)*int64(7*time.Millisecond), Line: fmt.Sprintf("ERROR after %d", i), Container: "api", Q: "errors-q"})
	}
	h.lines.Store(lines)
	outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
	if outs["poll_loki"]["lines"].(float64) != 4100 || len(pq["band"].([]any)) != 4100 {
		t.Fatalf("the cut would land inside the same-nanosecond group: the whole band is kept instead: lines=%v band=%d", outs["poll_loki"]["lines"], len(pq["band"].([]any)))
	}
	outs2 := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000))
	if outs2["poll_loki"]["lines"].(float64) != 0 {
		t.Fatalf("tick 2 must count nothing a second time: %v", outs2["poll_loki"]["per_query"])
	}
}

// TestProdWatch_LokiZeroWidthWindowKeepsTheBand: a zero-width window on a
// tick with a cursor (the max window floored at `to`) reads nothing and
// keeps the band; the next full window does not count the lines again.
func TestProdWatch_LokiZeroWidthWindowKeepsTheBand(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, lokiOnly(1000, 60))
	var lines []pwLine
	for i := 0; i < 5; i++ {
		lines = append(lines, pwLine{TS: nsAgo(30*time.Second) + int64(i)*int64(time.Second), Line: fmt.Sprintf("ERROR boom %d", i), Container: "api", Q: "errors-q"})
	}
	h.lines.Store(lines)
	if outs := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000)); outs["poll_loki"]["lines"].(float64) != 5 {
		t.Fatalf("tick 1 reads the five lines: %v", outs["poll_loki"])
	}
	outs := h.cursorTick(t, wf, cursorVars(h, 0, 0, 5000))
	pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
	if pq["error"] != "" || outs["poll_loki"]["lines"].(float64) != 0 || len(pq["band"].([]any)) != 5 {
		t.Fatalf("a zero-width window reads nothing and keeps the band: %v", pq)
	}
	if outs3 := h.cursorTick(t, wf, cursorVars(h, 60, 0, 5000)); outs3["poll_loki"]["lines"].(float64) != 0 {
		t.Fatalf("the full window after a zero-width one must not count the lines again: %v", outs3["poll_loki"]["per_query"])
	}
}

// TestProdWatch_CoverageNoteCarriesTheLaneErrorAndRefiresOnChange: a
// query that fails is quoted under the partial-coverage note, and the
// note fires again when the KIND of failure changes — not every tick, and
// not never: a query dark for good says why, in the channel.
func TestProdWatch_CoverageNoteCarriesTheLaneErrorAndRefiresOnChange(t *testing.T) {
	t.Parallel()
	wf := compileFixture(t, "prod-watch/main.bot")
	h := newPWHarness(t)
	h.writeConfig(t, lokiOnly(1000, 60))
	h.lines.Store([]pwLine{{TS: nsAgo(30 * time.Second), Line: "ERROR once", Container: "api", Q: "errors-q"}})
	vars := cursorVars(h, 60, 0, 5000)
	secrets := map[string]string{"grafana_token": h.tokenFile, "webhooks": h.webhooksFile}
	tick := func(label string, breakWindow func(w map[string]any)) (map[string]any, map[string]any) {
		t.Helper()
		run := func(id string, inputs map[string]any) map[string]any {
			t.Helper()
			out, stderr, err := runPyWhole(t, h.ws, pwSub(t, pwTool(t, wf, id).Script, inputs, vars, secrets))
			if err != nil {
				t.Fatalf("%s: %s failed: %v\nstderr: %s", label, id, err, stderr)
			}
			return out
		}
		plan := run("plan", nil)
		if breakWindow != nil {
			breakWindow(plan["loki"].(map[string]any)["windows"].(map[string]any)["errors"].(map[string]any))
		}
		loki := run("poll_loki", map[string]any{"grafana": plan["grafana"], "loki": plan["loki"], "timeout_secs": 5, "scratch_dir": h.scratch, "allow_private": true})
		leak := run("leak_scan", map[string]any{"raw_file": loki["raw_file"], "per_query": loki["per_query"], "app": plan["app"], "scratch_dir": h.scratch})
		decide := run("decide", map[string]any{
			"signals_file": leak["signals_file"], "prom_results": []any{}, "http_results": []any{},
			"loki_ok": loki["ok"], "loki_truncated": loki["truncated"], "loki_errors": loki["errors"], "loki_per_query": loki["per_query"],
		...
		_ = os.MkdirAll(filepath.Join(h.ws, ".prod-watch"), 0o755)
		_ = os.WriteFile(filepath.Join(h.ws, ".prod-watch", "state.json"), b, 0o644)
		return decide, notify
	}
	...
	tick("tick 1", nil) // a healthy first tick establishes the cursor
	farAhead := func(w map[string]any) {
		to, _ := strconv.ParseInt(w["to_ns"].(string), 10, 64)
		w["from_ns"] = strconv.FormatInt(to+2*int64(time.Hour), 10)
	}
	decide, notify := tick("tick 2", farAhead)
	if ok, errs := coverageNote(decide); !ok || !strings.Contains(errs, "inverted window") || !strings.Contains(fmt.Sprint(notify["messages"]), "inverted window") {
		t.Fatalf("the failing query is quoted under the coverage note, in the channel: %v / %v", decide["stale_sources"], notify["messages"])
	}
	if decide, _ = tick("tick 3", farAhead); func() bool { ok, _ := coverageNote(decide); return ok }() {
		t.Fatalf("the same kind of failure again must not repeat the note: %v", decide["stale_sources"])
	}
	decide, _ = tick("tick 4", func(w map[string]any) { w["from_ns"] = "0" })
	if ok, errs := coverageNote(decide); !ok || !strings.Contains(errs, "missing window") {
		t.Fatalf("a different kind of failure fires the note again with its reason: %v", decide["stale_sources"])
	}
}
...
func TestProdWatch_LokiExactlyOnceUnderRandomizedIngestion(t *testing.T) {
	...
	wf := compileFixture(t, "prod-watch/main.bot")
	seeds := 5
	if v, err := strconv.Atoi(os.Getenv("PW_SEEDS")); err == nil && v > 0 {
		seeds = v
	}
	regimes := []struct {
		name     string
		maxLines []int
		lags     []int
		churn    bool // overlap / page / max_lines redrawn every tick
		failures bool // one of the first three Loki calls of a tick answers 500
		burst    bool // 4500 lines injected at once inside the overlap
	}{
		{"no-truncation", []int{5000}, []int{0}, false, false, false},
		{"truncation", []int{3, 7, 20}, []int{0}, false, false, false},
		...
		{"churn-lag-and-failures", []int{3, 7, 20}, []int{0, 1, 2}, true, true, false},
	...
	for _, rg := range regimes {
		for seed := 0; seed < seeds; seed++ {
			...
			t.Run(fmt.Sprintf("%s/seed%d", rg.name, seed), func(t *testing.T) {
				t.Parallel()
				rnd := rand.New(rand.NewSource(int64(seed)*7919 + int64(len(rg.name))))
				h := newPWHarness(t)
				overlap := []int{0, 1, 5, 60}[rnd.Intn(4)]
				page := []int{1, 2, 3, 7, 1000}[rnd.Intn(5)]
				if rg.burst {
					overlap, page = 300, 1000
				}
				configure := func() {
					if rg.failures {
						h.writeConfig(t, lokiTwoQueries(page, overlap))
					} else {
						h.writeConfig(t, lokiOnly(page, overlap))
					}
				}
				configure()
				...
				inject := func(n, tick int) {
					for i := 0; i < n; i++ {
						var ts int64
						switch rnd.Intn(3) {
						case 0: // fresh, within the last two minutes
							ts = now - int64(rnd.Intn(120))*int64(time.Second) - int64(rnd.Intn(1000))*int64(time.Millisecond)
						case 1: // late, five to eight minutes old (maybe below the overlap: then lost by design)
							ts = now - int64(300+rnd.Intn(200))*int64(time.Second)
						default: // shares a nanosecond with an existing line
							if len(lines) > 0 {
								ts = lines[rnd.Intn(len(lines))].TS
							} else {
								ts = now - int64(rnd.Intn(60))*int64(time.Second)
							}
						}
				...
				tick := func(k, maxLines, lag int) {
					...
					pq := outs["poll_loki"]["per_query"].(map[string]any)["errors"].(map[string]any)
					if pq["error"] != "" && !failing {
						t.Fatalf("tick %d (max_lines=%d lag=%d overlap=%d page=%d): %v", k, maxLines, lag, overlap, page, pq["error"])
					}
					...
					if os.Getenv("PW_DEBUG") != "" {
						t.Logf("tick %d max_lines=%d lag=%d overlap=%d page=%d failing=%v | from=%v to=%v lines=%v trunc=%v gap=%v err=%q covered=%v frontier=%v ofrom=%v band=%d | now-from=%.1fs now-to=%.1fs",
							k, maxLines, lag, overlap, page, failing, pq["from_ns"], pq["to_ns"], pq["lines"], pq["truncated"], pq["gap"], pq["error"], pq["covered_to_ns"], pq["frontier_ns"], pq["overlap_from_ns"], len(pq["band"].([]any)),
							float64(time.Now().UnixNano()-from)/1e9, float64(time.Now().UnixNano()-to)/1e9)
					}
					if c < hwm {
						t.Fatalf("tick %d: the high-water mark moved backwards: %d < %d (max_lines=%d lag=%d overlap=%d page=%d)", k, c, hwm, maxLines, lag, overlap, page)
					}
					hwm = c
				}
				for k := 0; k < 10; k++ {
...
func TestProdWatch_DecideLifecycle(t *testing.T) {
	...
	base := func() map[string]any {
		return map[string]any{"version": 1, "generation": 7, "cursors": map[string]any{"loki": map[string]any{"errors": map[string]any{"covered_to_ns": "111", "frontier_ns": "111", "band": []any{}, "overlap_from_ns": "0"}}}, "incidents": map[string]any{}, "health": map[string]any{}}
	}
	...
	t.Run("a failed query keeps its cursor and its band; the zero-façade guard refuses an all-dead tick", func(t *testing.T) {
		h := newPWHarness(t)
		// poll_loki hands back the previous mark untouched and a band that
		// knows the lines written before the failure; decide persists both.
		out, _, err := pwDecide(t, wf, h, map[string]any{"templates": []any{}, "leak": []any{}}, base(), map[string]any{
			...
			"loki_per_query": map[string]any{"errors": map[string]any{"lines": 2, "error": "boom", "covered_to_ns": "111", "frontier_ns": "111",
				"band": []any{"0:deadbeefdeadbeef", "5:cafebabecafebabe"}, "band_base_ns": "111", "overlap_from_ns": "100"}}})
		if err != nil {
			t.Fatal(err)
		}
		cur := pwStateNext(t, out)["cursors"].(map[string]any)["loki"].(map[string]any)["errors"].(map[string]any)
		if cur["covered_to_ns"] != "111" || len(cur["band"].([]any)) != 2 || cur["overlap_from_ns"] != "100" {
			t.Fatalf("a failed query keeps its mark and the band handed back: %v", cur)
		}
	...
	t.Run("a log template's first-seen renders as a date, not a nanosecond epoch", func(t *testing.T) {
		h := newPWHarness(t)
		sig := map[string]any{"templates": []map[string]any{{"template_id": "t1", "query": "errors", "template": "ERROR job # failed", "count": 3, "count_live": 3,
