...
// exercise a red verify pass or the MR path. toStr is shared with
// whole_improve_loop_test.go (same package).
...
func TestBranchImproveLoop_Structural(t *testing.T) {
	...
	if wf.Entry != "campaign" {
		t.Errorf("workflow entry = %q, want %q (the mission starts working immediately — no blocking upfront chunk scan)", wf.Entry, "campaign")
	}
