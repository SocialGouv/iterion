...
246 packages, excluding vendored and generated trees. The third column lists exported **interfaces** — the seams a new variant plugs into rather than branching the core.
...
| `pkg/bundle` | implements the `.botz` archive format: a ZIP archive that packages an iterion workflow (`main.bot`) with adjacent resources (skills, prompts,… | — | 23 · 174 |
...
| `pkg/dsl/workflowfile` | the single source of truth for which file extensions iterion recognises as workflow source files. | — | 1 · 8 |
