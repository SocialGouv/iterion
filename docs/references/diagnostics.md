...
| **C129** | error | `file` field outside a human pause | A schema used as the `output:` of a node that never pauses for an operator declares a `file`-typed field — any non-human node, or a `human` node with `interaction: llm` (auto-answered by a model, which can only invent a path). Only an operator upload at a real pause produces one | Move the `file` field to a human node's `output:` schema with `interaction: human` (or `llm_or_human`, which can escalate to a pause), or use `string` if the node computes a path itself |
