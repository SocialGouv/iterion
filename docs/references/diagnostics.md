...
| **C135** | error | Unknown tool name | An agent/judge `tools:` entry (or a Verified Action's `recovery: agent_tools:`) names a bare tool the registry cannot resolve, on a backend where the list is a real constraint — `claw`, whether as the node's own backend or as a `fallbacks:` route. The run would die on `unknown tool "..."` the moment the node dispatches. CLI backends run their own native toolset and ignore the list, so nothing is reported there; `mcp.<server>.<tool>` names, the `mcp__server__tool` alias form, wildcards and `${VAR}` refs resolve at run time and are never flagged | Use a claw built-in (`read_file`, `write_file`, `file_edit`, `glob`, `grep`, `bash`, `web_fetch`, …) — the diagnostic names the nearest match; for an MCP tool use its `mcp.<server>.<tool>` name |
