...
| **C136** | warning | Gated CLI backend needs a host-side run | A node routes `permission: ask\|deny` to `grok` or `kimi` (primary or fallback) on a workflow that has not opted out of the sandbox. Those two enforce the gate through a **host-side** `PreToolUse` hook — the CLI home and the hook binary live on the host — so a sandboxed run cannot reach it and the node is refused at execution time. Since the shipped default is `sandbox: auto`, the common shape (no `sandbox:` block) hits this every time | Declare `sandbox: none` on the workflow (or the node), or launch with `--sandbox none` / `ITERION_SANDBOX_DEFAULT=none`. A warning rather than an error because those run-time overrides make the workflow legal without it saying anything |
