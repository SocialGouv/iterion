...
| **C129** | error | `file` field outside a human node | A schema used as a non-human node's `output:` declares a `file`-typed field. Only an operator upload at a human pause produces one, so the field would always arrive empty | Move the `file` field to a human node's `output:` schema, or use `string` if the node computes a path itself |
