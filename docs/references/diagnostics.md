...
| **C135** | error (warning when the workflow wires its own `mcp_server:` blocks) | Unknown tool name | An agent/judge `tools:` entry (or a Verified Action's `recovery: agent_tools:`) names a bare tool the registry cannot resolve, on a backend where the list is a real constraint — `claw`, whether as the node's own backend or as a `fallbacks:` route. The run would die on `unknown tool "…"` the moment the node dispatches. CLI backends run their own native toolset and ignore the list, so nothing is reported there. Never flagged: qualified MCP references (`mcp.<server>.<tool>`, the `mcp__server__tool` alias, wildcards), and iterion's own board/watch tools by their bare name (`create_issue`, `subscribe`, …), which `Registry.Resolve` reaches through its shorthand path. Where the workflow wires MCP servers of its own, a bare name may resolve the same way over tool lists that only exist once the server connects, so the finding degrades to a warning | Use a claw built-in (`read_file`, `write_file`, `file_edit`, `glob`, `grep`, `bash`, `web_fetch`, …) — the diagnostic names the nearest match; for an MCP tool use its `mcp.<server>.<tool>` name. `tools:` is not environment-expanded, so name the tool literally |
...
| **C244** | error | Loop/foreach in parallel-branch body | A bounded-iteration edge (`as <loop>(…)` or `as foreach`) leaves a node inside a `fan_out_all`, `fan_out_each`, or `llm` `multi: true` body, or re-enters a body node from the join — those subgraphs run through `execBranch`, which has no local loop counters. On a multi-edge fan the walk stops at structural joins, not at a loop head elected only by its own back-edge (that graph mis-executes the sibling join). On `fan_out_each` that election is a real join. A loop after a non-elected `await:` in a sibling branch is not C244 | Move the loop to the trunk (after the join, or wrapping the *router* from the join), or run the per-item retry as a `subbot` |
