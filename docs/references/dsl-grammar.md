...
Use `iterion validate file.bot`. The authoritative sparse code ranges are DSL C001–C199 and C240–C248 (async interaction, `session: persist` scope, parallel-branch ownership, typed `fail` codes) and bundle checks C200–C234; see [diagnostics](diagnostics.md).
