...
- `opencode` — the [opencode](https://opencode.ai) agent CLI through the same CLI-agent seam (ADR-065, [pkg/backend/delegate/opencode.go](../../pkg/backend/delegate/opencode.go)): `backend: "opencode"`, headless via the **`run` subcommand**, prompt on **stdin** (its message is a variadic positional, which swallows a prompt starting with `-` — the same trap pi documents), `--format json`, model via `-m provider/model` **passed through unchanged** (its own format is iterion's spec shape), `reasoning_effort` via `--variant` (accepted names are per-model; iterion passes the level verbatim). Credentials come from the CLI itself. **Capability gaps that are easy to miss:** it cannot enforce iterion's permission gate in ANY mode — no `PreToolUse` hook — so a gated node is refused by C176 at compile and again at dispatch; `interaction: async` is refused by C267; session resume/fork, MCP forwarding and `tools:` narrowing are not wired; `command:` (C174) and `provider:` (C088) are inert. **Security:** opencode executes the workspace's `.opencode/plugin[s]/*.ts` inside the agent process, so iterion refuses a workspace carrying one unless `ITERION_OPENCODE_TRUST_PROJECT=1`, and pins `OPENCODE_DISABLE_CLAUDE_CODE=1` so a node's prompt never depends on the operator's `~/.claude`. Not baked into the stock sandbox image.
...
Per-run container isolation is **on by default**: at product entry points (`iterion run`/`resume`, studio, dispatcher) a workflow with no `sandbox:` block runs as `sandbox: auto` (reads `.devcontainer/devcontainer.json`, falling back to a published `iterion-sandbox-slim:<version>` image). Workflows can still pin block-form inline configuration (`sandbox:` with `image:` or `build:`) or explicitly opt out via `sandbox: none` — discouraged and flagged by the C128 warning. The two branches carry different guarantees when the host cannot sandbox (no docker/podman on PATH — the 107 scheduled ticks that dropped on the operator's own host, #1425):

- **`sandbox: auto`** ("isolate if you can") — degrades to unsandboxed with a visible `sandbox_skipped` event on the run's stream when the host has no container runtime (and, for a library caller that resolves neither a repo root nor a default image, on those too — `iterion run`, studio and the runner always resolve both). The workflow runs, and the run record says how — including `file_secrets_dropped: true` when it declares `as: file` secrets, which only a container mount (or the cloud runner's no-sandbox path) can deliver. The MODE decides this, never the tier that named it: workflow block, `--sandbox=auto` and the built-in default get one answer.
- **`sandbox: { mode: inline, image/build: … }`** (an explicit container the author wired) — the refusal is a guarantee. The run parks with typed `FailureCode SANDBOX_DRIVER_UNAVAILABLE` (`store.FailureSandboxDriverUnavailable`, wrapping `sandbox.ErrDriverUnavailable`); the run document carries it as `failure_code` and its error text, and the schedule record's `last_run_error_code` copies it, and `pkg/retrypolicy` classes it Deterministic so the runner acks instead of redelivering into the same absent runtime. To run such a workflow on a host without a driver, say so: `--sandbox none`. `ITERION_SANDBOX_DEFAULT=none` restores the historical opt-in behaviour machine-wide; the cloud runner was long assumed to pin `ITERION_SANDBOX_OVERRIDE=none` (the runner pod being the isolation boundary), but the production deployment measured on 2026-08-05 does NOT: its config carries `ITERION_SANDBOX_DEFAULT=auto` with an EMPTY override, so cloud runs DO get the k8s sandbox. Anything that needs a bind-mounted workspace there — auto-memory, for one — must declare `sandbox: none`. When active, claw, claude_code, pi, Kimi, Grok, and tool nodes execute against a long-lived container that bind-mounts the worktree — by default at the host workspace's absolute path so Claude Code project keys match in/out container. Codex is the exception: its pinned SDK refuses Iterion's outer sandbox because it cannot route through the command builder. Network egress is **unrestricted by default** (`network: open`, since 2026-05-22 — no proxy is started). Opting into `network: allowlist` (or `denylist`) starts an HTTP CONNECT proxy on the host that enforces the policy; the built-in `iterion-default` preset covers LLM endpoints + npm/pypi/golang + github/gitlab/bitbucket + Nix cache. Sandboxed `claw` calls are routed through the hidden `iterion __claw-runner` subprocess inside the container, so the `iterion` binary must be present on the container PATH (or bind-mounted by the host when available).
