...
|---|---|
| Run lifecycle/control | `run_started`, `run_paused`, `human_input_requested`, `human_answers_recorded`, `interaction_answered`, `run_resumed`, `run_auto_resumed`, `run_retry_scheduled`, `run_retry_skipped`, `run_workspace_reset`, `run_workspace_bank_restored`, `run_bank_attempt`, `run_bank_refused`, `run_bank_superseded`, `run_redelivery_deferred`, `run_delivery_exhausted`, `run_steered`, `run_rewound`, `run_health`, `run_finished`, `run_failed`, `run_cancelled`, `run_interrupted` |
...
| Graph/budget/artifacts | `branch_started`, `branch_finished`, `branch_abandoned`, `node_started`, `node_recovery`, `node_verified_action`, `node_finished`, `edge_selected`, `join_ready`, `budget_warning`, `budget_exceeded`, `budget_exit_grace`, `usage_progress`, `usage_cap`, `artifact_written`, `run_attachment_published`, `plan_written` |
| LLM, delegation, and tools | `llm_request`, `llm_prompt`, `llm_retry`, `llm_step_finished`, `assistant_text`, `llm_compacted`, `tool_started`, `tool_called`, `tool_error`, `delegate_started`, `delegate_finished`, `delegate_error`, `delegate_retry`, `delegate_stall`, `model_fallback`, `model_drift`, `session_degraded`, `persist_session_degraded`, `mcp_server_degraded` |
| Review gate | `review_turn`, `review_verdict`, `review_merged` |
| Sandbox/network | `sandbox_skipped`, `sandbox_started`, `sandbox_shared`, `sandbox_claw_routed_via_runner`, `sandbox_host_state_mounted`, `sandbox_user_remap`, `sandbox_uid_mismatch_warning`, `sandbox_devbox_provisioned`, `sandbox_workspace_export_failed`, `network_blocked`, `sandbox_build_started`, `sandbox_build_finished`, `sandbox_build_failed` |
...
The three **death-bank** events are the only durable record of git refs the
run document deliberately does not name. `run_bank_attempt` parks an
attempt's work on its own `iterion/run-<id>-parked-<sha12>` ref instead of
the storage branch — an interrupted delivery, a paused run, or a death whose
lease was already lost — and leaves `final_branch` / `final_commit` /
`final_branch_error` unset on purpose, so a half-done run never reads as
merge-eligible; the event carries `ref`/`head` (success) or `error`
(failure), exactly one of the two. `run_bank_refused` marks THIS attempt's
head being dropped while an earlier attempt keeps the branch — because that
chain was strictly richer, the workspace failed its integrity check, or the
push failed — which is otherwise invisible outside the pod log, since
`final_branch`/`final_commit` keep naming a valid but *different* attempt's
pair. `run_bank_superseded` marks a finished outcome force-taking the branch
from a dead attempt whose chain it does not contain, naming the
`iterion/run-<id>-attempt-<sha12>` archive that now holds the dropped chain
(or `archive_error` when it could not be archived). The `-parked-` and
`-attempt-` infixes differ so a pruning policy can tell a live run's parked
work from a dead attempt's archive by ref name alone.

`run_rewound` is the audit trail of an in-place rewind: `events.jsonl` is
append-only, so the superseded `node_started` / `node_finished` records of
the invalidated nodes stay in the timeline and this marker explains why they
are about to repeat. Its `data` names `from_node` / `to_node`,
`dropped_nodes`, `tombstoned_artifacts` (published artifacts superseded by a
`rewound` marker version), `orphaned_child_runs`, and the workspace half —
`files_reverted`, `files_restore_scope`, `files_skip_reason`, and the
`files_overwritten` / `files_left_in_place` pairs that say what the restore
took or left behind. A rewind driven over the API or by an agent never sees
the CLI's stderr, so without those the record could not answer what the
rewind took. Distinct from a fork, which mints a new run id and leaves this
run untouched.
