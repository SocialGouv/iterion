...
| `pool` | `status · history · share · pause · resume · withdraw · donors · policy` — lend your own LLM subscription or personal metered key to the shared [credential pool](credential-pool.md), bounded by ceilings you set on `share` (`--max-usd-day/-week`, `--max-runs-day`, `--max-concurrent`, `--from-hour/--to-hour`, `--bots`). `donors` + `policy` are the operator half (org-admin): `donors` reads the pool and its lenders, `policy` creates or changes it — `--enabled` is the master switch (a pool being created comes up on), and `--all-teams`/`--orgs`/`--teams`/`--contributors` set the audience, which is replaced WHOLE on every call. |
| `server` | `info · health` |
