...
| `admin` | `orgs · users · dlq · llm · caps · usage-readings · bots · roles · sandbox · vars` (super-admin; `llm api-keys`/`llm oauth` = the platform fallback credentials — rotate without a redeploy, see [cloud-llm-credentials.md](cloud-llm-credentials.md); `caps` = the runtime usage-cap percentages — retune without a restart, and `usage-readings clear <fingerprint>` forgets ONE credential's stored provider-window readings when the provider reset a window early and the stale reading is refusing every run pre-flight, see [usage-caps.md](usage-caps.md#changing-the-caps-at-runtime-no-restart); `bots` = platform bot overrides — push any bot without an image rollout, `roles`/`sandbox` = runtime webhook role bindings + `sandbox: auto` image, `vars` = DB-backed overrides for a bot's `${ITERION_X:-default}` expansions (precedence: setting > pod env > the `.bot`'s own default; infra namespaces and credential-shaped names refused at write, values stored and audit-logged **in clear** — never a secret), see [platform-bots.md](platform-bots.md)) |
