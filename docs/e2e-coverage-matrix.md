...
| studio-ui.launch-modal | Launch modal: bot picker, vars, overrides, target repo | studio-ui | covered-deterministic | studio/e2e/specs/launch.spec.ts | Playwright: `?bot=` resolves through the catalog to the workflow, the `vars:` block splits into required primaries vs defaulted Bot options, missing-required validation blocks the launch, and Engine options offers the workflow's own agent node for model retargeting. The "target repo" attach needs a forge connection (covered at REST level by the forge rows). The same spec carries a tripwire test (`KNOWN BUG — Launch view crashes for a workflow with no LLM nodes`) pinning a REAL BUG it found: `preview-cost` answers `nodes: null` for a workflow with NO agent/judge node and CostPreviewChip dereferences it, so the view drops into its error boundary. It asserts the defect, so it goes RED the day the crash is fixed |
