...
| cli.run-budget-override | `iterion run --max-*` re-budgets the workflow for this run | cli | uncovered | | ir.ApplyBudgetOverrides is unit-covered; nothing asserts the CLI actually applies it to a real run. Plan: same `.bot`, once with and once without a lowered cap |
...
| cli.report | `iterion report` renders a run's chronological markdown report | cli | uncovered | | only the `verify:` header special case is tested (pkg/cli/report_test.go). Plan: execute a run, then assert the written report.md carries its nodes, artifacts and outcome |
| cli.diagram | `iterion diagram` emits a Mermaid graph for a `.bot` | cli | uncovered | | only the not-found path is tested (TestDiagram_FileNotFound); mermaid rendering itself is unit-covered by pkg/dsl/ir/mermaid_test.go. Plan: assert nodes + edges of a real fixture reach the CLI output |
...
| cli.runs-async-questions | `iterion runs questions` / `runs answer` drive an async question to delivery | cli | uncovered | | cli.RunQuestions / cli.RunAnswer have NO test at all. Plan: list a pending question, answer it, assert the queued node-scoped delivery and that a parked await_answers gate releases |
...
| cli.issue | `iterion issue create/list/show/move/update/close/board` | cli | uncovered | | the native board store + HTTP surface are covered (pkg/dispatcher/native); the `iterion issue` command wrapper is not. Plan: create → move → close against a temp board |
...
| observability.report-generation | `report.md` chronological rendering from events + artifacts | observability | uncovered | | see cli.report — the same gap seen from the observability side |
