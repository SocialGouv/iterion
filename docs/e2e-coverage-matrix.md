...
| studio-ui.launch-modal | Launch modal: bot picker, vars, overrides, target repo | studio-ui | covered-deterministic | studio/e2e/specs/launch.spec.ts | Playwright: `?bot=` resolves through the catalog to the workflow, the `vars:` block splits into required primaries vs defaulted Bot options, missing-required validation blocks the launch, and Engine options offers the workflow's own agent node for model retargeting. The "target repo" attach needs a forge connection (covered at REST level by the forge rows). The same spec carries a `test.fail` reproducing a REAL BUG it found: the Launch view crashes for a workflow with NO LLM node — `preview-cost` returns `nodes: null` and CostPreviewChip dereferences it |
