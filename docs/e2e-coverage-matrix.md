...
| cli.dispatch | `iterion dispatch` daemon boots from a config with the bot catalogue | cli | uncovered | | config building + catalogue extraction covered (TestBuildDefaultConfig_ValidatesAndExtractsCatalogue); the daemon boot loop itself (config → dispatcher actor → serving) is the gap — plan: in-process boot with a temp store + immediate shutdown |
