...
- Neither form may originate inside a `fan_out_all`, `fan_out_each`, or llm `multi: true` body (**C244**): parallel branches have no local loop counters. Put a per-item retry in a `subbot`, or wrap the router from the join.
