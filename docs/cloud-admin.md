...
| Anthropic calls fail with `refusing to spend a subscription OAuth token outside the vendor's own CLI` | The runner has `ITERION_FORBID_SUBSCRIPTION_OAUTH=1` and a `claw` or `pi` node would use a subscription token | Keep the guard and configure tenant BYOK / switch to `claude_code`, or deliberately remove the guard after accepting Anthropic's separate extra-usage billing |
| Anthropic reports that the subscription extra-usage balance is empty | A permitted `claw` or `pi` call used subscription OAuth after its separate extra-usage balance was exhausted | Replenish/enable extra usage, configure BYOK, or route the node through `claude_code` to use plan limits |
