...
|---|---|---|
| Studio blank; console shows `/assets/*.js` blocked — as a disallowed MIME type (`text/html`) on replicas before v3.146.7, as a 404 after | Replicas are serving different builds — `image.tag` is a moving tag and a scale-up pulled a newer one. A mixed fleet shows both symptoms at once | Pin `image.digest` (§4). Confirm first: `kubectl -n <ns> get pods -l app.kubernetes.io/component=server -o custom-columns=NAME:.metadata.name,IMAGEID:.status.containerStatuses[0].imageID` — more than one digest across those pods means this. (`<none>` is a pod still pulling, not a second build) |
