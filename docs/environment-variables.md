...
| `ITERION_PI_ALLOW_ANTHROPIC_OAUTH` | `1` lets pi drive Anthropic with a Claude Pro/Max subscription OAuth token: disables both the refusal and the environment strip. Anthropic bills third-party apps against a **separate extra-usage balance**, not your plan limits ([backends.md](backends.md#subscription-credentials)). Local development only — never set it in a shared or cloud runner, where it becomes a decision taken for every user of that deployment. | refused |
