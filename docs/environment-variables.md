...
| `ITERION_OPENCODE_BIN` | Absolute path to the opencode CLI on the **host**, for a host whose PATH the iterion process does not share. Detection and execution read the same variable; a relative value is refused. Ignored inside a sandbox, where a host path means nothing. | PATH lookup |
