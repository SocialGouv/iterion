...
| `ITERION_RUNNER_SCHEMA_MISMATCH_DELAY` | Redelivery delay a runner applies when it rejects a queue message whose schema version it does not speak (mixed fleet during a rolling schema bump). Must cover a rolling restart of the runner Deployment — raise it on fleets with a slow lame-duck turnover. **Set it on the server pods too**: the server derives the orphan sweeper's queued-staleness cutoff from `RedeliveryWindow()` (`MaxDeliver × max(AckWait, this delay)`), so a value raised only on runners lets the sweeper flip a still-bouncing `queued` run to `failed_resumable`. The shipped Helm chart puts it in the shared ConfigMap, so both Deployments already read it. See [cloud-queue-schema-rollout.md](cloud-queue-schema-rollout.md). | `30s` |
...
| `ITERION_LOCK_TTL` | Cloud: TTL of the per-run NATS-KV lease that makes a claim exclusive. A runner self-cancels rather than split-brain when a refresh fails, so this is also how long a dead pod's run stays unclaimable. Must be > 0. Set the SAME value on server and runner — the orphan sweeper reads the same bucket. | `60s` |
| `ITERION_HEARTBEAT_INTERVAL` | Cloud: how often the runner refreshes that lease (and pings the JetStream delivery, so a long LLM step does not trip `AckWait`). Keep it well under `ITERION_LOCK_TTL` — the shipped 20s/60s pair tolerates two missed refreshes. Must be > 0. | `20s` |
| `ITERION_RUNNER_CONCURRENCY` | Cloud: runs one runner pod executes at once. Horizontal scale is "more pods", not a higher value here. Must be ≥ 1. | `1` |
| `ITERION_RUNNER_WORKDIR` | Cloud: where a runner clones the target repo and stages a run's workspace. | `/tmp/iterion` |
| `ITERION_RUNNER_GIT_TIMEOUT` | Cloud: bounds ONE runner-side git subprocess (clone, fetch, push). Without it a wedged remote pins the pod for the whole run — the heartbeat proves the pod lives, not that the clone progresses. `0` or negative disables the bound. | `15m` |
| `ITERION_FORFAIT_CAP_PCT` | Utilization percentage at or above which `--auto-resume` (and `iterion schedule`'s retry) **withholds** a retry that would spend an Anthropic subscription, read from the provider's own `/api/oauth/usage`. A missing token or unreachable endpoint is a best-effort skip. `0` or negative disables it; a malformed value keeps the default. **Not the same knob as `ITERION_USAGE_CAP_*`** ([usage-caps.md](usage-caps.md)): those stop a *run* off the provider's per-call telemetry, this one withholds an *auto-retry* off a different endpoint, and it is on by default. | `85` |
