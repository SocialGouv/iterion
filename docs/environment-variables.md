...
| `ITERION_PI_MODE` | Selects the `pi` transport. `rpc` uses the long-lived `--mode rpc` session — tool events reach the studio timeline, operator chat is delivered by pi's native `steer`, accounting comes from `get_session_stats`, and a pre-flight handshake resolves the model before any token is spent. Anything else (the default) uses the one-shot `--mode json` path. | print |
