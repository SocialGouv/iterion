...
| `ITERION_RETRY_CIRCUIT_THRESHOLD` | Failures across the runs sharing one breaker key — `workflow:<hash>`, falling back to `workflow-name:<name>` for a legacy run with no hash — before the **shared retry circuit** opens. While it is open the next retry wave waits out the cooldown instead of starting one pod per run against the same provider wall; a run that succeeds clears the streak. Durable breaker state is an *optional* store capability (`store.AsRetryCircuitStore`): the cloud Mongo store implements it, a local filesystem store does not and keeps its existing per-run retry behaviour untouched. A value that is not an integer `> 0` leaves the default in place. | `3` |
| `ITERION_RETRY_CIRCUIT_COOLDOWN` | Minimum quiet period (Go duration) once that circuit is open. A value that is not a positive duration leaves the default in place. | `15m` |
...
| `ITERION_WORKSPACE_CHECKPOINT` | Env level of the `workspace_checkpoint:` chain (workflow block → this → default `on`); `on`/`off`. Turns off the mid-run net that force-pushes a copy-based sandbox's tree as `iterion/run-<id>-checkpoint` onto the run's own remote — the right setting for a bot that reads without committing, since the push runs `git add -A` and carries its scratch files with it. An invalid value *in the workflow block* is diagnostic **C139**, never a silent fall back to the default. See [dsl.md](dsl.md#the-mid-run-safety-net--workspace_checkpoint). | `on` |
...
Read by the `iterion-desktop` launcher only, before it starts the embedded
studio; the `iterion` CLI ignores them.

| Variable | Effect | Default |
|---|---|---|
| `ITERION_ENV_FILE` | Explicit path to the dotenv file the desktop app sources at startup. It is the first entry in the lookup order (`cmd/iterion-desktop/env_file.go`), ahead of `$ITERION_HOME/env` when `ITERION_HOME` is set, then `~/.iterion/env`, then `~/.iterion/.env` — point it at a file outside the project when provider keys live in a shared location. The first file that loads wins and a missing one is not an error, the lookup just moves on. A variable already set by the launching shell is never overwritten, so `OPENAI_API_KEY=… iterion-desktop` still works as a one-shot override. | unset (default lookup order) |
