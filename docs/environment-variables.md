...
| `ITERION_OUTCOME_ROUTER` | `on` — matched exactly, any other value is off — activates the **outcome router** on the **server** deployment: a terminal run carrying a launch-frozen `routing_policy` is decided by its own contract (merge, relaunch or escalate) once per terminal episode, against a durable decision registry. Fleet-global on purpose: it rides the Deployment manifest, so every replica reads the same value and flipping it is an explicit rollout rather than a per-pod accident. The first replica to start with it on pins an activation watermark, so turning it on does **not** retro-route a lookback's worth of historical terminals. Read [outcome-router.md](outcome-router.md) before flipping it. | off |
