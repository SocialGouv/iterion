...
| `grok` | opt-in | Runs xAI's `grok` Build CLI through the same CLI-agent seam (`grok -p <prompt> --output-format json [-m <model>]`); `system:` appends via `--rules`, `reasoning_effort` maps to `--reasoning-effort`, headless approval forced with `--permission-mode bypassPermissions --always-approve`. Credentials come from the CLI's own Grok Build login (`~/.grok`). Distinct from the metered xAI HTTP path (`backend: claw` + `model: "xai/…"`). See [Backends → Third-party agent CLIs](backends.md#third-party-agent-clis-kimi-grok-and-the-cli-agent-seam) and [ADR-065](adr/065-dedicated-cli-agent-backend.md). |
