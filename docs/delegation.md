...
| `codex` | supported, explicit opt-in | Runs the `codex` CLI through the Codex Agent SDK, using its login, native tool loop and sandbox. Iterion cannot currently narrow its built-in tool set or run it inside the outer Docker/Kubernetes sandbox; see [Backends → codex](backends.md#codex). |
