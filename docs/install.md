...
The published image (`ghcr.io/socialgouv/iterion:latest`) bundles the `iterion` binary, `git`, Node 26 and the pinned `claude` / `codex` CLIs. Override the default `server` command for ad-hoc runs:
