...
What that note does **not** extend is the set of run-scoped executor features a branch node reaches. Session compaction on retry (`session: persist` is already refused by C243), operator/supervisor steering messages, and async questions (`interaction: async`) are all keyed on `(run, node)` with no branch discriminator, while a `fan_out_each` runs N branches concurrently under ONE node id — so they stay **trunk-only**, unchanged. Rendering identically is a statement about templates, not about conversation state: put a node that needs steering or an async question on the trunk, or give the item its own run with a `subbot`.
