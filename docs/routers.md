...
The router itself is a pass-through — it forwards its input unchanged to all targets. The number of concurrent branches is bounded by the `max_parallel_branches` budget setting. For workspace safety, only one **mutating** branch is allowed at a time. A branch is mutating if it contains a **tool node** (always, on a static fan-out — `parallel_safe:` only relaxes it inside a `fan_out_each` template), a **subbot** without `isolated: true`, or an **agent/judge** that is not `readonly:` and holds a non-read-only tool (or runs unrestricted on a CLI backend). Human nodes never count. Read-only branches can run freely in parallel.
