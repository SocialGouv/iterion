...
**Which node is the collector.** A node that declares `await:` is the collector for the branches that reach it. Without the annotation, the engine elects the first node (breadth-first from the router's targets) that has more than one distinct predecessor **inside the fan-out** — the router itself or a node reachable from it, bounded back-edges excluded. A predecessor from outside the fan-out does not count: the mono/dual topology, where a `condition` router reaches the same reviewer directly *or* through a `fan_out_all` router, gives that reviewer two predecessors, and it is still an ordinary branch head, not the collector. Declare `await:` on the intended collector rather than relying on the implicit election.
