...
The same `.bot` file produces the same graph of nodes and edges every time. The compiler and validators under [`pkg/dsl/ir/`](../pkg/dsl/ir/) turn the AST into an IR and emit sparse DSL diagnostic codes in C001–C199 (plus C240–C248) for structural problems *before* you spend a token; bundle checks use C200–C234.
