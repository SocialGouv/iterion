...
| ⚖️ Themis | [`arbitrate`](../bots/arbitrate/) | Judge the divergence cases a modernisation programme leaves blocked, applying the target repository’s own written arbitration doctrine and nothing else; decides, never executes. |
| 🎭 Bmady | [`bmady`](../bots/bmady/) | Human-steered analyst → PM → architect → developer → QA delivery pipeline. |
| 🧭 Campy | [`campaign`](../bots/campaign/) | Supervise a whole modernisation programme lot after lot by running `modernize` as a subbot; deterministic (no LLM node of its own), progress judged by git rather than self-report. |
| 💬 Copi | [`copilot`](../bots/copilot/) | Conversational assistant whose subject is iterion itself — the DSL, the Cxxx diagnostics, run/resume semantics, backends and bundles; read-only, with info / design / debug postures. |
...
| 🕸️ Endy | [`e2e-coverage`](../bots/e2e-coverage/) | Inventory an application’s features from the outside, maintain a committed feature×coverage matrix, and close each gap with a real e2e test in the repo’s own harness. |
...
| 📡 Obsy | [`instrument`](../bots/instrument/) | Wire a repo for error tracking (the Sentry DSN protocol), one central structured-logging seam, and opt-in tracing — one verified commit at a time behind a deterministic build/test gate. |
...
| 🌐 Envy | [`review-env`](../bots/review-env/) | Deploy the workspace’s already-CI-published image to the operator-attached platform and hand back a live https URL — a real review environment, its verdict measured rather than believed. |
...
| 🛡️ Senti | [`vuln-watch`](../bots/vuln-watch/) | Inventory-scoped vulnerability sentinel: zero-LLM hourly watch over Dependabot alerts, advisory feeds and CISA KEV/EPSS, alerting chat only on an exploitation signal. |
