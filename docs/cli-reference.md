...
**Author documents.** `fmt` takes a `.bot.yaml` — the YAML twin of a `.bot` ([dsl.md](dsl.md)) — like a `.bot`: named or met in a walk, it is rewritten in its canonical YAML form, the program it describes written back by the author writer and proven to read as the same program, except that a document carrying YAML comments is refused and left as it is (the writer keeps none, and a rewrite would lose them). `fix` and `dsl migrate` take no document: named one, they say which command writes the `.bot` they work on. `--to bot` writes the `.bot` a named document stands for, beside it (`x.bot.yaml` → `x.bot`), after the proof `validate` reports as E054 when it fails — a document whose program has no written `.bot` form is refused; `--to yaml` writes the document of a named `.bot` (`x.bot` → `x.bot.yaml`) from a text that parses without an error — never from one the parser recovered on, even with `--force` — and says under `note:` what the document does not carry: the frontmatter keys beyond the four of `catalog:`, a frontmatter the catalog reader cannot read, the ordinary comments. Either way a destination already there and not what the source writes is refused without `--force`, one that is already it is a no-op, and `--check` reports what a write would do — the CI form of "is the `.bot` beside the document the one it writes?". `--to` takes named files: a conversion writes beside what it reads, and a walk would write trees. `--json` carries each conversion as a file with `from`, and the notices as `notices`.
