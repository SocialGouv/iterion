...
Accepted inputs are `.bot`, `.botz`, bundle directories — and an author document, `x.bot.yaml`, the YAML twin a `.bot` can be written as ([dsl.md](dsl.md)): it is read into the program it describes — every finding the stage can attribute positioned on the document; a bundle finding carries no position, as for a `.bot` — and validated as its `.bot` would be — `main.bot.yaml` beside an iterion manifest or a `skills/` is that bundle's main, whether or not the `main.bot` is written (a manifest export naming `main.bot` is that main); a document whose program has no written `.bot` form is refused (E054, the cause named), and so is one whose `.bot` path `fmt --to bot` will not write over (a directory, a file that cannot be read), in `fmt`'s words. `--json` says so with `source_kind: "author"` and `bot_path`. `run`, `resume`, the schedules and the MCP run tools refuse a document by name, before any effect; a directory whose only source is a document is a bundle for none of them — the twin is the main of no launch — and `validate <dir>` says so, naming the document. A bare `main.bot` whose parent is a bundle (an iterion `manifest.yaml`/`.yml` — one carrying iterion's own keys, `schema_version` first — or a `skills/` beside it; a `prompts/` alone marks nothing, and neither does a manifest of another tool, a common filename) is validated as that bundle — its `prompts/*.md` in scope, its manifest cross-checked — the same promotion `run` and `resume` apply, so `iterion validate bots/x/main.bot` and `iterion validate bots/x` give one verdict — including on a bundle that does not open (a `manifest.yaml` that does not decode), which both forms refuse by name rather than validating the bare file. Validation reports sparse DSL diagnostics in C001–C199 plus the async-interaction and structural band C240–C249, and bundle checks in C200–C234; the [diagnostic catalogue](references/diagnostics.md) is authoritative.
...
**`--exec` — the dry run.** A program that compiles is then run twice under a simulation — every condition true and every enum at its first value, then the other way — without a model, a shell or the workspace: each node's prompts, `command:`, `script:` and `postcondition:` are rendered by the production renderers and every `{{…}}` left as written is named with the reason (a node that has not produced on this path, a field the input never carries); shell text is held to `bash -n` (`sh -n` for `language: sh`; other interpreters are said unchecked); a human node is answered by a schema-shaped output, a `wait` at once, a `subbot` child read within the bundle's collection and simulated under the same bias (its findings prefixed `node/child_node`). The report — `exec` in `--json`, a block in the human output — lists each pass's status and path, the findings by node, the nodes whose output was a **shape** (a condition read from one decided nothing about the real bot), and the nodes and edges no pass reached. It never runs on a program with a compile error, and its findings do not decide `valid`: they say what the first paid run would have met. `--fixtures f.json` answers the named nodes with recorded outputs — `{node: output}`, or the replay shape, a list of `{"node": …, "output": {…}}` — and reports the nodes without one; it implies `--exec`. `--strict` (implies `--exec`) exits non-zero when the report is **failing** — a pass died, or a reference, shell or fixture finding stands — the switch a CI gate flips; an expression left **inconclusive** on a shape (below) leaves the report not `clean` but not failing: printed, exit 0, so a value the dry run cannot know never blocks a gate. Without `--strict` the exit code is the compiler's. In `--json`, `exec.clean` and `exec.failing` are the report's two words: a job that tolerates undecided expressions reads `failing`, one that does not reads `clean`. `--exec-timeout` bounds one pass, the simulated children included (default one minute); a pass that runs out of time is said so (`timed_out`) — the bound's doing, not the program's — and the report is not clean, since what the pass would have met past that point is unknown. `--var k=v` (repeatable) and `--preset <name>` give the dry run its launch values, with the precedence `run` has: a bot that guards its entry on a var — the gallery's `TAG_UNSET` shape — is otherwise refused at the gate on every pass, and the graph behind it is never walked; either implies `--exec`, and a var without a default and without a value stays shaped; a bundle's `presets/<name>.md` resolve as on `run`. Where `run` is lax, the dry run says: a name no var declares, or a value the var's type cannot read, is the dry run's error beside the compile verdict, attributed to its source (`--var`, the tool's `vars`, or the preset that set it); a bundle preset file the dry run cannot read is its error too, where `run` logs it and goes on. The MCP tool takes `vars` (an object of strings keyed by var name, the key `local_run` uses) and `preset`. The vars `run` derives from the credentials at launch (`review_mode`, `mono_family`, `plan_review`, `llm_families`) are not derived by the dry run, which reads no credential: a bot that gates on one sees a shape, or the value you give with `--var`. Until the dry run crosses a bounded loop a few times only (#1307), `--strict` is a per-bot gate: a bot whose loops cross many times runs them to their cap, and needs `--exec-timeout` raised or is left out of the gate. A key `node/child_node` answers a node of a simulated child; a key that names no node is a `fixture` finding, not a thin recording. Each simulated child's passes are listed under `children` — a child that dies is the parent's death — and `clean` says whether every pass finished (or ended at a declared `fail`) with no reference, shell or fixture finding and no inconclusive expression; the human block prints it as `verdict`. A pass that ran to a **ceiling** the shapes imposed — the bot's own `budget:`, an unbounded loop's fuel, the liveness monitor or the budget guard declining a loop edge whose exit rides a verdict the dry run shaped — is said so (`ceiling`) and is not a death: a loop that converges in production cannot converge on shapes, so its cap is the run's guard doing its job; pin the verdict with `--fixtures` to run it to its exit. Every branch of a fan-out runs to its own end under the dry run — a failure cancels no sibling — so a death in one branch beside a ceiling in another is read as the death it is. A `best_effort` fan-out finishes the run around its branches' deaths, but the deaths still land on the pass under `dead_branches`, and `clean` reads them: a fan-out whose branches all die is not clean, whatever the collector's `await` did with the finish. A bounded loop spent with no exit is the program's — C145's shape — and a death. A `json`-typed field has no defined shape, and the dry run does not guess one: it takes the object shape — the one-element list only where an iteration reads it (a fan_out_each `over:`, a foreach collection, the collection of `map`/`filter`/`reduce`), so the body is entered on both passes — and an expression that fails while reading a value the dry run invented (a shaped `json` field, a `json` var with neither a default nor a `--var`, a value computed or mapped from one, the item a fan-out drew from one) is **inconclusive**, never a death: the pass goes on with a shape in the expression's place, the finding (`kind: inconclusive`) names the value and what would decide it — `--var x='<sample>'`, a fixture on the producing node, a typed field in its schema — and the report lists them under their own heading, `inconclusive (N)`. A failure on the program's own value — a `--var`, a fixture, a literal, a field the schema types, a node the pass has not produced (a loop's `previous_output` before its second crossing included) — stays the death it is. The MCP tool takes `exec` and `fixtures` the same way.
...
iterion diagram x.bot.yaml              # an author document, drawn as the .bot it stands for
```

`--detailed` and `--full` are aliases for the corresponding `--view` values.

An author document (`x.bot.yaml`, the YAML twin of a `.bot`) is drawn as the `.bot` it stands for — the diagram `iterion diagram x.bot` draws once `iterion fmt --to bot` has written it, which `diagram` never does. The document is compiled as that `.bot` is: in the bundle the `.bot`'s own diagram reads (a bundle's `main.bot`, or the bundle any other workflow file belongs to — a manifest export naming the `.bot` is that `.bot`, written or not, spelled through a symlinked directory of the bundle or not), with the fragments its imports name read where that diagram reads them, and through the same stages. What refuses the `.bot` refuses the document, in the same words: the bundle's (a manifest that does not decode), the unit's errors, the compile's, the MCP servers' (a server nobody declares, a `.mcp.json` that does not parse). Before those, what only a document has refuses it: its own errors, a program with no written `.bot` form (E054), a `.bot` name `diagram` would not read back (`UPPER.BOT.YAML` stands for `UPPER.BOT`, which `fmt --to bot` does not write either), and a `.bot` path `fmt --to bot` will not write over (a directory, a file that cannot be read). `--json` then carries `source_kind: "author"` and `bot_path`, as `validate` does.
...
iterion fmt x.bot.yaml                   # an author document, in its canonical YAML form (refused when it carries YAML comments)
iterion fmt --to bot x.bot.yaml          # write the .bot the document stands for, beside it
iterion fmt --to yaml x.bot              # write the author document of a .bot, beside it
```

`fmt` parses each file, writes it back through the writer the studio saves with (`pkg/dsl/unparse`) and proves, before writing, that the text reads as the same program (`Verify`: same parse, same profile, same compiled workflow and diagnostics, prompt bodies in their canonical form). The text lands on the file's own bytes: a BOM and CRLF line endings are kept. `Verify` also compares the comments: the text carries the same ones, on the same declarations. A file it cannot rewrite without changing it is refused by name and left as it is — one that does not parse, one whose comments the round trip would lose, one holding a value written over several lines (a `|` block scalar, a raw string spanning lines): the writer has no multi-line form and would give it back as a single line, which is the same program and not the same file ([#1612](https://github.com/SocialGouv/iterion/issues/1612)) — while the files beside it are formatted all the same. That last refusal is not `fmt`'s alone: it lives in `pkg/dsl/canon` and every path that turns a document into the bytes of a `.bot` asks for it — the studio's save of a file, of one file of a unit, and the cloud write-back — so a file `fmt` leaves alone is a file the studio refuses to rewrite too, naming it and the line the value is written on. The studio's Source view still SHOWS such a file: as its own text, with the reason, read-only. Folding is a before/after property, so the refusal is only made where the file's current text is in scope; the two writes a client performs — `PUT /api/teams/{id}/bot-sources/{slug}/files/{path}` and the whole-bundle `PUT /api/teams/{id}/bot-sources/{slug}` a cloud save of a bot in several files lands on — are each judged on the two texts their route holds. A comment goes back where it was written, its own indentation and its paragraph breaks included; one whose line the writer does not emit (a property equal to its default, an empty list) is written at the end of the block it was in rather than dropped. Two move, by name: a comment at the END of the `dsl:` header's own line, and one above the file's first declaration separated from it by a blank line — both land at the file's head, which is where the head of a `.bot` has always been read. `.botz` archives are not workflow files: naming one is an error, a walk passes over them. `--check` writes nothing and exits non-zero when a file would change or is refused; `--json` lists each file (`changed`, `written`), the refusals and their paths (`refused_paths`).

**Author documents.** `fmt` takes a `.bot.yaml` — the YAML twin of a `.bot` ([dsl.md](dsl.md)) — like a `.bot`: named or met in a walk, it is rewritten in its canonical YAML form (`pkg/dsl/canon`, `Document`), the program it describes written back by the author writer and proven to read as the same program, on the document's own bytes — a BOM and CRLF line endings are kept. Two kinds of document are refused and left as they are, named among the refusals, because the rewrite would lose what their author wrote: one carrying YAML comments (the writer keeps none), and one the `.bot` reads otherwise than it was written — a prompt body with leading or trailing blank lines the lexer settles, a line separator in a block scalar, what `validate` reports as E053 — since the writer would put that reading in the author's place. `fix` and `dsl migrate` take no document: named one, they say which command writes the `.bot` they work on. `--to bot` writes the `.bot` a named document stands for, beside it (`x.bot.yaml` → `x.bot`), after the proof `validate` reports as E054 when it fails — a document whose program has no written `.bot` form is refused; the `.bot` written carries the settled reading of an E053 body, and the reading is said under `note:`, as are the document's YAML comments, which the `.bot` is not written with — counted, the first named (a ` #` inside a plain value starts one); its frontmatter is spelled as the writer spells a `catalog:` (a list as `[a, b]`), whatever the original `.bot` wrote — the identity is kept, the spelling is a comment's. A document whose `.bot` name `fmt` would not read back (`UPPER.BOT.YAML` stands for `UPPER.BOT`; a workflow file's suffix is `.bot`, lower-case) is refused before anything is written. `--to yaml` writes the document of a named `.bot` (`x.bot` → `x.bot.yaml`) from a text that parses without an error — never from one the parser recovered on, even with `--force` — once it has proven the document reads back as the same program (a writer that spelled what the reader refuses is refused, nothing written). Its `catalog:` is what the catalog reader reads off the frontmatter — one block and one reading, shared by the bundle loader, the author writer and this notice: `parser.Frontmatter` takes the fence block that opens the file's head (blank lines, the `dsl:` header and `import` lines may precede the fence, a comment may not; a BOM or CRLF endings read as the lexer reads them), and `workflowfile.DecodeFrontmatter` decodes it, so a key named twice keeps its last value in both, and a block that reading does not read (a scalar `triggers:`, a malformed line) gives the `.bot` no identity and the document no `catalog:`. What the document does not carry is said under `note:`, off the document written and with the reason: a `catalog:` it has none of, the frontmatter keys beyond the four of `catalog:`, the ordinary comments — the lines of a block that became no `catalog:` counted among them. Either way a destination already there and not what the source writes is refused without `--force`, one that is already it is a no-op, one that is there and is not a file (a directory) or cannot be read is refused, whatever `--force` says, while the files named beside it are converted all the same, and `--check` reports what a write would do — the refusal included: a differing destination is refused under `--check` as it is by the write, and "would write" only with `--force` — the CI form of "is the `.bot` beside the document the one it writes?". `--to` takes named files: a conversion writes beside what it reads, and a walk would write trees. `--json` carries each conversion as a file with `from`, and the notices as `notices`.

`--baseline <file>` makes `--check` usable as a gate on a tree that is not yet wholly canonical. The file lists, one path per line (`#` comments ignored), what this tree already knows its canonical form refuses — in this repository, `.fmt-refused` and its 39 bots behind [#1612](https://github.com/SocialGouv/iterion/issues/1612). The check is then GREEN while the refusals are exactly those, and red — naming the file — on each of the three ways that stops being true: a formattable file drifted out of its canonical form, a file newly became unformattable, or the baseline names one nothing refuses any more (a file formatted at last, renamed, or deleted). That last direction is the ratchet: the backlog cannot grow quietly, and it cannot be left behind once the writer can carry those files. A baseline that is not there is an error, never "nothing is refused". `task fmt:check` runs exactly this over `bots` and `examples`, and the `fmt-check` CI job runs the task's command.
...
says how many under `Drafts left out` when there are any; the `--json`
counter `drafts_left_out` is always present, so zero reads as zero.
...
| Flag | Meaning |
|---|---|
| `--template <id>` | Start from a gallery template (default `blank`); `iterion bots templates` lists them. Five render the single-agent workflow with a different mission (`blank`, `daily-digest`, `code-reviewer`, `docs-writer`, `issue-triager`); ten render a complete, commented **shape** — `campaign-loop`, `review-fanout`, `plan-gate-implement`, `scheduled-digest`, `per-ticket-subbots`, `verified-action`, `async-questions`, `multi-file`, `library` (a bot in several files: `main.bot` imports `lib/schemas.bot` and `lib/nodes.bot`), `contract` (a bot whose public contract — inputs mirroring its vars, outputs naming their producers, a delivered file, criteria, effects — is bound to its program) — each held to its form by a test (the table in `SKILL.md` § Start from a template). A shape's annex files (a child `worker.bot`, `prompts/*.md`, `skills/*.md`) are written with the bundle. `campaign-loop` and `scheduled-digest` need `jq` and ship a `devbox.json` + `devbox.lock` pinning it, which the engine installs on any image that ships devbox (every iterion sandbox image does, and ships jq too). |
...
`iterion models pricing` audits the rate a run is **actually charged at** against the price published by the spec aggregator (models.dev), and reports every disagreement — it never rewrites the committed table in `pkg/backend/cost/cost.go` (prices feed budget decisions, so a change is a human judgement call). The charged rate is resolved in tiers: claw's live registry, then the aggregator's published pair, then the committed table. Three verdicts: **DISAGREES** (the charged rate differs from the published one — in practice claw's live registry quoting something else, since the aggregator's own pair is used when claw has no entry), **IGNORED** (a price is published and a run still reports none — now expected only for a **half-published** pair, which is refused whole rather than priced with a zero half), and **table only** (nothing is published — expected for brand-new models, which is why the committed table exists). `--refresh` refetches published prices first; `--check` exits non-zero on drift, for CI:
