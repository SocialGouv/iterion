...
The cache is keyed on a fingerprint of the tree (path, size, modification time of every `.go`, `.md`, `.bot` and `go.mod`). Any change rebuilds all of it: patching a graph in place would be faster and could describe a repository that no longer exists, which is the one failure an index must not have.
