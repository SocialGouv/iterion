...
| A **claw → CLI** crossing, whatever the `tools:` list | The list does not mean there what it means on claw — claude_code narrows only its own 14-name native roster (MCP tools and the names outside it survive), codex maps it onto a sandbox mode, and pi/kimi/grok never receive it — so the route changes what the node can do — and the parallel-branch admission was already decided on the claw reading. (**CLI → claw** restricts, and is refused only when the list is UNDECLARED, which on claw means zero tools; an explicit `tools: []` means the same on both sides and is not refused) |
