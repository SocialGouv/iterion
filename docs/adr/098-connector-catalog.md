...
per-bump checklist, so the ADR references it and P1 adds the v19 → v20 entry (v19 now carries literal template delimiters)
...
| Guarded dialer on every connector path | **Local path shipped** (`connection.LocalHTTPClient` is `httpdial.SafeClient`), **with the deployment-controlled exception this ADR owes it**: `ITERION_CONNECTOR_ALLOW_PRIVATE=1` opens the guard for a self-hosted instance, the refusal names it, and `connections add` warns when a base URL will be refused. Local tier only — a cloud tier must not read it, since there the base URL is tenant-supplied. The executor no longer accepts any non-nil client: a fit one carries a MARK (`exec.MarkGuarded`, set by `connection.LocalHTTPClient`) and `Call` refuses one that does not, so a second wiring fails on its first call instead of silently reaching the deployment's own network. A cleartext `http://` origin is warned about at `connections add` and on every `connections list` — a warning, not a refusal, since a self-hosted instance on a trusted network is a legitimate choice; loopback stays silent, the packet not leaving the machine. **Not said at RUN time**: `pkg/connection` has no log sink and giving it one means threading it through every launch surface, so the decision moment and the audit moment are covered and the moment of use is not |
...
| Guarded dialer | **Local path shipped, with its deployment-controlled exception** (`ITERION_CONNECTOR_ALLOW_PRIVATE`, round four); the executor refuses a client that carries no guard mark, and a cleartext origin is warned about rather than refused |
