...
## 2026-09-14 — first catalogue DSL 2 lot (#1159, PR #1206)

- Status: **review published, draft preparation only** — [review](https://github.com/SocialGouv/iterion/pull/1206#pullrequestreview-5194427952) on `bfd857ee9da16b0e0743dc3ac80d1b75d83b4a22`, run [01a09e86-9b18-7c69-ba61-c35ae6482b1f](https://iterion.cloud/runs/01a09e86-9b18-7c69-ba61-c35ae6482b1f). Zero findings; the published ticket assessment correctly says #1159 is **partial**, since only two bundles are prepared and activation remains outstanding.
- Versions and method: source base `882c76afbb` (3.143.0); `/revi` requested explicitly because opening the draft did not launch a review. The live server reports v3.142.1. Platform override executed `reviewer_gpt` and synthesis through **claw / openai/gpt-5.6-sol**, high / medium effort. Launch input `mono_family=claude` is not the served route; the events and published table are the evidence.
- Result: review 408,508 tokens + synthesis 25,534 = **434,042** at publication. The reviewer covered both headers/floors, full-IR equivalence, production request captures, prompt goldens, convergence truth tables, mixed-profile migration and the rollout prerequisites. No campaign, platform push, deployment or merge was performed by this preparation.
- Tool evidence and limits: 29 `tool_called` events, including successful `bash` and `read_file`, and 17 `llm_request` events. The observed errors have different meanings: in `reviewer_gpt`, `git diff --check` found two trailing spaces in rendered empty-scope fixture lines and a bash call reached its declared 30-second timeout. In the read-only `converge` phase, `todo_write` was denied by policy (it is allowed in the reviewer phase), and a `read_file` of nonexistent `.review-pr` recovered from the model’s wrong path assumption. These are not, by themselves, tool-wiring defects. The separate ambient Sentry MCP initialization EOF is explicit connector degradation; its redacted event is retained for the end-to-end review audit (#1203).
- Follow-up: use the explicit fixture scope `src/**` so the actual captured strings stay byte-exact without incidental trailing spaces. Regenerated the two survey goldens without trimming any rendered bytes; the empty-default scope remains explicitly tested under both profiles. Prompt/gate tests and `git diff --check HEAD` pass. Before the push, no `branch_improve_loop` run was active. There were no Revi findings to delegate to Billy.
- Validation: full relevant bots/DSL/botreplay/bundlelint/CLI suites passed under `-race`; standalone termination-output goldens and lint passed. Temporary paragraph-removal and missing-scope-conjunct mutations both failed the new tests, then the restored source passed. These are deterministic render/contract checks, not a claim that either migrated campaign has been validated by a live run.
- Lesson: check added fixtures against `HEAD` after staging; an unstaged `git diff --check` does not inspect untracked files. Keep the rollout decision separate from a locally green migration: the actual catalogue release, full fleet floor and deployment-wide campaign state remain prerequisites in [the lot runbook](../catalogue-dsl2-lot1.md).
...
- Versions: catalog 0.9.2; production override 0.9.2-codex-claw.3 (platform v8). All LLM nodes remain on `openai/gpt-5.6-sol` / `claw`, using the Codex subscription. Model routes, permissions, tools and budgets match the previously deployed override.
- Method: launch through the Iterion CLI's `POST /api/runs`, with repository checkout, forge connection, pinned PR head, mono GPT and `post_to_board=false`. Read run outputs and events through the Iterion MCP, then fetch the actual GitHub review and inspect its rendered details/link.
- Result: 0 findings, 0 questions. Exactly one visible sentence precedes the collapsed details. The published ID links to the ordinary `/runs/{id}` page on the executing instance; no callback credential, query or fragment is included. Clicking it reaches that route. The browser used for this check was signed out and showed the cloud sign-in landing; authenticated run access was verified through MCP. Existing login and access controls remain in force.
- Telemetry: reviewer requested `high`, 275,182 tokens; synthesis requested `medium`, 24,647 tokens; published run total **299,829**. Both engine outputs and the rendered table report `openai/gpt-5.6-sol` through `claw`.
...
- Iteration: the first concise trial (platform v6, run `01a09a70`) still displayed a “no ticket refs” diagnostic. The second (v7, run `01a09a77`, 281,402 tokens) verified its relocation into details. The final v8 trial above also verifies the linked ID. None of these trials needed a publication retry.
- CI friction: the existing tier/convergence E2E scenarios return typed Go string slices for `scanned_areas`; unlike live JSON output, these were rejected by `concat()`. It now uses the expression runtime's existing slice normalizer, preserving scalar rejection. The original E2E fixtures remain unchanged, and scope tests also exercise typed and mixed slices.
- Validation: `go test ./bots -run 'TestReviewPR|TestCatalog' -count=1` passes; catalog and override both validate. The actual publisher is exercised for clean/inline/unanchored/degraded outputs, ticket/question filtering, base-path preservation, callback query/fragment removal and unavailable-ID fallbacks. Scope evaluation covers skipped and glance branches. Production and catalog publisher code match apart from inactive Claude telemetry rows.
