...
## 2026-09-18 — DSL profile 2 dogfood: one design turn on a workspace bot, GPT window closed, Claude fallback, the validator catches the draft (run 01a0b32b-5576)
- Status: **partial** — one complete operator turn delivered (diagnosis, draft and the validator's verdict at the chat pause), reached through a `failed_resumable` and a resume; the run paid a stale-binary trap for twenty minutes.
- Versions: bot copilot 0.2.8 (`dsl: 2`, the wave-4 PR of #1344) · iterion v3.162.2 (`d98abd04d`; the engine did not change in this wave).
- Method: CLI run from a two-file workspace (`bots/demo/main.bot`: a bounded fix loop with no exhaustion exit), `--sandbox none --merge-into none --no-interactive`, `--max-cost-usd 3 --max-duration 15m`, vars `mode=design` and an `initial_message` asking Copi to validate the demo bot, explain the C145 warning and draft the fix. Backends as declared: Terra on claw/gpt-5.6-terra with the Claude Opus fallback (0.2.6).
- Result: the ChatGPT window was closed (429 on every attempt). Terra's opening turn went through on GPT after two retries (9k tokens); the following turns fell back to `claw anthropic/claude-opus-5` as designed. Two permission pauses (`diagnostic_shell`: `iterion validate …`, then `iterion validate --help`) answered `allow`, then `allow always`. The third turn died `failed_resumable` after 25 min and 83k tokens: `structured output invalid: missing required field "requires_authoring_context"` on the fallback route. A first resume with `--max-duration 10m` was refused at once (`BUDGET_EXCEEDED duration 1269s/600s`: the cap counts the pauses); the second, with 45m, completed the turn in four minutes ($0.41 reported for that segment; the earlier claw segments report 0 tokens) and paused at `chat` with the reply.
- Value: real but bounded — Copi's reply (French, mirrored) corroborates the C145 risk, explains LOOP_EXHAUSTED correctly and proposes a draft — of the WRONG shape (`fix -> fail else`); the deterministic `validate_draft` node caught it (C015: `else` without a `when` sibling) and the bubble carries "iterion validate did NOT pass on this draft" with the diagnostic. The product's own gate did its job.
- Findings / misses: (1) `diagnostic_shell` runs the operator's PATH `iterion` — here `~/.local/bin/iterion` v3.112.2, older than C145 — so `iterion validate` answered OK and Copi spent 130 messages searching the binary's strings for a warning the engine running it would have printed: a bot's shell tools see a different iterion than the engine that runs the bot (ticket). (2) Copi's skills did not know the loop-exhaustion exit shape (its reply says so) and the run-debug table explained LOOP_EXHAUSTED as "the exit condition never became true" — both fixed in this PR (the DSL skill teaches the bare exit beside the loop edge; the row names the real cause). (3) The fallback route ended a turn with a schema-invalid structured output → `failed_resumable`; the resume cured it (ticket: whether the engine should re-ask the model for the missing field). (4) A resume's `--max-duration` must exceed the run's elapsed time including its pauses.
- Engine hardening: two tickets — the PATH iterion a bot's shell tools see; the schema-invalid structured output on a fallback route.
- Lessons for next run: refresh the operator's installed iterion before dogfooding a bot that shells out to it; resume with a `--max-duration` above the elapsed time; a delivered Copi turn costs about a dollar when metered.
