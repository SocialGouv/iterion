...
## 2026-09-21 — the #1464 receipt: two passes, the drifted lock read as noise, 15 ADRs in stride (run 01a0c5a5-b07d)
- Status: **validated** (the receipt this run was launched for is measured, live and reproduced) — with one engine regression surfaced, ticketed (#1558).
- Versions: bot v2.0.2 · iterion `v3.177.0+8ee6f0e6d` (the #1530 branch head, = main once merged as `9a9cd778e9`) · host run (`--sandbox none`), worktree: auto, Sonnet on both agents (`ITERION_ADR_MODEL_CLAUDE` / `ITERION_ADRY_MODEL_CLAUDE=claude-sonnet-5`, the account's Opus meter was capped).
- Method: CLI run from the branch worktree, `--store-dir /home/jo/lab/ai/iterion/.iterion`, `--merge-into none`, `--no-interactive`, `--timeout 75m`, `max_passes=1` (= up to two passes), `bundle_self_path=bots/adr-cartograph`; account selected by `CLAUDE_CONFIG_DIR=<cswap session profile>` with the auth-override variables AND `ZAI_API_KEY` unset (a first attempt died on z.ai's `[1310]` weekly quota: the engine reads `ZAI_API_KEY` from the ambient environment as the z.ai hint and routes `claude_code` through the facade; a `resume` keeps the credential pinned at launch — a fresh run re-resolves). ~62 min wall (20:26Z → 21:07Z, survey 33 min on Sonnet).
- Result: `finished`, two passes. Pass 1: scan 108 ADRs → survey → manifest → campaign authored ADRs one commit each → `scope_check` `{scope_ok: true, out_of_scope: []}` with the engine's `.claude/skills/` mirror laid untracked → `verify_build` (the agent's `devbox run -- task build`) → gate not converged. Pass 2: campaign → **`scope_check` at 21:06:28Z with `devbox.lock` DRIFTED (lock polls 21:05:15 → 21:07:30 all drifted) → `{scope_ok: false, out_of_scope: ["docs/references/map-docs.md"]}` — the drifted lock is NOT listed; the one path named is a real out-of-scope write (the agent regenerated the docs map with `task map:gen`)** → gate not converged → `mark_issue_for_review` → `update_cache` → done. Storage branch `iterion/run/01a0c5a5-…` → `2c078ab74`: **15 ADRs** (rewind produced-scope default, repograph ×4, tree-noise canonical list, list-shaped run vars by env, credpool ×2, usagecap ×2, pool audience, credential spend attribution, schedgate overlap vocabulary, secretguard certainty/suspicion) — cherry-pick candidates; the run's own worktree preserved (see the regression below).
- The receipt, reproduced on a real engine worktree (the preserved worktree of the earlier failed run, mirror laid by the engine): the merged `scope_check` command run verbatim against a hand-drifted `devbox.lock` — with `ITERION_TREE_NOISE` provisioned: `{"scope_ok": true, "out_of_scope": []}`; with the variable unset (the pre-#1530 floor): `{"scope_ok": false, "out_of_scope": ["devbox.lock"]}`. The only difference between the two runs is the drifted lock — the defect #1464 was opened for, closed.
- Value: HIGH — the receipt itself, plus 15 undocumented decisions of the last days captured as Nygard ADRs (including the tree-noise decision this very run was validating: ADR-107 "one canonical tree-noise list, mirror-only finalizer exception").
- Findings / misses: (1) **engine regression from #1507's wip bank**: `git add -A -- ':/' ':(exclude,top).claude' …` exits 1 on a repository that gitignores `.claude/` (iterion's own `**/.claude/`): git reports the exclusion pathspec as "paths ignored by .gitignore" and refuses — the wip bank fails, the run's uncommitted work is left unbanked and the worktree preserved (`wip bank git add -A failed: exit status 1`). The finalize e2e laid an untracked mirror that was NOT ignored — a fixture that does not have the run's real shape (the same class the review-fanout exec test paid in #1530). Ticket #1558. (2) The lock drift came from the `verify_build` AGENT's own `devbox run` (the class #1464 leaves open by design) and, once, went back to identical within 45 s (mechanism not determined: the agent's inner Bash calls are not in the event stream). (3) The campaign's `task map:gen` is out of Adry's docs/adr scope — the gate flagged it correctly.
- Engine hardening: #1558 (the staging gesture must not name an ignored path: drop the exclusion when `git check-ignore` says the path is already ignored, and the e2e's fixture must carry the ignored-mirror shape too).
- Lessons for next run: pin Sonnet when the account's Opus meter is short (the survey is 30+ min either way); unset `ZAI_API_KEY` in the launch environment unless z.ai is wanted; expect `map-docs.md` to trip the scope gate whenever the campaign regenerates the docs map — either widen `adr_dir`-scoped allowances to the generated maps or tell the campaign not to regenerate them.
