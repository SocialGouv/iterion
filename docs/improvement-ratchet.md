...
| **The keyed connector** | a defect class made structurally impossible to repeat — the notch that only lets the plug in one way | the DSL [diagnostics](references/diagnostics.md) (`C001`–`C199`, `C240`–`C249` and `C260`–`C268`, plus the bundle's `C200`–`C234` and `C250`–`C253`), [`bots/catalog_universality_test.go`](../bots/catalog_universality_test.go) (greps every catalog bot's var defaults for iterion paths, and its whole source for stack-specific logic), `bots/verify_probe_wiring_test.go` |
