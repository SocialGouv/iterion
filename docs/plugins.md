...
destination file still hashes to the marker. That predicate is exactly
`reconcileSkillFile`'s "refresh-vs-shadow" test: a matching hash means
iterion wrote the file and the operator did not edit it, so pruning it
takes nobody's work. Anything else — a file with no marker, a file the
operator edited (hash diverged), a file that already vanished — is left
alone. The sweep runs by default only when the workspace is a
**run-owned** worktree (`worktree: auto`, where a stale mirror file has no
operator interpretation). A `worktree: none` / direct-mode run against the
