...
A loop or foreach whose source sits **inside** a `fan_out_all`, `fan_out_each`, or `llm` `multi: true` body is a compile error (**C244**): those subgraphs run as parallel branches with no local loop counters. The walk stops at structural joins (`await:` or multiple non-iteration predecessors), not at a node `findConvergencePoint` elects only because of a loop back-edge — a per-branch retry (`a -> a as refine`, `impl -> review -> impl as fix`) is C244, because that election swallows the real join. A loop after a non-elected `await:` in a sibling branch is a remaining `execBranch` hole (compile-ok, skipped at run time). A loop that wraps the fan-out from the join (`join -> router as outer(N)`) is on the trunk and is allowed. Per-item retry loops belong in a `subbot` until branch-local execution exists; see [composition/iteration/sub-bots](groups-iteration-subbots.md).
