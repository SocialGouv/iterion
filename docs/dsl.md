...
- `compress: off|on|ultra` sets command-output compression on the `workflow` block and on `agent`/`judge`/`tool` nodes. Agent and judge nodes are opt-OUT (on whenever a rewriter plugin is enabled and its binary is present); tool nodes are opt-IN, so a review loop's `git diff` stays full-fidelity. An invalid value raises C102; see [command-output compression](plugins.md#command-output-compression-rtk-generalized).
