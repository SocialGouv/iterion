...
An included file may itself include another, relative to **its own** directory, up to 8 levels; a file that includes itself is refused as a cycle. Beyond those, one whole expansion — a compile, or one server-side resolution of a document's prompts — may inline **2 MiB across at most 1024 files in total**, and is refused by name when it runs out. The total is what the per-file cap and the depth limit do not bound: a graph where each level includes the next *k* times expands `k^d` times with every single file under 256 KiB, so a handful of tiny files can produce megabytes — and that expansion runs in the server process, over files a tenant controls. Neither bound is reachable by ordinary authoring, and neither is an operator dial: they are the isolation between tenants sharing a pod.

What an include resolves against is the prompt's **source file**, never the working directory of the process compiling it. A prompt with no source file — a document validated from the studio canvas (JSON, no positions), an AST that reached a runner with a marker still in it — is refused with C055; the studio still saves such a document, marker intact, and the next parse of the file on disk resolves it beside that file. The one exception is a source compiled **inline on this host** (the studio's run-from-editor, `iterion run` on stdin): it has no file, so its includes resolve against the current working directory — the workspace root for a studio — which is why the same source is refused when published to a cloud instance.
