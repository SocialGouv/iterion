...
## Checklist: v14 → v15 (loop cap expressions)

v15 carries AST loop caps that may contain expressions over vars and outputs.
An older runner cannot compile that meaning of `max_iterations_expr`; it must
reject the envelope before admission. Every new publication uses v15, including
runs whose loop caps are still literals or templates.

- [x] Schema version is 15 and `MinSchemaVersion` remains 10.
- [x] The AST JSON retains the authored cap for runner-side compilation.
- [ ] Roll out v15-capable runners first, then the server publisher. This order
...
- [ ] Confirm the fleet reports support before publishing v15. A rollback to
      the old publisher emits v14, which the new runners continue to accept.

## Checklist: v15 → v16 (connector packages) — NOT YET BUMPED

v16 will carry the connector packages a run resolved at launch
...
server emits v16, a fleet of digest-pinned v15 runners rejects *all* new
...
- [ ] `MinSchemaVersion` stays **10**. The addition is additive from a new
      runner's point of view, so new runners keep consuming v15 for the whole
      window.
- [ ] **Runners first, then the server** — the inverse of the default order,
      ...
      restart.
- [ ] Only after the runner fleet reports v16-capable: let the server publish
      v16. A rollback then re-publishes v15, which the new runners still
      accept.
