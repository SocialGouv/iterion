...
## v15: model effort and run-level permission overrides

Schema v15 adds `RunMessage.permission`. This is intentionally a versioned
...
declarations. `MinSchemaVersion` remains 10 so new runners can drain older
queued messages whose permission field was necessarily absent.

This version also carries `ModelOverride.Effort`. The assistant branch's
...
## Checklist: v18 → v19 (connector packages) — NOT YET BUMPED

v19 will carry the connector packages a run resolved at launch
...
server emits v19, a fleet of digest-pinned v18 runners rejects *all* new
...
- [ ] `MinSchemaVersion` stays **10**. The addition is additive from a new
      runner's point of view, so new runners keep consuming v18 for the whole
      window.
- [ ] **Runners first, then the server** — the inverse of the default order,
      ...
      restart.
- [ ] Only after the runner fleet reports v19-capable: let the server publish
      v19. A rollback then re-publishes v18, which the new runners still
      accept.
