...
| Studio/API/CLI/Copi | Actual browser and API document round trips, four views, map/cost visibility | Studio public/technical graph editing, API/CLI public projection, MCP local validation draft and real Chromium save pass. MCP now offers the registry-backed public contract/graph authoring schema with technical kinds on demand; direct Copi editing, runtime cost visibility and composition outstanding |
