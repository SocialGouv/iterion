...
| `config` | Non-secret env: `rollout.*` (monotonic runner epoch), `mongo.*`, `nats.*`, `s3.*`, `redis.*` (Valkey/Redis for distributed state — the fourth `/readyz` ping), `server.*` (`shutdownDelay` / `shutdownTeardown`, see [probes-and-graceful-shutdown.md](../../docs/probes-and-graceful-shutdown.md)), `runner.*`, `auth.*` (public half: publicUrl, cookies, TTLs, signupMode, OIDC client ids), `disableAuth` (**turns authentication off wholesale — smoke tests only, never a reachable deployment**), `smtp.*` (host/port/from — enables invitations + password-reset email), `webpush.*` (VAPID subscriber — see [notifications.md](../../docs/notifications.md)), `orgDefaults.*` (platform-wide launch limits → `ITERION_ORG_DEFAULT_*`), `marketplace.*` (hosted bot registry → `ITERION_CLOUD_MARKETPLACE` — **adds anonymous browse/download routes**), `log.*`, `extraEnv` (raw env passthrough for anything the chart does not model) |
| `secrets` | Five bundles. Four share a shape — `create:` (chart-rendered, dev) or `existingSecret:` (sealed-secrets / external-secrets, prod): `llm` (fallback provider keys), `storage` (S3 creds), `auth` (`ITERION_JWT_SECRET`, `ITERION_SECRETS_KEY`, bootstrap admin, OIDC client secrets, `completionWebhookSecret`, `webpush` VAPID keypair), `smtp` (relay credentials, server-only). The fifth, `forfait` (`enabled:` + `existingSecret:`), is **never created or sealed by the chart** — you create it by hand with `kubectl` and the chart mounts it as an *optional* `envFrom` on the **runner only**, so a missing Secret never blocks startup. It carries subscription credentials such as `CLAUDE_CODE_OAUTH_TOKEN` so `claude_code` nodes run on the forfait instead of a metered key; `values-prod.yaml` ships it `enabled: true` with `existingSecret: iterion-forfait`. Mind the ToS caveat in [cloud-public-exposure-checklist.md](../../docs/cloud-public-exposure-checklist.md#4-secrets-management) |
| `service` / `ingress` | Exposure; pair ingress TLS with your issuer |
| `probes` | `/healthz` liveness, `/readyz` readiness (Mongo / NATS / S3 / Valkey pings, 1s each, run concurrently so the handler answers in max(ping), not sum). Only **Mongo** is critical — the other three report `degraded` in the body and still answer 200. Plus `startup` (suspends liveness + readiness during boot, 30 × 5s by default) and `preStopDelaySeconds`. See [probes-and-graceful-shutdown.md](../../docs/probes-and-graceful-shutdown.md) |
| `networkPolicy` | Opt-in deny-all egress + allowlist (see `examples/networkpolicy-egress.yaml`) |
| `serviceAccount` / `securityContext` / `containerSecurityContext` | SA creation plus IRSA / Workload-Identity annotations; pod hardening (`runAsNonRoot`, uid/gid/fsGroup `10001`, seccompProfile `RuntimeDefault`) and container hardening (no privilege escalation, all capabilities dropped, `readOnlyRootFilesystem: false` because `node_modules` and the git index need a writable root) |
...
- **Generation-aware rollout** — first ship wire v12 with
  `config.rollout.runnerEpoch: 0`, then raise it to `1` in a separate release.
  Increment it for runner-code rollouts. Never `helm rollback` to a lower
  epoch: re-release the older image with a higher epoch. The value and its
  mismatch delay are literal PodTemplate envs so an old ReplicaSet scaled by
  HPA/KEDA cannot inherit the new generation through the shared ConfigMap.
