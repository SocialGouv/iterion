...
// and materializes exactly that dependency. Local Git sources must be clean by
...
func BotsUpdate(ctx context.Context, opts BotsUpdateOptions) (*BotsUpdateResult, error) {
	...
	if !ok {
		return nil, fmt.Errorf("bot dependencies: %q is not declared in %s", opts.Name, filepath.Join(absWorkdir, botlock.FileName))
	}
	if opts.Ref != "" {
		dep.Ref = opts.Ref
	}
	...
	previous := lock.Dependencies[opts.Name]
	dep.BundleSHA256 = hash
	lock.Dependencies[opts.Name] = dep
	if err := botlock.Save(absWorkdir, lock); err != nil {
		return nil, err
	}
	synced, err := syncBotDependency(ctx, absWorkdir, opts.Name, dep)
	if err != nil {
		lock.Dependencies[opts.Name] = previous
		if restoreErr := botlock.Save(absWorkdir, lock); restoreErr != nil {
			return nil, fmt.Errorf("%v; additionally failed to restore bots.lock: %w", err, restoreErr)
		}
		return nil, err
	}
	return &BotsUpdateResult{Name: opts.Name, Ref: dep.Ref, BundleSHA256: hash, InstalledPath: synced.InstalledPath}, nil
}
