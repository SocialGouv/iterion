...
func TestAdminBotsPush_RefusesAProfileTwoBundleWithAFloorBelowTheRelease(t *testing.T) {
	...
	if !strings.Contains(w.Body.String(), "0.0.1") || !strings.Contains(w.Body.String(), "below "+parser.ProfileSince[2]) {
	...
	if w.Code/100 != 2 || !strings.Contains(w.Body.String(), "FORCED past the syntax-floor guard") || !strings.Contains(w.Body.String(), parser.ProfileSince[2]) {
