...
//
// scopeOverride is for routes whose {id} is NOT the store scope — the org
// credential tier, where {id} is an ORG id and the rows live under the
// reserved secrets.OrgTierTenantID(orgID). Deriving the scope from the path
// there would filter on an id no row carries: the lookup returns "not
// found" and the org's own key reads as absent.
func apiKeyTenantCtx(r *http.Request, scopeOverride string) context.Context {
	if scopeOverride != "" {
		return store.WithTenant(r.Context(), scopeOverride)
	}
...
func (s *Server) handleUpdateApiKeyIn(w http.ResponseWriter, r *http.Request, scopeOverride string) {
	...
	ctx := apiKeyTenantCtx(r, scopeOverride)
