...
type blockedMissionSweepStore struct {
...
func TestAssistantMissionReplacementAndDrainCancelInFlightSweeps(t *testing.T) {
	s := &Server{}
	st := &blockedMissionSweepStore{entered: make(chan context.Context, 3)}
	...
	cancelled := func(ctx context.Context) {
		...
		case <-ctx.Done():
		case <-time.After(2 * time.Second):
			t.Fatal("old sweep was not cancelled")
		}
	}
	...
	s.restartAssistantMissions(nil, nil, st)
	s.stateMu.RLock()
	current := s.assistantMission
	s.stateMu.RUnlock()
	if current != nil {
		t.Fatal("a draining server published a new coordinator")
	}
}
