...
func newEmitTestServer(t *testing.T) *Server {
	...
	coord := StartTriggerCoordinator(ns, subs, nil, &recordingLauncher{}, nil, iterlog.New(iterlog.LevelError, nil))
	if coord == nil {
...
func TestEmitTrigger_PublishesAndFires(t *testing.T) {
	...
	coord := StartTriggerCoordinator(ns, subs, nil, rl, nil, iterlog.New(iterlog.LevelError, nil))
	if coord == nil {
		t.Skip("trigger coordinator unavailable (fsnotify)")
	}
	t.Cleanup(coord.Close)
