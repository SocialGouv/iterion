...
func TestPushSubscribeUnsubscribe(t *testing.T) {
	...
	subs, _ := s.cfg.PushSubscriptions.ListForUsers(ctx, []string{"u1"})
	...
	if subs, _ = s.cfg.PushSubscriptions.ListForUsers(ctx, []string{"u1"}); len(subs) != 1 {
	...
	if subs, _ = s.cfg.PushSubscriptions.ListForUsers(ctx, []string{"u1"}); len(subs) != 0 {
