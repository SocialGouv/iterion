...
func TestReconcileStalled_ForceReapsCtxIgnoringWorker(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if len(snap.Running) == 0 {
			...
			// The outer `deadline` bounds only a wedged actor. #1471.
			claimsBefore := ft.claimCallsFor("fake:stall")
			listBefore := ft.listCalls.Load()
			c.Refresh()
			for ft.listCalls.Load() < listBefore+2 && time.Now().Before(deadline) {
			...
			if got := ft.listCalls.Load(); got < listBefore+2 {
				t.Fatalf("only %d/2 discovery round-trips landed within the outer deadline — actor is wedged, cannot decide the tombstone assertion", got-listBefore)
			}
