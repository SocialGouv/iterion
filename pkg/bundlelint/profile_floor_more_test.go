...
func TestAFloorBelowTheProfilesReleaseDrawsC252(t *testing.T) {
	...
	diags := CheckConsistency(Input{Manifest: low, Syntax: bundle.SyntaxRequirements{Profile: 2, DeclaredBy: []string{"kids/c.bot"}}, EngineBuild: "v" + parser.ProfileSince[2]})
	var found *Diag
	for i := range diags {
	...
	if !strings.Contains(found.Message, `">= 0.0.1"`) || !strings.Contains(found.Message, parser.ProfileSince[2]) || !strings.Contains(found.Hint, `">= `+parser.ProfileSince[2]+`"`) {
		t.Fatalf("C252 = %+v", *found)
	}
	for _, req := range []string{">= " + parser.ProfileSince[2], ">= " + parser.ContractSince, parser.ProfileSince[2]} {
		m := &bundle.Manifest{Name: "probe", Requires: &bundle.Requires{Iterion: req}}
		for _, d := range CheckConsistency(Input{Manifest: m, Syntax: bundle.SyntaxRequirements{Profile: 2, DeclaredBy: []string{"main.bot"}}, EngineBuild: "v" + parser.ProfileSince[2]}) {
...
func TestAProfileTwoBundleWithoutAManifestDrawsC252(t *testing.T) {
	diags := CheckConsistency(Input{Manifest: nil, Syntax: bundle.SyntaxRequirements{Profile: 2, DeclaredBy: []string{"main.bot"}}, EngineBuild: "v" + parser.ProfileSince[2]})
	...
	if diags := CheckConsistency(Input{Manifest: nil, Syntax: bundle.SyntaxRequirements{Profile: 1}, EngineBuild: "v" + parser.ProfileSince[2]}); len(diags) != 0 {
...
func TestAnUnreadChildDrawsC253(t *testing.T) {
	...
	diags := CheckConsistency(Input{Manifest: m, Syntax: bundle.SyntaxRequirements{Profile: 1, Unread: []string{"../sib/main.bot"}}, EngineBuild: "v" + parser.ProfileSince[2]})
	...
	if diags := CheckConsistency(Input{Manifest: m, Syntax: bundle.SyntaxRequirements{Profile: 1}, EngineBuild: "v" + parser.ProfileSince[2]}); len(diags) != 0 {
