...
func TestAFloorBelowTheProfilesReleaseDrawsC252(t *testing.T) {
	...
	diags := CheckConsistency(Input{Manifest: low, Syntax: bundle.SyntaxRequirements{Profile: 2, DeclaredBy: []string{"kids/c.bot"}}, EngineBuild: "v3.141.0"})
	var found *Diag
	for i := range diags {
	...
	for _, req := range []string{">= 3.141.0", ">= 3.150.0", "3.141.0"} {
		...
		for _, d := range CheckConsistency(Input{Manifest: m, Syntax: bundle.SyntaxRequirements{Profile: 2, DeclaredBy: []string{"main.bot"}}, EngineBuild: "v3.141.0"}) {
...
func TestAProfileTwoBundleWithoutAManifestDrawsC252(t *testing.T) {
	diags := CheckConsistency(Input{Manifest: nil, Syntax: bundle.SyntaxRequirements{Profile: 2, DeclaredBy: []string{"main.bot"}}, EngineBuild: "v3.141.0"})
	...
	if diags := CheckConsistency(Input{Manifest: nil, Syntax: bundle.SyntaxRequirements{Profile: 1}, EngineBuild: "v3.141.0"}); len(diags) != 0 {
...
func TestAnUnreadChildDrawsC253(t *testing.T) {
	...
	diags := CheckConsistency(Input{Manifest: m, Syntax: bundle.SyntaxRequirements{Profile: 1, Unread: []string{"../sib/main.bot"}}, EngineBuild: "v3.141.0"})
	...
	if diags := CheckConsistency(Input{Manifest: m, Syntax: bundle.SyntaxRequirements{Profile: 1}, EngineBuild: "v3.141.0"}); len(diags) != 0 {
