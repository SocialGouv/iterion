...
// launchValues is the tool's `vars` object as the dry run takes it: the
// strings the var's type reads, the key `local_run` uses too.
func launchValues(vars map[string]string) map[string]any {
	if len(vars) == 0 {
		return nil
	}
