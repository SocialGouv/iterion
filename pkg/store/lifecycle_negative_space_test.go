...
var negativeSpaceAllowlist = map[string]allowEntry{
	...
	"pkg/store/store_run.go :: Queued+Running":                                                                                         {[]string{"SaveRun"}, "routing-policy launch window: a contract may only land while the run is still pre-outcome (queued/running) — a saver cannot retroactively contract already-terminal work (PR #598)"},
	...
	"pkg/store/mongo/runs.go :: Queued+Running":                                                                                        {[]string{"CountActiveRunsByTenant", "SaveRun"}, "CountsAgainstLaunchLimit twin inside a $in filter + SaveRun's routing-policy launch window (a contract may only land while the run is pre-outcome — mirrors the FS twin, PR #598)"},
