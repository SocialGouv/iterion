...
func TestUsageWindowRetry_TypedErrorSurvivesTheProductionChain(t *testing.T) {
	...
	at, source, ok := usageWindowRetryAt(execErr, retrypolicy.Normalize(retrypolicy.Policy{}), now, time.Time{}, 0)
	if !ok {
...
func TestUsageWindowRetry_FallsBackToTheProvidersOwnWords(t *testing.T) {
	...
	at, source, ok := usageWindowRetryAt(flattened, retrypolicy.Normalize(retrypolicy.Policy{}), now, time.Time{}, 0)
	if !ok {
...
func TestUsageWindowRetry_DoesNotArmOnUnrelatedFailures(t *testing.T) {
	...
	for name, msg := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if _, _, ok := usageWindowRetryAt(err, pol, now, time.Time{}, 0); ok {
