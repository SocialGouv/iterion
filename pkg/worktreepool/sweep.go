...
func Sweep(all []Entry, opts SweepOptions) SweepResult {
	...
	for i := range all {
		...
		if st, ok := reloadRunStatus(wt.StoreDir, wt.RunID); ok &&
			...
			wt.SkipReason = SkipRunActive
			if opts.reclaimStaleNonTerminal && isPausedResumable(st) {
				wt.SkipReason = SkipPausedRun
			} else if !ownsWorktree(st) {
		...
		if reason, ok := stillEligible(opts.ctx(), wt, opts.admission(), opts.RefuseIgnoredEntries, opts.during); !ok {
