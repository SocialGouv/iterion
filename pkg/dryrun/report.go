...
// Clean reports whether the passes met nothing to fix: no reference kept
// as written, no shell text refused. Unchecked things and shapes are said,
// not held against the bot.
func (r *Report) Clean() bool {
	for _, f := range r.Findings {
		if f.Kind == KindUnresolvedRef || f.Kind == KindShellSyntax {
			return false
		}
	}
	return true
