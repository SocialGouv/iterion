...
var probes = map[string]string{
	...
	"port_policy":        "port_policy p:\n  %s: 1\n",
	"port_policy.effect": "port_policy p:\n  effects:\n    request:\n      %s: 1\n",
	"graph":              "workflow w:\n  graph:\n    %s: 1\n",
	"graph.node":         "workflow w:\n  graph:\n    nodes:\n      a:\n        %s: 1\n",
	"agent":              "agent x:\n  %s: 1\n",
	"judge":              "judge x:\n  %s: 1\n",
	"router":             "router x:\n  %s: 1\n",
	"human":              "human x:\n  %s: 1\n",
...
var freeEntryProbes = map[string]string{
	...
	"port_policy.effects": "port_policy p:\n  effects:\n    zz_probe:\n      recovery: idempotent\n",
	"graph.nodes":         "workflow w:\n  graph:\n    nodes:\n      zz_probe:\n        contract: C\n",
	"graph.bindings":      "workflow w:\n  graph:\n    bindings:\n      input.zz_probe -> a.value\n",
	"graph.exports":       "workflow w:\n  graph:\n    exports:\n      zz_probe: a.value\n",
	"vars":                "vars:\n  zz_probe: string\n",
	"presets":             "presets:\n  zz_probe:\n    a: 1\n",
...
func sampleValues(p spec.Property) []string {
	...
	switch p.Form {
	case spec.JSON:
		return []string{p.Name + ": null", p.Name + ": []", p.Name + ": {\"key\": [true, 1, \"x\"]}"}
	case spec.String:
		if len(p.Values) > 0 {
