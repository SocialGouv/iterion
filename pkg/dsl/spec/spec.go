...
const (
	...
	JSON          Form = "json value"   // one JSON value on one line ("text", 12, true, null, [...], {key: value}): the subset the text writes — no signed number, no exponent, keys unique; null explicit, a bare word is not a value
...
var Kinds = append([]Kind{
	...
	{Name: "tool", Role: Node, Doc: "The deterministic node, no LLM. One of three recipes: `command:` runs through bash -c, `script:` through the interpreter `language:` names, `action:` calls a connector operation (ADR-098). With `output:` a command prints schema-shaped JSON on stdout. A Verified Action adds goal + postcondition + policy + recovery (ADR-044), which an `action:` refuses (C262/C263).",
		Properties: []Property{
			...
			prop("retry", String, "Action: how many EXTRA attempts, e.g. `3`; a duration is refused and empty means none (C265). Inert without `action:` (C266)"),
			prop("timeout", String, "Action: bound on one call, e.g. \"30s\" (C265). Inert without `action:` (C266)"),
		}},
