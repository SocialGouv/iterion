...
var Catalog = map[DiagCode]DiagInfo{
	...
	DiagSessionSlotWithoutPersist: {"Session slot without persistence", "Set `session: persist` when using `session_slot:`, or remove the slot."},
	...
	DiagRoutingFieldRef:        {"Routing field holds a template that will not resolve", "Only `{{vars.<name>}}` resolves in `model:`, `backend:`, `provider:` and `interaction_model:` (a node's or a fallback route's) and in the workflow's `default_backend:`: the route is decided before the node runs, and any other `{{…}}` span reaches the backend as written. Write the id, a `${VAR:-default}`, or a declared var; a launch-time choice goes through `iterion run --model` / `--backend`. A warning: a fielded bot keeps compiling and the node fails loud at its first delegation. A supervisor's `model:` renders no template at all — the run starts with an inert supervisor (#1450)."},
	...
	DiagPermissionRulesNoGate:      {"Permission rules that reach no gated reader", "Set `permission: ask` or `deny` where the rules live (workflow or node) so `allow`/`ask`/`deny` apply; or, when the workflow list is shadowed, restate the rules in the node lists that need them — a node list REPLACES the workflow list of the same kind; or remove the lists."},
	DiagToolNodePermissionInert:    {"Tool-node permission inert", "Remove `permission:` from the tool node; gate the agent nodes instead."},
	DiagInvalidPermissionRule:      {"Unreadable permission rule", "Write `Tool` or `Tool(pattern)` — e.g. `Bash(go test:*)`, `Read(**)`, `mcp__github__get_*`. The gate's own parser refuses this entry, so the node would fail at dispatch."},
	...
	DiagVarMatchingNonString:       {"Var matching on non-string type", "Declare the var as `string`, or drop the `[matching: ...]` constraint — for a non-string var the type is the constraint."},
	DiagVarDefaultNotMatching:      {"Var default does not match its pattern", "Make the default satisfy the pattern, or widen the pattern."},
	DiagVarMatchingUncompilable:    {"Var matching pattern is not valid RE2", "Fix the pattern; Go's regexp accepts RE2 syntax (no backreferences, no lookaround)."},
	DiagVarMatchingUnanchored:      {"Var matching pattern is not anchored", "Anchor it with `^...$` — Go matches by SEARCH, so an unanchored pattern also accepts a value that merely CONTAINS a match."},
	DiagPresetViolatesConstraint:   {"Preset value violates its var's constraint", "Use a value the var admits, or widen the `[enum: ...]` / `[matching: ...]` it declares — a run that selects this preset and keeps this value is refused at the launch gate (unless the engine resolves the var itself, as it does for `review_mode`, `mono_family`, `plan_review` and `llm_families`). A warning, not a refusal: no other run is affected. Only a value the run's expander leaves alone is judged; one it rewrites goes to the gate, which sees it expanded."},
	DiagVarRedeclaredUnconstrained: {"Var redeclared without its constraint", "Constrain the redeclaration too — with either `[enum: ...]` or `[matching: ...]`, both being string-only — or drop the constraint from both declarations. The later one REPLACES the earlier, so omitting it removes the guard in silence."},
	...
	DiagVarListDefaultRouting:   {"A site reads a list-typed var whose default moved", "The run reads a `string[]`/`json` default as a value of its declared type, the same value an override of the same text produces, so a site written against the compiler's TEXT reads something else: an expression answers differently (`length()` counts elements, an `==` against a string literal never matches), a `command:` loses a `string[]` to argv splitting (the first element is kept, the rest become a command of their own), a `script:` receives a literal where it received a quoted string. Supply the var at launch, write the site against the value (`vars.x[0] == 'a'`, `length(vars.x) > 0`, `\"${X}\"` around a shell reference that must stay one word), or declare the var `string` when the text is what is wanted."},
