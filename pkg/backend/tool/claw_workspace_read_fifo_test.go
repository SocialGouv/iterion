...
func TestWorkspaceDescriptorOpenRejectsSymlinkComponents(t *testing.T) {
	root := t.TempDir()
	outside := t.TempDir()
	...
	for _, parts := range [][]string{{"swapped", "secret"}, {"leaf"}} {
		f, err := openWorkspaceFileAt(root, parts)
		if f != nil {
			f.Close()
		}
		if err == nil {
			t.Fatalf("opened replaced path %v", parts)
		}
	}
}

func TestWorkspaceDescriptorOpenDoesNotWaitForFIFOWriter(t *testing.T) {
	root := t.TempDir()
	...
	go func() {
		f, _ := openWorkspaceFileAt(root, []string{"pipe"})
		if f != nil {
			f.Close()
		}
		close(done)
	...
	case <-time.After(2 * time.Second):
		t.Fatal("descriptor open blocked on a substituted FIFO")
	}
}
