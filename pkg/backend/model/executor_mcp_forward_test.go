...
func TestExplicitMCPWildcardBootFailureFailsTheNode(t *testing.T) {
	...
	// The two facts an operator needs to act: WHICH server, and WHICH entry of
	// the node's `tools:` list to go look at. Asserted on those rather than on
	// the prose around them, which is not a contract — the pre-change message
	// carried the server but not the wildcard, so this still has its teeth.
	if !strings.Contains(err.Error(), "deadsrv") || !strings.Contains(err.Error(), "mcp.deadsrv.*") {
		t.Errorf("error must name the server and the wildcard that asked for it: %v", err)
	}
	if len(degraded) != 0 {
