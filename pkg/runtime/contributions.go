...
func mirrorInjectedPluginFiles(workDir string, files []ContributionFile, logger *iterlog.Logger) ([]string, error) {
	// Signature stays 2-return: mirrorPluginContributions' injected
	// branch reports complete=true unconditionally (the wire IS the whole
	// declaration; whatever arrived was resolved on the launching
	// instance). Any hard error here still propagates up.
