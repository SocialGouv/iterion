...
func TestResumeRebuildStateLoadsAttachments(t *testing.T) {
	...
	rs, cleanup, rbErr := e.resumeRebuildState(ctx, r, cp, map[string]map[string]any{}, map[string]int{}, map[string]store.ArtifactRevisionRef{}, map[string]map[string]any{})
	if rbErr != nil {
