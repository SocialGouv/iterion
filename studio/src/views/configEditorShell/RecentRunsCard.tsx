...
// ---------------------------------------------------------------------------
// RecentRunsCard — a read-only glance at the recent runs of this config-share's
// (bot, category), so the editor can see the effect of their edits: did the
// last run happen, when, did it succeed. Status + timestamps only (the server
...
// the content editor. Bot-agnostic: "run" here is whatever the config-share's
// bot does on its cadence (a feed-watch digest, a report, …).
// ---------------------------------------------------------------------------
...
export function RecentRunsCard({
  ...
  const runsQuery = useQuery({
    ...
    // The cadence is typically slow (daily/weekly); a short poll is wasteful.
    staleTime: 60_000,
  });

  const runs = runsQuery.data;
  // Self-hide while loading, on error (incl. FeatureUnavailable), or when the
  // config has never run — there is nothing useful to show.
    ...
    <Card>
      <div className="flex flex-col gap-2">
        <div className="flex items-baseline justify-between">
          <h3 className="text-sm font-medium text-fg-default">Recent runs</h3>
          <span className="text-caption text-fg-subtle">
            {share.category ? share.category : "this config"}
          </span>
        </div>
        ...
        <p className="text-caption text-fg-subtle">
          This config runs on the cadence above. Your edits take effect on the next run.
        </p>
      </div>
    </Card>
  );
